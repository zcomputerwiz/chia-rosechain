...
# Embedded file name: chia\farmer\farmer_api.py
import json, time
...
from chia.protocols.pool_protocol import get_current_authentication_token, PoolErrorCode, PostPartialRequest, PostPartialPayload
...
class FarmerAPI:
    ...
    @api_request
    @peer_required
    async def new_proof_of_space(self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: ws.WSChiaConnection):
        """
        ...
        if self.farmer.constants.NETWORK_TYPE != NetworkType.MAINNET:
            if self.farmer.number_of_responses[new_proof_of_space.sp_hash] > max_pos_per_sp:
                self.farmer.log.info(f"Surpassed {max_pos_per_sp} PoSpace for one SP, no longer submitting PoSpace for signage point {new_proof_of_space.sp_hash}")
                return
        if new_proof_of_space.sp_hash not in self.farmer.sps:
            self.farmer.log.warning(f"Received response for a signage point that we do not have {new_proof_of_space.sp_hash}")
            return
        sps = self.farmer.sps[new_proof_of_space.sp_hash]
        for sp in sps:
            computed_quality_string = new_proof_of_space.proof.verify_and_get_quality_string(self.farmer.constants, new_proof_of_space.challenge_hash, new_proof_of_space.sp_hash)
            ...
            required_iters = calculate_iterations_quality(self.farmer.constants.DIFFICULTY_CONSTANT_FACTOR, computed_quality_string, new_proof_of_space.proof.size, sp.difficulty, new_proof_of_space.sp_hash)
            p2_singleton_rose_puzzle_hash = None
            if required_iters < calculate_sp_interval_iters(self.farmer.constants, sp.sub_slot_iters):
                request = harvester_protocol.RequestSignatures(new_proof_of_space.plot_identifier, new_proof_of_space.challenge_hash, new_proof_of_space.sp_hash, [
                 sp.challenge_chain_sp, sp.reward_chain_sp])
                p2_singleton_rose_puzzle_hash = self.farmer.get_rose_hash_by_proof(new_proof_of_space.proof)
                if p2_singleton_rose_puzzle_hash is not None:
                    pool_target = PoolTarget(p2_singleton_rose_puzzle_hash, uint32(0))
                    pool_target_Hash = std_hash(bytes(pool_target))
                    request = harvester_protocol.RequestSignatures(new_proof_of_space.plot_identifier, new_proof_of_space.challenge_hash, new_proof_of_space.sp_hash, [
            ...
            if p2_singleton_rose_puzzle_hash is None:
                p2_singleton_rose_puzzle_hash = self.farmer.get_rose_hash_by_proof(new_proof_of_space.proof)
            if p2_singleton_rose_puzzle_hash is not None:
                p2_singleton_puzzle_hash = p2_singleton_rose_puzzle_hash
            if p2_singleton_puzzle_hash is not None:
                if p2_singleton_puzzle_hash not in self.farmer.pool_state:
                    ...
                    pool_state_dict = self.farmer.pool_state[p2_singleton_puzzle_hash]
                    pool_url = pool_state_dict['pool_config'].pool_url
                    if pool_url == '':
                        return
                    if pool_state_dict['current_difficulty'] is None:
                        self.farmer.log.warning(f"No pool specific difficulty has been set for {p2_singleton_puzzle_hash}, check communication with the pool, skipping this partial to {pool_url}.")
                        return
                    required_iters = calculate_iterations_quality(self.farmer.constants.DIFFICULTY_CONSTANT_FACTOR * 274877906944, computed_quality_string, new_proof_of_space.proof.size, pool_state_dict['current_difficulty'], new_proof_of_space.sp_hash)
                    if required_iters >= calculate_sp_interval_iters(self.farmer.constants, self.farmer.constants.POOL_SUB_SLOT_ITERS):
                        self.farmer.log.info(f"Proof of space not good enough for pool {pool_url}: {pool_state_dict['current_difficulty']}")
                        return
                    authentication_token_timeout = pool_state_dict['authentication_token_timeout']
                    if authentication_token_timeout is None:
                        self.farmer.log.warning(f"No pool specific authentication_token_timeout has been set for {p2_singleton_puzzle_hash}, check communication with the pool.")
                        return
                    is_eos = new_proof_of_space.signage_point_index == 0
                    payload = PostPartialPayload(pool_state_dict['pool_config'].launcher_id, get_current_authentication_token(authentication_token_timeout), new_proof_of_space.proof, new_proof_of_space.sp_hash, is_eos, peer.peer_node_id, p2_singleton_puzzle_hash)
                    m_to_sign = payload.get_hash()
                    request = harvester_protocol.RequestSignatures(new_proof_of_space.plot_identifier, new_proof_of_space.challenge_hash, new_proof_of_space.sp_hash, [
                     m_to_sign])
                    response = await peer.request_signatures(request)
                    ...
                    for sk in self.farmer.get_private_keys():
                        ...
                        if pk == response.farmer_pk:
                            ...
                            if agg_pk != new_proof_of_space.proof.plot_public_key:
                                agg_pk = ProofOfSpace.generate_plot_public_key(response.local_pk, pk, False)
                            ...
                            taproot_sig = AugSchemeMPL.sign(taproot_sk, m_to_sign, agg_pk)
                            if include_taproot == False:
                                taproot_sig = G2Element()
                            plot_signature = AugSchemeMPL.aggregate([
                             sig_farmer, response.message_signatures[0][1], taproot_sig])
                            if not AugSchemeMPL.verify(agg_pk, m_to_sign, plot_signature):
                    ...
                    if bytes(authentication_pk) is None:
                        ...
                        return
                    authentication_sk = self.farmer.authentication_keys[bytes(authentication_pk)]
                    ...
                    post_partial_request = PostPartialRequest(payload, agg_sig)
                    self.farmer.log.info(f"Submitting partial for {post_partial_request.payload.launcher_id.hex()} to {pool_url}")
                    ...
                    try:
                        async with aiohttp.ClientSession() as session:
                            async with session.post(f"{pool_url}/partial",
                              json=(post_partial_request.to_json_dict()),
                              ssl=(ssl_context_for_root(get_mozilla_ca_crt()))) as resp:
                                if resp.ok:
                                    pool_response = json.loads(await resp.text())
                                    self.farmer.log.info(f"Pool response: {pool_response}")
                                    if 'error_code' in pool_response:
                                        self.farmer.log.error(f"Error in pooling: {(pool_response['error_code'], pool_response['error_message'])}")
                                        ...
                                        if pool_response['error_code'] == PoolErrorCode.PROOF_NOT_GOOD_ENOUGH.value:
                                            self.farmer.log.error('Partial not good enough, forcing pool farmer update to get our current difficulty.')
    ...
    async def respond_signatures(self, response: harvester_protocol.RespondSignatures):
        ...
        if found_sp_hash_debug:
            ...
            include_taproot = pospace.pool_contract_puzzle_hash is not None
            computed_quality_string = pospace.verify_and_get_quality_string(self.farmer.constants, response.challenge_hash, response.sp_hash)
            if computed_quality_string is None:
                self.farmer.log.warning(f"Have invalid PoSpace {pospace}")
                return
        rose_pool_contract_sp = None
        rose_pool_contract_sp_sig = None
        ...
        if is_sp_signatures:
            ...
            if len(response.message_signatures) >= 3:
                rose_pool_contract_sp, rose_pool_contract_sp_sig = response.message_signatures[2]
            for sk in self.farmer.get_private_keys():
                pk = sk.get_g1()
                if pk == response.farmer_pk:
                    agg_pk = ProofOfSpace.generate_plot_public_key(response.local_pk, pk, include_taproot)
                    if not agg_pk == pospace.plot_public_key:
                        raise AssertionError
                    else:
                        if include_taproot:
                            taproot_sk = ProofOfSpace.generate_taproot_sk(response.local_pk, pk)
                            taproot_share_cc_sp = AugSchemeMPL.sign(taproot_sk, challenge_chain_sp, agg_pk)
                            taproot_share_rc_sp = AugSchemeMPL.sign(taproot_sk, reward_chain_sp, agg_pk)
                            if rose_pool_contract_sp is not None:
                                taproot_share_rose_contract_sp = AugSchemeMPL.sign(taproot_sk, rose_pool_contract_sp, agg_pk)
                        else:
                            taproot_share_cc_sp = G2Element()
                            taproot_share_rc_sp = G2Element()
                            taproot_share_rose_contract_sp = G2Element()
                        ...
                        p2_singleton_rose_puzzle_hash = self.farmer.get_rose_hash_by_proof(pospace)
                        if rose_pool_contract_sp is not None and rose_pool_contract_sp_sig is not None and p2_singleton_rose_puzzle_hash is not None:
                            rose_share_sp = AugSchemeMPL.sign(sk, rose_pool_contract_sp, agg_pk)
                            agg_sig_rose_pool_sig = AugSchemeMPL.aggregate([
                             rose_pool_contract_sp_sig,
                             rose_share_sp, taproot_share_rose_contract_sp])
                            assert AugSchemeMPL.verify(agg_pk, rose_pool_contract_sp, agg_sig_rose_pool_sig)
                            pool_target = PoolTarget(p2_singleton_rose_puzzle_hash, uint32(0))
                            ...
                            if pospace.pool_public_key is not None:
                                ...
                                if pool_pk not in self.farmer.pool_sks_map:
                                    ...
                                    return
                                pool_target = PoolTarget(self.farmer.pool_target, uint32(0))
                                assert pool_target is not None
                                pool_target_signature = AugSchemeMPL.sign(self.farmer.pool_sks_map[pool_pk], bytes(pool_target))
                            else:
                                assert pospace.pool_contract_puzzle_hash is not None
                                pool_target = None
                                pool_target_signature = None
                        request = farmer_protocol.DeclareProofOfSpace(response.challenge_hash, challenge_chain_sp, signage_point_index, reward_chain_sp, pospace, agg_sig_cc_sp, agg_sig_rc_sp, self.farmer.farmer_target, pool_target, pool_target_signature)
            ...
            for sk in self.farmer.get_private_keys():
                ...
                foliage_transaction_block_hash, foliage_transaction_block_sig_harvester = response.message_signatures[1]
                pk = sk.get_g1()
                if pk == response.farmer_pk:
                    agg_pk = ProofOfSpace.generate_plot_public_key(response.local_pk, pk, include_taproot)
                    if not agg_pk == pospace.plot_public_key:
                        ...
                        if include_taproot:
                            taproot_sk = ProofOfSpace.generate_taproot_sk(response.local_pk, pk)
                            foliage_sig_taproot = AugSchemeMPL.sign(taproot_sk, foliage_block_data_hash, agg_pk)
                            foliage_transaction_block_sig_taproot = AugSchemeMPL.sign(taproot_sk, foliage_transaction_block_hash, agg_pk)
                        else:
                        ...
                        request_to_nodes = farmer_protocol.SignedValues(computed_quality_string, foliage_agg_sig, foliage_block_agg_sig)
    ...
    async def new_signage_point(self, new_signage_point: farmer_protocol.NewSignagePoint):
        ...
        for p2_singleton_puzzle_hash, pool_dict in self.farmer.pool_state.items():
            ...
            if pool_dict['current_difficulty'] is None:
                self.farmer.log.warning(f"No pool specific difficulty has been set for {p2_singleton_puzzle_hash}, check communication with the pool, skipping this signage point, pool: {pool_dict['pool_config'].pool_url} ")
                continue
            else:
                pool_chia_c = None
                pool_chia_p = None
                if p2_singleton_puzzle_hash in self.farmer.pool_state_map_rose_to_chia:
                    pool_chia_c = self.farmer.pool_state_map_rose_to_chia[p2_singleton_puzzle_hash]
                if p2_singleton_puzzle_hash in self.farmer.pool_state_map_rose_to_ppky_contract:
                    pool_chia_p = self.farmer.pool_state_map_rose_to_ppky_contract[p2_singleton_puzzle_hash]
                pool_difficulties.append(PoolDifficulty(pool_dict['current_difficulty'], self.farmer.constants.POOL_SUB_SLOT_ITERS, p2_singleton_puzzle_hash, pool_chia_c, pool_chia_p))

        message = harvester_protocol.NewSignagePointHarvester(new_signage_point.challenge_hash, new_signage_point.difficulty, new_signage_point.sub_slot_iters, new_signage_point.signage_point_index, new_signage_point.challenge_chain_sp, pool_difficulties)
    ...
    async def request_signed_values(self, full_node_request: farmer_protocol.RequestSignedValues):
        if full_node_request.quality_string not in self.farmer.quality_str_to_identifiers:
            ...
            return
        plot_identifier, challenge_hash, sp_hash, node_id = self.farmer.quality_str_to_identifiers[full_node_request.quality_string]
        request = harvester_protocol.RequestSignatures(plot_identifier, challenge_hash, sp_hash, [
    ...
    async def farming_info(self, request: farmer_protocol.FarmingInfo):
        ...
        self.farmer.state_changed('new_farming_info', {'farming_info': {'challenge_hash':request.challenge_hash, 
