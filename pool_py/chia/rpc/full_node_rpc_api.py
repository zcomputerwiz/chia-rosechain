...
# Embedded file name: chia\rpc\full_node_rpc_api.py
...
class FullNodeRpcApi:
    ...
    def get_routes(self) -> Dict[(str, Callable)]:
        return {'/get_blockchain_state':self.get_blockchain_state, 
    ...
    async def get_blockchain_state(self, _request: Dict):
        ...
        if self.service.initialized is False:
            res = {'blockchain_state': {'peak':None, 
        ...
        sync_mode = self.service.sync_store.get_sync_mode() or self.service.sync_store.get_long_sync()
        sync_tip_height = uint32(0)
        if sync_mode:
            ...
            if peak is not None:
                sync_progress_height = peak.height
        ...
        if peak is not None and peak.height > 1:
            ...
            space = await self.get_network_space({'newer_block_header_hash':newer_block_hex, 
        ...
        response = {'blockchain_state': {'peak':peak, 
                              'genesis_challenge_initialized':self.service.initialized, 
                              'sync':{
                                'sync_mode': sync_mode,
                                'synced': synced,
    ...
    async def get_recent_signage_point_or_eos(self, request: Dict):
        if 'sp_hash' not in request:
            challenge_hash = hexstr_to_bytes(request['challenge_hash'])
            ...
            curr = self.service.blockchain.get_peak()
            if curr is None:
            ...
            while number_of_slots_searched < 10:
                if curr.first_in_sub_slot:
                    if not curr.finished_challenge_slot_hashes is not None:
                        ...
                        if curr.finished_challenge_slot_hashes[-1] == eos.challenge_chain.get_hash():
                            return {'eos':eos, 
                             'time_received':time_received,  'reverted':False}
                ...
                if curr is None:
                    return {'eos':eos, 
                     'time_received':time_received,  'reverted':True}
        ...
        rc_challenge = sp.rc_vdf.challenge
        next_b = None
        curr_b_optional = self.service.blockchain.get_peak()
        ...
        for _ in range(200):
            ...
            if curr_b.reward_infusion_new_challenge == rc_challenge:
                if next_b is None:
                    return {'signage_point':sp,  'time_received':time_received,  'reverted':False}
                next_b_total_iters = next_b.ip_sub_slot_total_iters(self.service.constants) + next_b.ip_iters(self.service.constants)
                return {'signage_point':sp, 
    ...
    async def get_block(self, request: Dict) -> Optional[Dict]:
        ...
        block = await self.service.block_store.get_full_block(header_hash)
        if block is None:
    ...
    async def get_blocks(self, request: Dict) -> Optional[Dict]:
        ...
        blocks = await self.service.block_store.get_full_blocks_at(block_range)
        json_blocks = []
        for block in blocks:
    ...
    async def get_block_records(self, request: Dict) -> Optional[Dict]:
        ...
        for a in range(start, end):
            if peak_height < uint32(a):
                ...
                header_hash = self.service.blockchain.height_to_hash(uint32(a))
                record = self.service.blockchain.try_block_record(header_hash)
                if record is None:
    ...
    async def get_block_record_by_height(self, request: Dict) -> Optional[Dict]:
        ...
        header_hash = self.service.blockchain.height_to_hash(header_height)
    ...
    async def get_network_space(self, request: Dict) -> Optional[Dict]:
        ...
        network_space_bytes_estimate = UI_ACTUAL_SPACE_CONSTANT_FACTOR * weight_div_iters * additional_difficulty_constant * eligible_plots_filter_multiplier
    ...
    async def get_coin_records_by_puzzle_hash(self, request: Dict) -> Optional[Dict]:
        ...
        kwargs = {'include_spent_coins':False, 
    ...
    async def get_coin_records_by_puzzle_hashes(self, request: Dict) -> Optional[Dict]:
        ...
        kwargs = {'include_spent_coins':False,  'puzzle_hashes':[hexstr_to_bytes(ph) for ph in request['puzzle_hashes']]}
        if 'start_height' in request:
            kwargs['start_height'] = uint32(request['start_height'])
        if 'end_height' in request:
            kwargs['end_height'] = uint32(request['end_height'])
    ...
    async def get_coin_record_by_name(self, request: Dict) -> Optional[Dict]:
        ...
        coin_record = await self.service.blockchain.coin_store.get_coin_record(name)
    ...
    async def get_coin_records_by_parent_ids(self, request: Dict) -> Optional[Dict]:
        ...
        kwargs = {'include_spent_coins':False,  'parent_ids':[hexstr_to_bytes(ph) for ph in request['parent_ids']]}
        if 'start_height' in request:
            kwargs['start_height'] = uint32(request['start_height'])
        if 'end_height' in request:
            kwargs['end_height'] = uint32(request['end_height'])
    ...
    async def get_puzzle_and_solution(self, request: Dict) -> Optional[Dict]:
        coin_name = hexstr_to_bytes(request['coin_id'])
        height = request['height']
        coin_record = await self.service.coin_store.get_coin_record(coin_name)
        if not coin_record is None:
            if not coin_record.spent or coin_record.spent_block_index != height:
            ...
            block_generator = await self.service.blockchain.get_block_generator(block)
            assert block_generator is not None
            error, puzzle, solution = get_puzzle_and_solution_for_coin(block_generator, coin_name, self.service.constants.MAX_BLOCK_COST_CLVM)
            if error is not None:
                raise ValueError(f"Error: {error}")
            puzzle_ser = SerializedProgram.from_program(Program.to(puzzle))
            solution_ser = SerializedProgram.from_program(Program.to(solution))
    ...
    async def get_additions_and_removals(self, request: Dict) -> Optional[Dict]:
        ...
        async with self.service.blockchain.lock:
            ...
            additions = await self.service.coin_store.get_coins_added_at_height(block.height)
            removals = await self.service.coin_store.get_coins_removed_at_height(block.height)
    ...
    async def get_mempool_item_by_tx_id(self, request: Dict) -> Optional[Dict]:
        ...
        tx_id = hexstr_to_bytes(request['tx_id'])
