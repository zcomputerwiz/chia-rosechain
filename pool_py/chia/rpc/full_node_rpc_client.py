...
# Embedded file name: chia\rpc\full_node_rpc_client.py
...
class FullNodeRpcClient(RpcClient):
    ...
    async def get_network_space(self, newer_block_header_hash: bytes32, older_block_header_hash: bytes32) -> Optional[uint64]:
        try:
            network_space_bytes_estimate = await self.fetch('get_network_space', {'newer_block_header_hash':newer_block_header_hash.hex(), 
    ...
    async def get_coin_records_by_puzzle_hash(self, puzzle_hash: bytes32, include_spent_coins: bool=True, start_height: Optional[int]=None, end_height: Optional[int]=None) -> List:
        ...
        return [CoinRecord.from_json_dict(coin) for coin in (await self.fetch('get_coin_records_by_puzzle_hash', d))['coin_records']]

    async def get_coin_records_by_puzzle_hashes(self, puzzle_hashes: List[bytes32], include_spent_coins: bool=True, start_height: Optional[int]=None, end_height: Optional[int]=None) -> List:
        ...
        if end_height is not None:
            d['end_height'] = end_height
        return [CoinRecord.from_json_dict(coin) for coin in (await self.fetch('get_coin_records_by_puzzle_hashes', d))['coin_records']]

    async def get_coin_records_by_parent_ids(self, parent_ids: List[bytes32], include_spent_coins: bool=True, start_height: Optional[int]=None, end_height: Optional[int]=None) -> List:
        ...
        if end_height is not None:
            d['end_height'] = end_height
        return [CoinRecord.from_json_dict(coin) for coin in (await self.fetch('get_coin_records_by_parent_ids', d))['coin_records']]
    ...
    async def get_recent_signage_point_or_eos(self, sp_hash: Optional[bytes32], challenge_hash: Optional[bytes32]) -> Optional[Any]:
        try:
            if sp_hash is not None:
                ...
                return {'signage_point':SignagePoint.from_json_dict(response['signage_point']), 
            ...
            return {'eos':EndOfSubSlotBundle.from_json_dict(response['eos']), 
             'time_received':response['time_received'], 
             'reverted':response['reverted']}
        except Exception:
            return