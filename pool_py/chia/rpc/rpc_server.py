...
# Embedded file name: chia\rpc\rpc_server.py
...
class RpcServer:
    ...
    async def _state_changed(self, *args):
        if self.websocket is None:
            return
        payloads = await (self.rpc_api._state_changed)(*args)
        change = args[0]
        ...
        if self.rpc_api.service.server._local_type is NodeType.FULL_NODE:
            ...
            for con in connections:
                if peak_store is not None and con.peer_node_id in peak_store:
                    ...
                    peak_hash = None
                    peak_weight = None
                con_dict = {'type':con.connection_type,  'local_port':con.local_port, 
            ...
            try:
                if self.shut_down:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.ws_connect(f"wss://{self_hostname}:{daemon_port}",
                          autoclose=True,
                          autoping=True,
                          heartbeat=60,
                          ssl_context=(self.ssl_context),
                          max_msg_size=104857600) as ws:
                            self.websocket = ws
...
async def start_rpc_server(rpc_api, self_hostname, daemon_port, rpc_port, stop_cb, root_path, net_config, connect_to_daemon=True):
    """
    ...
    http_routes = rpc_api.get_routes()
