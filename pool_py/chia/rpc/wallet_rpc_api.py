...
# Embedded file name: chia\rpc\wallet_rpc_api.py
...
class WalletRpcApi:
    ...
    async def log_in(self, request):
        ...
        testing = False
        if 'testing' in self.service.config:
            if self.service.config['testing'] is True:
                testing = True
        if log_in_type == 'skip':
            started = await self.service._start(fingerprint=fingerprint, skip_backup_import=True)
        else:
            if log_in_type == 'restore_backup':
    ...
    async def get_private_key(self, request):
        ...
        if sk is not None:
            ...
            return {'private_key': {'fingerprint':fingerprint, 
                             'sk':bytes(sk).hex(), 
                             ...
                             'seed':s}}
        return {'success':False, 
         'private_key':{'fingerprint': fingerprint}}
    ...
    async def add_key(self, request):
        ...
        if request_type == 'new_wallet':
            ...
            if request_type == 'skip':
                started = await self.service._start(fingerprint=fingerprint, skip_backup_import=True)
            else:
                if request_type == 'restore_backup':
                    file_path = Path(request['file_path'])
                    started = await self.service._start(fingerprint=fingerprint, backup_file=file_path)
        if started is True:
            return {'fingerprint': fingerprint}
    ...
    async def _check_key_used_for_rewards(self, new_root: Path, sk: PrivateKey, max_ph_to_search: int) -> Tuple[(bool, bool)]:
    ...
    async def check_delete_key(self, request):
        ...
        if sk is not None:
            ...
            async with self.service.wallet_state_manager.lock:
                wallets = await self.service.wallet_state_manager.get_all_wallet_info_entries()
                for w in wallets:
    ...
    async def get_sync_status(self, request: Dict):
        ...
        return {'synced':synced, 
         'syncing':syncing,  'genesis_initialized':True}
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request['wallet_type'] == 'cc_wallet':
            if request['mode'] == 'new':
                async with self.service.wallet_state_manager.lock:
                    cc_wallet = await CCWallet.create_new_cc(wallet_state_manager, main_wallet, request['amount'])
        ...
        if request['wallet_type'] == 'rl_wallet':
            if request['rl_type'] == 'admin':
                ...
                async with self.service.wallet_state_manager.lock:
                    rl_admin = await RLWallet.create_rl_admin(wallet_state_manager)
                    success = await rl_admin.admin_create_coin(uint64(int(request['interval'])), uint64(int(request['limit'])), request['pubkey'], uint64(int(request['amount'])), uint64(int(request['fee'])) if 'fee' in request else uint64(0))
                    asyncio.create_task(self._create_backup_and_upload(host))
            ...
            if request['rl_type'] == 'user':
                log.info('Create rl user wallet')
                async with self.service.wallet_state_manager.lock:
                    rl_user = await RLWallet.create_rl_user(wallet_state_manager)
                    asyncio.create_task(self._create_backup_and_upload(host))
                assert rl_user.rl_info.user_pubkey is not None
                return {'id':rl_user.id(), 
                 'type':rl_user.type(), 
            ...
            if request['wallet_type'] == 'did_wallet':
                if request['did_type'] == 'new':
                    ...
                    async with self.service.wallet_state_manager.lock:
                        did_wallet = await DIDWallet.create_new_did_wallet(wallet_state_manager, main_wallet, int(request['amount']), backup_dids, uint64(num_needed))
                    my_did = did_wallet.get_my_DID()
                    return {'success':True, 
                     'type':did_wallet.type(), 
                     'my_did':my_did, 
                     'wallet_id':did_wallet.id()}
                if request['did_type'] == 'recovery':
                    async with self.service.wallet_state_manager.lock:
                        did_wallet = await DIDWallet.create_new_did_wallet_from_recovery(wallet_state_manager, main_wallet, request['filename'])
                ...
                if request['wallet_type'] == 'pool_wallet':
                    if request['mode'] == 'new':
                        owner_puzzle_hash = await self.service.wallet_state_manager.main_wallet.get_puzzle_hash(True)
                        from chia.pools.pool_wallet_info import initial_pool_state_from_dict
                        async with self.service.wallet_state_manager.lock:
                            last_wallet = await self.service.wallet_state_manager.user_store.get_last_wallet()
                            assert last_wallet is not None
                            next_id = last_wallet.id + 1
                            owner_sk = master_sk_to_singleton_owner_sk(self.service.wallet_state_manager.private_key, uint32(next_id))
                            owner_pk = owner_sk.get_g1()
                            initial_target_state = initial_pool_state_from_dict(request['initial_target_state'], owner_pk, owner_puzzle_hash)
                            ...
                            try:
                                ...
                                tr, p2_singleton_puzzle_hash, launcher_id = await PoolWallet.create_new_pool_wallet_transaction(wallet_state_manager, main_wallet, initial_target_state, fee, request.get('p2_singleton_delay_time', None), delayed_address, request.get('plot_contract_address', None))
                            except Exception as e:
                                try:
    ...
    async def get_wallet_balance(self, request: Dict) -> Dict:
        ...
        async with self.service.wallet_state_manager.lock:
            ...
            unconfirmed_removals = await wallet.wallet_state_manager.unconfirmed_removals_for_wallet(wallet_id)
    ...
    async def get_transaction(self, request: Dict) -> Dict:
        assert self.service.wallet_state_manager is not None
        transaction_id = bytes32(hexstr_to_bytes(request['transaction_id']))
        tr = await self.service.wallet_state_manager.get_transaction(transaction_id)
        if tr is None:
    ...
    async def send_transaction(self, request):
        ...
        wallet_id = int(request['wallet_id'])
        wallet = self.service.wallet_state_manager.wallets[wallet_id]
        if not (isinstance(request['amount'], int) and isinstance(request['fee'], int)):
            raise ValueError('An integer amount or fee is required (too many decimals)')
        amount = uint64(request['amount'])
        puzzle_hash = decode_puzzle_hash(request['address'])
        if 'fee' in request:
            fee = uint64(request['fee'])
        else:
            fee = uint64(0)
        async with self.service.wallet_state_manager.lock:
            tx = await wallet.generate_signed_transaction(amount, puzzle_hash, fee)
    ...
    async def cc_spend(self, request):
        ...
        wallet_id = int(request['wallet_id'])
        wallet = self.service.wallet_state_manager.wallets[wallet_id]
        puzzle_hash = decode_puzzle_hash(request['inner_address'])
        if not (isinstance(request['amount'], int) and isinstance(request['amount'], int)):
            raise ValueError('An integer amount or fee is required (too many decimals)')
        amount = uint64(request['amount'])
        if 'fee' in request:
            fee = uint64(request['fee'])
    ...
    async def create_offer_for_ids(self, request):
        ...
        async with self.service.wallet_state_manager.lock:
            success, spend_bundle, error = await self.service.wallet_state_manager.trade_manager.create_offer_for_ids(offer, file_name)
        if success:
    ...
    async def get_discrepancies_for_offer(self, request):
        ...
        async with self.service.wallet_state_manager.lock:
            success, discrepancies, error = await self.service.wallet_state_manager.trade_manager.get_discrepancies_for_offer(file_path)
        if success:
    ...
    async def respond_to_offer(self, request):
        ...
        async with self.service.wallet_state_manager.lock:
            success, trade_record, error = await self.service.wallet_state_manager.trade_manager.respond_to_offer(file_path)
        if not success:
            raise ValueError(error)
        return {}
    ...
    async def did_get_did(self, request):
        ...
        coin = coins.pop()
        return {'success':True, 
         'wallet_id':wallet_id,  'my_did':my_did,  'coin_id':coin.name()}
    ...
    async def did_recovery_spend(self, request):
        ...
        async with self.service.wallet_state_manager.lock:
            ...
            success = await wallet.recovery_spend(wallet.did_info.temp_coin, puzhash, info_list, pubkey, message_spend_bundle)
        return {'success': success}
    ...
    async def did_create_attest(self, request):
        ...
        async with self.service.wallet_state_manager.lock:
            ...
            pubkey = G1Element.from_bytes(hexstr_to_bytes(request['pubkey']))
            spend_bundle = await wallet.create_attestment(coin, hexstr_to_bytes(request['puzhash']), pubkey, request['filename'])
        if spend_bundle is not None:
            return {'success':True, 
             'message_spend_bundle':bytes(spend_bundle).hex(), 
             'info':[
              info[0].hex(), info[1].hex(), info[2]]}
        return {'success': False}
    ...
    async def rl_set_user_info(self, request):
        ...
        async with self.service.wallet_state_manager.lock:
            await rl_user.set_user_info(uint64(request['interval']), uint64(request['limit']), origin['parent_coin_info'], origin['puzzle_hash'], origin['amount'], request['admin_pubkey'])
        return {}
    ...
    async def get_farmed_amount(self, request):
        tx_records = await self.service.wallet_state_manager.tx_store.get_farming_rewards()
        amount = 0
    ...
    async def create_signed_transaction(self, request, hold_lock=True):
        ...
        amount_0 = uint64(additions[0]['amount'])
        ...
        if hold_lock:
            async with self.service.wallet_state_manager.lock:
                signed_tx = await self.service.wallet_state_manager.main_wallet.generate_signed_transaction(amount_0,
                  puzzle_hash_0, fee, coins=coins, ignore_max_send_amount=True, primaries=additional_outputs)
        else:
            signed_tx = await self.service.wallet_state_manager.main_wallet.generate_signed_transaction(amount_0,
              puzzle_hash_0, fee, coins=coins, ignore_max_send_amount=True, primaries=additional_outputs)
    ...
    async def pw_join_pool(self, request):
        wallet_id = uint32(request['wallet_id'])
        wallet = self.service.wallet_state_manager.wallets[wallet_id]
        pool_wallet_info = await wallet.get_current_state()
        ...
        new_target_state = create_pool_state(FARMING_TO_POOL, target_puzzlehash, owner_pubkey, request['pool_url'], uint32(request['relative_lock_height']))
        async with self.service.wallet_state_manager.lock:
    ...
    async def pw_absorb_rewards(self, request):
        ...
        fee = uint64(request['fee'])
        async with self.service.wallet_state_manager.lock:
            transaction = await wallet.claim_pool_rewards(fee)
    ...
    async def pw_status(self, request):
        ...
        state = await wallet.get_current_state()
        unconfirmed_transactions = await wallet.get_unconfirmed_transactions()
