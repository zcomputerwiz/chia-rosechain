...
# Embedded file name: chia\rpc\farmer_rpc_api.py
...
class FarmerRpcApi:
    ...
    def get_routes(self) -> Dict[(str, Callable)]:
        return {'/get_signage_point':self.get_signage_point, 
    ...
    async def _state_changed(self, change: str, change_data: Dict) -> List[WsRpcMessage]:
        if change == 'new_signage_point':
            ...
            return [
             create_payload_dict('new_signage_point', data, self.service_name, 'wallet_ui')]
        if change == 'new_farming_info':
            return [
             create_payload_dict('new_farming_info', change_data, self.service_name, 'wallet_ui')]
        if change == 'new_plots':
            return [
             create_payload_dict('get_harvesters', change_data, self.service_name, 'wallet_ui')]
        return []
        ...
        for _, sps in self.service.sps.items():
            for sp in sps:
                if sp.challenge_chain_sp == sp_hash:
                    ...
                    return {'signage_point':{'challenge_hash':sp.challenge_hash, 
    ...
    async def get_signage_points(self, _: Dict) -> Dict:
        ...
        for _, sps in self.service.sps.items():
            for sp in sps:
                pospaces = self.service.proofs_of_space.get(sp.challenge_chain_sp, [])
                result.append({'signage_point':{'challenge_hash':sp.challenge_hash, 
                  'challenge_chain_sp':sp.challenge_chain_sp, 
                  'reward_chain_sp':sp.reward_chain_sp, 
                  'difficulty':sp.difficulty, 
                  'sub_slot_iters':sp.sub_slot_iters, 
    ...
    async def set_payout_instructions(self, request: Dict) -> Dict:
        launcher_id = hexstr_to_bytes(request['launcher_id'])
    ...
    async def get_pool_login_link(self, request: Dict) -> Dict:
        ...
        login_link = await self.service.generate_login_link(launcher_id)
