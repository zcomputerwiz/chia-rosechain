...
# Embedded file name: chia\timelord\timelord_state.py
import logging
...
class LastState:
    ...
    def __init__(self, constants: ConsensusConstants):
        self.state_type = StateType.FIRST_SUB_SLOT
        self.peak = None
        self.subslot_end = None
        self.last_ip = uint64(0)
        self.deficit = constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK
        self.sub_epoch_summary = None
        self.constants = constants
        self.last_weight = uint128(0)
        self.last_height = uint32(0)
        self.total_iters = uint128(0)
        self.last_challenge_sb_or_eos_total_iters = uint128(0)
        self.last_block_total_iters = None
        self.last_peak_challenge = constants.GENESIS_CHALLENGE
        self.difficulty = constants.DIFFICULTY_STARTING
        self.sub_slot_iters = constants.SUB_SLOT_ITERS_STARTING
        self.reward_challenge_cache = [(constants.GENESIS_CHALLENGE, uint128(0))]
    ...
    def set_state(self, state: Union[(timelord_protocol.NewPeakTimelord, EndOfSubSlotBundle)]):
        if isinstance(state, timelord_protocol.NewPeakTimelord):
            ...
            self.peak = state
            self.subslot_end = None
            _, self.last_ip = iters_from_block(self.constants, state.reward_chain_block, state.sub_slot_iters, state.difficulty)
    ...
    def get_next_sub_epoch_summary(self) -> Optional[SubEpochSummary]:
        ...
        if self.passed_ses_height_but_not_yet_included:
            if self.get_deficit() == 0:
    ...
    def get_challenge(self, chain: Chain) -> Optional[bytes32]:
        if self.state_type == StateType.FIRST_SUB_SLOT:
            if not (self.peak is None and self.subslot_end is None):
            ...
            if self.state_type == StateType.PEAK:
                assert self.peak is not None
                ...
                if chain == Chain.INFUSED_CHALLENGE_CHAIN:
                    ...
                    if self.peak.deficit == self.constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK - 1:
                        return ChallengeBlockInfo(reward_chain_block.proof_of_space, reward_chain_block.challenge_chain_sp_vdf, reward_chain_block.challenge_chain_sp_signature, reward_chain_block.challenge_chain_ip_vdf).get_hash()
                    return
            else:
                if self.state_type == StateType.END_OF_SUB_SLOT:
