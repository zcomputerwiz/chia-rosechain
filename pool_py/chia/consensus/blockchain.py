...
# Embedded file name: chia\consensus\blockchain.py
...
class Blockchain(BlockchainInterface):
    ...
    _Blockchain__block_records: Dict[(bytes32, BlockRecord)]
    _Blockchain__heights_in_cache: Dict[(uint32, Set[bytes32])]
    _Blockchain__height_to_hash: Dict[(uint32, bytes32)]
    _Blockchain__sub_epoch_summaries = {}
    _Blockchain__sub_epoch_summaries: Dict[(uint32, SubEpochSummary)]
    ...
    @staticmethod
    async def create(coin_store: CoinStore, block_store: BlockStore, consensus_constants: ConsensusConstants):
        """
    ...
    async def _load_chain_from_store(self) -> None:
        ...
        self._Blockchain__height_to_hash = height_to_hash
        self._Blockchain__sub_epoch_summaries = sub_epoch_summaries
        self._Blockchain__block_records = {}
        self._Blockchain__heights_in_cache = {}
        ...
        assert len(self._Blockchain__height_to_hash) == self._peak_height + 1
    ...
    async def receive_block(self, block: FullBlock, pre_validation_result: Optional[PreValidationResult]=None, fork_point_with_peak: Optional[uint32]=None) -> Tuple[(ReceiveBlockResult, Optional[Err], Optional[uint32])]:
        """
        ...
        if not self.contains_block(block.prev_header_hash):
            if not genesis:
                return (
                 ...
                 None)
            if not genesis:
                if self.block_record(block.prev_header_hash).height + 1 != block.height:
                    return (
                     ReceiveBlockResult.INVALID_BLOCK, Err.INVALID_HEIGHT, None)
            npc_result = None
            if pre_validation_result is None:
                ...
                if block.is_transaction_block():
                    if block.transactions_generator is not None:
                        try:
                            block_generator = await self.get_block_generator(block)
                        except ValueError:
                            return (
                             ReceiveBlockResult.INVALID_BLOCK, Err.GENERATOR_REF_HAS_NO_GENERATOR, None)
                        else:
                            if not (block_generator is not None and block.transactions_info is not None):
            ...
            async with self.block_store.db_wrapper.lock:
                try:
                    ...
                    fork_height, peak_height, records = await self._reconsider_peak(block_record, genesis, fork_point_with_peak, npc_result)
                    await self.block_store.db_wrapper.commit_transaction()
                    self.add_block_record(block_record)
                    for fetched_block_record in records:
                        self._Blockchain__height_to_hash[fetched_block_record.height] = fetched_block_record.header_hash
                        if fetched_block_record.sub_epoch_summary_included is not None:
                            self._Blockchain__sub_epoch_summaries[fetched_block_record.height] = fetched_block_record.sub_epoch_summary_included
    ...
    async def _reconsider_peak(self, block_record: BlockRecord, genesis: bool, fork_point_with_peak: Optional[uint32], npc_result: Optional[NPCResult]) -> Tuple[(Optional[uint32], Optional[uint32], List[BlockRecord])]:
        """
        ...
        if genesis:
            if peak is None:
                block = await self.block_store.get_full_block(block_record.header_hash)
                assert block is not None
                ...
                return (
                 uint32(0), uint32(0), [block_record])
            return (None, None, [])
        assert peak is not None
        if block_record.weight > peak.weight:
            if block_record.prev_hash == peak.header_hash:
                fork_height = peak.height
            else:
            ...
            for fetched_full_block, fetched_block_record in reversed(blocks_to_add):
                ...
                if fetched_block_record.is_transaction_block:
                    if fetched_block_record.header_hash == block_record.header_hash:
                        tx_removals, tx_additions = await self.get_tx_removals_and_additions(fetched_full_block, npc_result)
                    else:
            ...
            await self.block_store.set_peak(block_record.header_hash)
            return (
             uint32(max(fork_height, 0)), block_record.height, records_to_add)
        return (
         None, None, [])

    async def get_tx_removals_and_additions(self, block: FullBlock, npc_result: Optional[NPCResult]=None) -> Tuple[(List[bytes32], List[Coin])]:
        if block.is_transaction_block():
            if block.transactions_generator is not None:
    ...
    async def get_sp_and_ip_sub_slots(self, header_hash: bytes32) -> Optional[Tuple[(Optional[EndOfSubSlotBundle], Optional[EndOfSubSlotBundle])]]:
        block = await self.block_store.get_full_block(header_hash)
        if block is None:
            return
        curr_br = self.block_record(block.header_hash)
        ...
        if len(curr.finished_sub_slots) > 1:
            return (
             curr.finished_sub_slots[-2], ip_sub_slot)
        prev_curr = await self.block_store.get_full_block(curr.prev_header_hash)
    ...
    def get_recent_reward_challenges(self) -> List[Tuple[(bytes32, uint128)]]:
        ...
        while curr is not None:
            if len(recent_rc) < 2 * self.constants.MAX_SUB_SLOT_BLOCKS:
                if curr != peak:
    ...
    async def validate_unfinished_block(self, block: UnfinishedBlock, skip_overflow_ss_validation=True) -> PreValidationResult:
        if not self.contains_block(block.prev_header_hash):
            if not block.prev_header_hash == self.constants.GENESIS_CHALLENGE:
                return PreValidationResult(uint16(Err.INVALID_PREV_BLOCK_HASH.value), None, None)
            unfinished_header_block = UnfinishedHeaderBlock(block.finished_sub_slots, block.reward_chain_block, block.challenge_chain_sp_proof, block.reward_chain_sp_proof, block.foliage, block.foliage_transaction_block, b'')
            prev_b = self.try_block_record(unfinished_header_block.prev_header_hash)
            sub_slot_iters, difficulty = get_next_sub_slot_iters_and_difficulty(self.constants, len(unfinished_header_block.finished_sub_slots) > 0, prev_b, self)
            required_iters, error = validate_unfinished_header_block(self.constants, self, unfinished_header_block, False, difficulty, sub_slot_iters, skip_overflow_ss_validation)
            if error is not None:
                return PreValidationResult(uint16(error.code.value), None, None)
            prev_height = -1 if block.prev_header_hash == self.constants.GENESIS_CHALLENGE else self.block_record(block.prev_header_hash).height
            npc_result = None
            if block.transactions_generator is not None:
                ...
                error_code, cost_result = await validate_block_body(self.constants, self, self.block_store, self.coin_store, self.get_peak(), block, uint32(prev_height + 1), npc_result, None, self.get_block_generator)
                if error_code is not None:
                    return PreValidationResult(uint16(error_code.value), None, None)
            return PreValidationResult(None, required_iters, cost_result)

    async def pre_validate_blocks_multiprocessing(self, blocks: List[FullBlock], npc_results: Dict[(uint32, NPCResult)], batch_size: int=4, wp_summaries: Optional[List[SubEpochSummary]]=None) -> Optional[List[PreValidationResult]]:
        return await pre_validate_blocks_multiprocessing(self.constants, self.constants_json, self, blocks, self.pool, True, npc_results, self.get_block_generator, batch_size, wp_summaries)
    ...
    def contains_block(self, header_hash: bytes32) -> bool:
        ...
        """
        return header_hash in self._Blockchain__block_records

    def block_record(self, header_hash: bytes32) -> BlockRecord:
        return self._Blockchain__block_records[header_hash]
    ...
    def get_ses_heights(self) -> List[uint32]:
        return sorted(self._Blockchain__sub_epoch_summaries.keys())

    def get_ses(self, height: uint32) -> SubEpochSummary:
        return self._Blockchain__sub_epoch_summaries[height]

    def height_to_hash(self, height: uint32) -> Optional[bytes32]:
        return self._Blockchain__height_to_hash[height]

    def contains_height(self, height: uint32) -> bool:
        return height in self._Blockchain__height_to_hash
    ...
    def clean_block_record(self, height: int):
        ...
        if height < 0:
            return
        blocks_to_remove = self._Blockchain__heights_in_cache.get(uint32(height), None)
        while blocks_to_remove is not None:
            if height >= 0:
                for header_hash in blocks_to_remove:
                    del self._Blockchain__block_records[header_hash]

                del self._Blockchain__heights_in_cache[uint32(height)]
                if height == 0:
                    break
                else:
                    height = height - 1
    ...
    def clean_block_records(self):
        ...
        """
        if len(self._Blockchain__block_records) < self.constants.BLOCKS_CACHE_SIZE:
            return
        peak = self.get_peak()
        assert peak is not None
    ...
    async def get_header_blocks_in_range(self, start: int, stop: int, tx_filter: bool=True) -> Dict[(bytes32, HeaderBlock)]:
        ...
        for height in range(start, stop + 1):
            if self.contains_height(uint32(height)):
                header_hash = self.height_to_hash(uint32(height))
        ...
        blocks_on_disk = await self.block_store.get_blocks_by_hash(hashes)
        ...
        for block in blocks:
            if self.height_to_hash(block.height) != block.header_hash:
                ...
                if tx_filter is False:
                    ...
                    tx_additions = [c for c in await self.coin_store.get_coins_added_at_height(block.height) if not c.coinbase]
                    removed = await self.coin_store.get_coins_removed_at_height(block.height)
                    header = get_block_header(block, [record.coin for record in tx_additions], [record.coin.name() for record in removed])
                header_blocks[header.header_hash] = header

        return header_blocks

    async def get_header_block_by_height(self, height: int, header_hash: bytes32, tx_filter: bool=True) -> Optional[HeaderBlock]:
        header_dict = await self.get_header_blocks_in_range(height, height, tx_filter)
    ...
    async def get_block_record_from_db(self, header_hash: bytes32) -> Optional[BlockRecord]:
        if header_hash in self._Blockchain__block_records:
            return self._Blockchain__block_records[header_hash]
    ...
    def remove_block_record(self, header_hash: bytes32):
        ...
        del self._Blockchain__block_records[header_hash]
        self._Blockchain__heights_in_cache[sbr.height].remove(header_hash)

    def add_block_record(self, block_record: BlockRecord):
        """
        Adds a block record to the cache.
        """
        self._Blockchain__block_records[block_record.header_hash] = block_record
        if block_record.height not in self._Blockchain__heights_in_cache.keys():
            self._Blockchain__heights_in_cache[block_record.height] = set()
        self._Blockchain__heights_in_cache[block_record.height].add(block_record.header_hash)

    async def persist_sub_epoch_challenge_segments(self, ses_block_hash: bytes32, segments: List[SubEpochChallengeSegment]):
        return await self.block_store.persist_sub_epoch_challenge_segments(ses_block_hash, segments)

    async def get_sub_epoch_challenge_segments(self, ses_block_hash: bytes32) -> Optional[List[SubEpochChallengeSegment]]:
        segments = await self.block_store.get_sub_epoch_challenge_segments(ses_block_hash)
        if segments is None:
            return
        return segments
    ...
    async def get_block_generator(self, block: Union[(FullBlock, UnfinishedBlock)], additional_blocks=None) -> Optional[BlockGenerator]:
        ...
        if self.try_block_record(previous_block_hash) and self.height_to_hash(self.block_record(previous_block_hash).height) == previous_block_hash:
            ...
            while curr.prev_header_hash in additional_blocks:
                prev = additional_blocks[curr.prev_header_hash]
            ...
            if self.contains_block(curr.prev_header_hash):
                if peak is not None:
                    ...
                    curr_2 = prev_block
                    if not (curr_2 is not None and isinstance(curr_2, FullBlock)):
                        raise AssertionError
                    reorg_chain[curr_2.height] = curr_2
                    while curr_2.height > fork:
                        if curr_2.height > 0:
                            curr_2 = await self.block_store.get_full_block(curr_2.prev_header_hash)
                            if not curr_2 is not None:
                                raise AssertionError
                            else:
                                reorg_chain[curr_2.height] = curr_2
