...
# Embedded file name: chia\consensus\pot_iterations.py
...
def calculate_sp_iters(constants, sub_slot_iters, signage_point_index):
    if signage_point_index >= constants.NUM_SPS_SUB_SLOT:
        raise ValueError('SP index too high')
...
def calculate_ip_iters(constants, sub_slot_iters, signage_point_index, required_iters):
    ...
    if required_iters >= sp_interval_iters or required_iters == 0:
        raise ValueError(f"Required iters {required_iters} is not below the sp interval iters {sp_interval_iters} {sub_slot_iters} or not >0.")
    return uint64((sp_iters + constants.NUM_SP_INTERVALS_EXTRA * sp_interval_iters + required_iters) % sub_slot_iters)


def calculate_iterations_quality(difficulty_constant_factor, quality_string, size, difficulty, cc_sp_output_hash):
    """
    ...
    """
    sp_quality_string = std_hash(quality_string + cc_sp_output_hash)
    iters = uint64(int(difficulty) * int(difficulty_constant_factor) * int.from_bytes(sp_quality_string, 'big', signed=False) // (int(pow(2, 256)) * int(_expected_plot_size(size))))
