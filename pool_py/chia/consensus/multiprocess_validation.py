...
# Embedded file name: chia\consensus\multiprocess_validation.py
...
import chia.consensus.get_block_challenge as get_block_challenge
...
class PreValidationResult(Streamable):
    ...
    required_iters: Optional[uint64]
                    ...
                    try:
                        ...
                        if block.transactions_generator is not None:
                            if npc_result is None:
                                ...
                                block_generator = BlockGenerator.from_bytes(prev_generator_bytes)
                        ...
                        required_iters, error = validate_finished_header_block(constants, BlockCache(blocks), header_block, check_filter, expected_difficulty[i], expected_sub_slot_iters[i])
                        error_int = None
...
async def pre_validate_blocks_multiprocessing(constants: ConsensusConstants, constants_json: Dict, block_records: BlockchainInterface, blocks: Sequence[Union[(FullBlock, HeaderBlock)]], pool: ProcessPoolExecutor, check_filter: bool, npc_results: Dict[(uint32, NPCResult)], get_block_generator: Optional[Callable], batch_size: int, wp_summaries: Optional[List[SubEpochSummary]]=None) -> Optional[List[PreValidationResult]]:
    ...
    if blocks[0].height > 0:
        ...
        while curr.sub_epoch_summary_included is None or num_blocks_seen < constants.NUMBER_OF_TIMESTAMPS or num_sub_slots_found < num_sub_slots_to_look_for:
            if curr.height > 0:
    ...
    for block in blocks:
        ...
        sub_slot_iters, difficulty = get_next_sub_slot_iters_and_difficulty(constants, len(block.finished_sub_slots) > 0, prev_b, block_records)
        ...
        q_str = block.reward_chain_block.proof_of_space.verify_and_get_quality_string(constants, challenge, cc_sp_hash)
        if q_str is None:
            for i, block_i in enumerate(blocks):
                if not block_record_was_present[i]:
                    if block_records.contains_block(block_i.header_hash):
                        block_records.remove_block_record(block_i.header_hash)

            return
        required_iters = calculate_iterations_quality(constants.DIFFICULTY_CONSTANT_FACTOR, q_str, block.reward_chain_block.proof_of_space.size, difficulty, cc_sp_hash)
        block_rec = block_to_block_record(constants, block_records, required_iters, block, None)
    ...
    for i in range(0, len(blocks), batch_size):
        ...
        for block in blocks_to_validate:
            ...
            if isinstance(block, FullBlock):
                if not get_block_generator is not None:
                    ...
                    try:
                        block_generator = await get_block_generator(block, prev_blocks_dict)
                    except ValueError:
                        return
        ...
        futures.append(asyncio.get_running_loop().run_in_executor(pool, batch_pre_validate_blocks, constants_json, final_pickled, b_pickled, hb_pickled, previous_generators, npc_results_pickled, check_filter, [diff_ssis[j][0] for j in range(i, end_i)], [diff_ssis[j][1] for j in range(i, end_i)]))

    return [PreValidationResult.from_bytes(result) for batch_result in await (asyncio.gather)(*futures) for result in iter(batch_result)]