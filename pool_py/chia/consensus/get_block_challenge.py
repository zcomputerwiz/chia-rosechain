...
# Embedded file name: chia\consensus\get_block_challenge.py
import logging
...
def final_eos_is_already_included(header_block: Union[(UnfinishedHeaderBlock, UnfinishedBlock, HeaderBlock, FullBlock)], blocks: BlockchainInterface, sub_slot_iters: uint64) -> bool:
    ...
    while not curr.first_in_sub_slot:
        if not curr.height == 0:
            if curr.overflow:
                if header_block.total_iters - curr.total_iters < sub_slot_iters // 2:
                    seen_overflow_block = True
            curr = blocks.block_record(curr.prev_hash)

    if curr.first_in_sub_slot:
        if seen_overflow_block:
            return True
    return False


def get_block_challenge(constants: ConsensusConstants, header_block: Union[(UnfinishedHeaderBlock, UnfinishedBlock, HeaderBlock, FullBlock)], blocks: BlockchainInterface, genesis_block: bool, overflow: bool, skip_overflow_last_ss_validation: bool):
    if len(header_block.finished_sub_slots) > 0:
        if overflow:
            if skip_overflow_last_ss_validation:
                challenge = header_block.finished_sub_slots[-1].challenge_chain.get_hash()
            else:
                challenge = header_block.finished_sub_slots[-1].challenge_chain.challenge_chain_end_of_slot_vdf.challenge
        else:
            challenge = header_block.finished_sub_slots[-1].challenge_chain.get_hash()
    else:
        if genesis_block:
            ...
            while len(reversed_challenge_hashes) < challenges_to_look_for:
                ...
                if curr.height == 0:
                    ...
                    elif not len(curr.finished_challenge_slot_hashes) > 0:
                        raise AssertionError
                    break
                else:
                    curr = blocks.block_record(curr.prev_hash)
