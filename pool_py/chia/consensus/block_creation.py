...
# Embedded file name: chia\consensus\block_creation.py
...
import chia.util.recursive_replace as recursive_replace
log = logging.getLogger(__name__)

def create_foliage(constants: ConsensusConstants, reward_block_unfinished: RewardChainBlockUnfinished, block_generator: Optional[BlockGenerator], aggregate_sig: G2Element, additions: List[Coin], removals: List[Coin], prev_block: Optional[BlockRecord], blocks: BlockchainInterface, total_iters_sp: uint128, timestamp: uint64, farmer_reward_puzzlehash: bytes32, pool_target: PoolTarget, get_plot_signature: Callable[([bytes32, G1Element], G2Element)], get_pool_signature: Callable[([PoolTarget, Optional[G1Element]], Optional[G2Element])], seed: bytes32=b'') -> Tuple[(Foliage, Optional[FoliageTransactionBlock], Optional[TransactionsInfo])]:
    """
    ...
    if prev_block is not None:
        ...
        is_transaction_block = res[0]
        prev_transaction_block = res[1]
    else:
        prev_transaction_block = None
        is_transaction_block = True
    random.seed(seed)
    extension_data = random.randint(0, 100000000).to_bytes(32, 'big')
    ...
    tx_additions = []
    tx_removals = []
    pool_target_signature = get_pool_signature(pool_target, reward_block_unfinished.proof_of_space.pool_public_key)
    foliage_data = FoliageBlockData(reward_block_unfinished.get_hash(), pool_target, pool_target_signature, farmer_reward_puzzlehash, extension_data)
    foliage_block_data_signature = get_plot_signature(foliage_data.get_hash(), reward_block_unfinished.proof_of_space.plot_public_key)
    prev_block_hash = constants.GENESIS_CHALLENGE
    if height != 0:
        assert prev_block is not None
        prev_block_hash = prev_block.header_hash
    generator_block_heights_list = []
    if is_transaction_block:
        cost = uint64(0)
        if block_generator is not None:
            generator_block_heights_list = block_generator.block_height_list()
            result = get_name_puzzle_conditions(block_generator,
              (constants.MAX_BLOCK_COST_CLVM), cost_per_byte=(constants.COST_PER_BYTE), safe_mode=True)
        ...
        if height > 0:
            ...
            pool_coin = create_pool_coin(curr.height, curr.pool_puzzle_hash, calculate_pool_reward(curr.height), constants.GENESIS_CHALLENGE)
            farmer_coin = create_farmer_coin(curr.height, curr.farmer_puzzle_hash, uint64(calculate_base_farmer_reward(curr.height) + curr.fees), constants.GENESIS_CHALLENGE)
            ...
            if curr.height > 0:
                ...
                while not curr.is_transaction_block:
                    pool_coin = create_pool_coin(curr.height, curr.pool_puzzle_hash, calculate_pool_reward(curr.height), constants.GENESIS_CHALLENGE)
                    farmer_coin = create_farmer_coin(curr.height, curr.farmer_puzzle_hash, calculate_base_farmer_reward(curr.height), constants.GENESIS_CHALLENGE)
                    reward_claims_incorporated += [pool_coin, farmer_coin]
                    curr = blocks.block_record(curr.prev_hash)
        ...
        for coin in tx_additions:
            if coin.puzzle_hash in puzzlehash_coin_map:
                ...
                puzzlehash_coin_map[coin.puzzle_hash] = [
                 coin]
        ...
        transactions_info = TransactionsInfo(generator_hash, generator_refs_hash, aggregate_sig, uint64(spend_bundle_fees), cost, reward_claims_incorporated)
        ...
        foliage_transaction_block = FoliageTransactionBlock(prev_transaction_block_hash, timestamp, filter_hash, additions_root, removals_root, transactions_info.get_hash())
        assert foliage_transaction_block is not None
        foliage_transaction_block_hash = foliage_transaction_block.get_hash()
        foliage_transaction_block_signature = get_plot_signature(foliage_transaction_block_hash, reward_block_unfinished.proof_of_space.plot_public_key)
        if not foliage_transaction_block_signature is not None:
...
def create_unfinished_block(constants: ConsensusConstants, sub_slot_start_total_iters: uint128, sub_slot_iters: uint64, signage_point_index: uint8, sp_iters: uint64, ip_iters: uint64, proof_of_space: ProofOfSpace, slot_cc_challenge: bytes32, farmer_reward_puzzle_hash: bytes32, pool_target: PoolTarget, get_plot_signature: Callable[([bytes32, G1Element], G2Element)], get_pool_signature: Callable[([PoolTarget, Optional[G1Element]], Optional[G2Element])], signage_point: SignagePoint, timestamp: uint64, blocks: BlockchainInterface, seed: bytes32=b'', block_generator: Optional[BlockGenerator]=None, aggregate_sig: G2Element=G2Element(), additions: Optional[List[Coin]]=None, removals: Optional[List[Coin]]=None, prev_block: Optional[BlockRecord]=None, finished_sub_slots_input: List[EndOfSubSlotBundle]=None) -> UnfinishedBlock:
    ...
    total_iters_sp = uint128(sub_slot_start_total_iters + sp_iters)
    is_genesis = prev_block is None
    new_sub_slot = len(finished_sub_slots) > 0
    cc_sp_hash = slot_cc_challenge
    ...
    cc_sp_signature = get_plot_signature(cc_sp_hash, proof_of_space.plot_public_key)
    ...
    rc_block = RewardChainBlockUnfinished(total_iters, signage_point_index, slot_cc_challenge, proof_of_space, signage_point.cc_vdf, cc_sp_signature, signage_point.rc_vdf, rc_sp_signature)
    if additions is None:
        additions = []
    if removals is None:
        removals = []
    foliage, foliage_transaction_block, transactions_info = create_foliage(constants, rc_block, block_generator, aggregate_sig, additions, removals, prev_block, blocks, total_iters_sp, timestamp, farmer_reward_puzzle_hash, pool_target, get_plot_signature, get_pool_signature, seed)
    return UnfinishedBlock(finished_sub_slots, rc_block, signage_point.cc_proof, signage_point.rc_proof, foliage, foliage_transaction_block, transactions_info, block_generator.program if block_generator else None, block_generator.block_height_list() if block_generator else [])


def unfinished_block_to_full_block(unfinished_block: UnfinishedBlock, cc_ip_vdf: VDFInfo, cc_ip_proof: VDFProof, rc_ip_vdf: VDFInfo, rc_ip_proof: VDFProof, icc_ip_vdf: Optional[VDFInfo], icc_ip_proof: Optional[VDFProof], finished_sub_slots: List[EndOfSubSlotBundle], prev_block: Optional[BlockRecord], blocks: BlockchainInterface, total_iters_sp: uint128, difficulty: uint64) -> FullBlock:
    """
