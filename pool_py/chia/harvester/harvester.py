...
# Embedded file name: chia\harvester\harvester.py
import asyncio, concurrent, logging
...
class Harvester:
    ...
    def __init__(self, root_path: Path, config: Dict, constants: ConsensusConstants):
        ...
        self.parallel_read = config.get('parallel_read', True)
    ...
    def get_plots(self) -> Tuple[(List[Dict], List[str], List[str])]:
        ...
        for path, plot_info in self.provers.items():
            ...
            response_plots.append({'filename':str(path), 
        ...
        self.log.debug(f"get_plots response: plots: {len(response_plots)}, failed_to_open_filenames: {len(self.failed_to_open_filenames)}, no_key_filenames: {len(self.no_key_filenames)}")
        return (
         response_plots, [str(s) for s, _ in self.failed_to_open_filenames.items()],
         [str(s) for s in self.no_key_filenames])

    async def refresh_plots(self):
        locked = self._refresh_lock.locked()
        changed = False
        if not locked:
            async with self._refresh_lock:
                changed, self.provers, self.failed_to_open_filenames, self.no_key_filenames = load_plots(self.provers, self.failed_to_open_filenames, self.farmer_public_keys, self.pool_public_keys, self.match_str, self.show_memo, self.root_path)
        if changed:
