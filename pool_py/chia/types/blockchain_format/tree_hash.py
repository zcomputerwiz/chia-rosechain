...
# Embedded file name: chia\types\blockchain_format\tree_hash.py
...
def sha256_treehash(sexp: CLVMObject, precalculated: Optional[Set[bytes32]]=None) -> bytes32:
    ...
    def handle_sexp(sexp_stack, op_stack, precalculated):
        sexp = sexp_stack.pop()
        if sexp.pair:
            p0, p1 = sexp.pair
            ...
            if sexp.atom in precalculated:
                ...
                r = std_hash(b'\x01' + sexp.atom)
            sexp_stack.append(r)

    def handle_pair(sexp_stack, op_stack, precalculated) -> None:
        p0 = sexp_stack.pop()
        p1 = sexp_stack.pop()
        sexp_stack.append(std_hash(b'\x02' + p0 + p1))
