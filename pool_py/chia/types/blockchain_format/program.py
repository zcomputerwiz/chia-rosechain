...
def run_program(program, args, max_cost, operator_lookup=OPERATOR_LOOKUP, pre_eval_f=None):
    return default_run_program(program,
      args,
      operator_lookup,
      max_cost,
...
class SerializedProgram:
    ...
    def _run(self, max_cost: int, flags, *args) -> Tuple[(int, Program)]:
        ...
        native_opcode_names_by_opcode = dict((('op_%s' % OP_REWRITE.get(k, k), op) for op, k in KEYWORD_FROM_ATOM.items() if k not in 'qa.'))
        cost, ret = deserialize_and_run_program2(self._buf, serialized_args, KEYWORD_TO_ATOM['q'][0], KEYWORD_TO_ATOM['a'][0], native_opcode_names_by_opcode, max_cost, flags)
        return (
         cost, Program.to(ret))
