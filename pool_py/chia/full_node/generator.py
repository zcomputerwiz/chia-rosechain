...
# Embedded file name: chia\full_node\generator.py
import logging
...
DECOMPRESS_CSE_WITH_PREFIX = load_clvm('decompress_coin_spend_entry_with_prefix.clvm',
  package_or_requirement='chia.wallet.puzzles')
log = logging.getLogger(__name__)

def create_block_generator(generator: SerializedProgram, block_heights_list: List[uint32], generator_block_cache: GeneratorBlockCacheInterface) -> Optional[BlockGenerator]:
...
def create_compressed_generator(original_generator: CompressorArg, compressed_cse_list: List[List[Union[(List[uint64], List[Union[(bytes, None, Program)]])]]]) -> BlockGenerator:
    """
    ...
    program = DECOMPRESS_BLOCK.curry(DECOMPRESS_PUZZLE, DECOMPRESS_CSE_WITH_PREFIX, Program.to(start), Program.to(end), compressed_cse_list)
