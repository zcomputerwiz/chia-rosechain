...
# Embedded file name: chia\full_node\block_store.py
import logging
...
class BlockStore:
    ...
    async def create(cls, db_wrapper: DBWrapper):
        ...
        await self.db.execute('CREATE TABLE IF NOT EXISTS full_blocks(header_hash text PRIMARY KEY, height bigint,  is_block tinyint, is_fully_compactified tinyint, block blob)')
        await self.db.execute('CREATE TABLE IF NOT EXISTS block_records(header_hash text PRIMARY KEY, prev_hash text, height bigint,block blob, sub_epoch_summary blob, is_peak tinyint, is_block tinyint)')
        await self.db.execute('DROP TABLE IF EXISTS sub_epoch_segments_v2')
        await self.db.execute('CREATE TABLE IF NOT EXISTS sub_epoch_segments_v3(ses_block_hash text PRIMARY KEY, challenge_segments blob)')
    ...
    async def add_full_block(self, header_hash, block, block_record):
        ...
        cursor_2 = await self.db.execute('INSERT OR REPLACE INTO block_records VALUES(?, ?, ?, ?,?, ?, ?)', (
         header_hash.hex(),
    ...
    async def persist_sub_epoch_challenge_segments(self, ses_block_hash: bytes32, segments: List[SubEpochChallengeSegment]) -> None:
        async with self.db_wrapper.lock:
            cursor_1 = await self.db.execute('INSERT OR REPLACE INTO sub_epoch_segments_v3 VALUES(?, ?)', (
    ...
    async def get_sub_epoch_challenge_segments(self, ses_block_hash: bytes32) -> Optional[List[SubEpochChallengeSegment]]:
        ...
        if cached is not None:
            return cached
        cursor = await self.db.execute('SELECT challenge_segments from sub_epoch_segments_v3 WHERE ses_block_hash=?', (ses_block_hash.hex(),))
    ...
    async def get_block_records_by_hash(self, header_hashes: List[bytes32]):
        ...
        all_blocks = {}
        for row in rows:
            block_rec = BlockRecord.from_bytes(row[0])
    ...
    async def get_blocks_by_hash(self, header_hashes: List[bytes32]) -> List[FullBlock]:
        ...
        if len(header_hashes) == 0:
            return []
        header_hashes_db = tuple([hh.hex() for hh in header_hashes])
        formatted_str = f"SELECT header_hash, block from full_blocks WHERE header_hash in ({'?,' * (len(header_hashes_db) - 1)}?)"
        cursor = await self.db.execute(formatted_str, header_hashes_db)
        rows = await cursor.fetchall()
        await cursor.close()
        all_blocks = {}
        for row in rows:
            ...
            full_block = FullBlock.from_bytes(row[1])
    ...
    async def get_block_record(self, header_hash: bytes32) -> Optional[BlockRecord]:
        cursor = await self.db.execute('SELECT block from block_records WHERE header_hash=?', (
         header_hash.hex(),))
        row = await cursor.fetchone()
        await cursor.close()
        if row is not None:
    ...
    async def get_peak_height_dicts(self) -> Tuple[(Dict[(uint32, bytes32)], Dict[(uint32, SubEpochSummary)])]:
        ...
        return (
         height_to_hash, sub_epoch_summaries)

    async def set_peak(self, header_hash: bytes32) -> None:
        cursor_1 = await self.db.execute('UPDATE block_records SET is_peak=0 WHERE is_peak=1')
        await cursor_1.close()
        cursor_2 = await self.db.execute('UPDATE block_records SET is_peak=1 WHERE header_hash=?', (
         header_hash.hex(),))
        await cursor_2.close()

    async def is_fully_compactified(self, header_hash: bytes32) -> Optional[bool]:
        cursor = await self.db.execute('SELECT is_fully_compactified from full_blocks WHERE header_hash=?', (header_hash.hex(),))
        row = await cursor.fetchone()
        await cursor.close()
        if row is None:
            return
    ...
    async def get_first_not_compactified(self) -> Optional[int]:
        cursor = await self.db.execute('SELECT height FROM full_blocks GROUP BY height HAVING sum(is_fully_compactified)=0 ORDER BY height LIMIT 1')
        row = await cursor.fetchone()
        await cursor.close()
        if row is None:
            return
