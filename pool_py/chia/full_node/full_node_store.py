...
# Embedded file name: chia\full_node\full_node_store.py
...
class FullNodeStore:
    ...
    tx_fetch_tasks: Dict[(bytes32, asyncio.Task)]
    ...
    def add_candidate_block(self, quality_string, height, unfinished_block, backup=False):
        if backup:
            self.candidate_backup_blocks[quality_string] = (
             height, unfinished_block)
        else:
            self.candidate_blocks[quality_string] = (
             height, unfinished_block)

    def get_candidate_block(self, quality_string: bytes32, backup: bool=False) -> Optional[Tuple[(uint32, UnfinishedBlock)]]:
        if backup:
    ...
    def add_unfinished_block(self, height, unfinished_block, result):
    ...
    def add_to_future_ip(self, infusion_point: timelord_protocol.NewInfusionPointVDF):
        ch = infusion_point.reward_chain_ip_vdf.challenge
    ...
    def in_future_sp_cache(self, signage_point: SignagePoint, index: uint8) -> bool:
        ...
        for cache_index, cache_sp in self.future_sp_cache[signage_point.rc_vdf.challenge]:
            if cache_index == index:
                if cache_sp.rc_vdf == signage_point.rc_vdf:
                    return True

        return False

    def add_to_future_sp(self, signage_point: SignagePoint, index: uint8):
        if signage_point.cc_vdf is None or signage_point.rc_vdf is None or signage_point.cc_proof is None or signage_point.rc_proof is None:
            return
        if signage_point.rc_vdf.challenge not in self.future_sp_cache:
        ...
        for index, (sub_slot, _, total_iters) in enumerate(self.finished_sub_slots):
            if sub_slot is not None:
                if sub_slot.challenge_chain.get_hash() == challenge_hash:
                    return (
                     sub_slot, index, total_iters)

    def initialize_genesis_sub_slot(self):
        self.clear_slots()
        self.finished_sub_slots = [(None, [None] * self.constants.NUM_SPS_SUB_SLOT, uint128(0))]

    def new_finished_sub_slot(self, eos: EndOfSubSlotBundle, blocks: BlockchainInterface, peak: Optional[BlockRecord], peak_full_block: Optional[FullBlock]) -> Optional[List[timelord_protocol.NewInfusionPointVDF]]:
        """
        ...
        cc_challenge = last_slot.challenge_chain.get_hash() if last_slot is not None else self.constants.GENESIS_CHALLENGE
        rc_challenge = last_slot.reward_chain.get_hash() if last_slot is not None else self.constants.GENESIS_CHALLENGE
        icc_challenge = None
        icc_iters = None
        ...
        if peak is not None and peak.total_iters > last_slot_iters:
            ...
            if peak.deficit == self.constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK:
                ...
                if peak.deficit == self.constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK - 1:
            ...
            if peak.deficit < self.constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK:
                ...
                while not curr.first_in_sub_slot:
                    if not curr.is_challenge_block(self.constants):
                ...
                if can_finish_sub_and_full_epoch(self.constants, blocks, peak.height, peak.prev_hash, peak.deficit, peak.sub_epoch_summary_included is not None)[0]:
                    assert peak_full_block is not None
                    ses = next_sub_epoch_summary(self.constants, blocks, peak.required_iters, peak_full_block, True)
                    if ses is not None:
            ...
            icc_iters = sub_slot_iters
            icc_challenge = last_slot.infused_challenge_chain.get_hash() if last_slot is not None and last_slot.infused_challenge_chain is not None and (last_slot.reward_chain.deficit != self.constants.MIN_BLOCKS_PER_CHALLENGE_BLOCK) else None
        partial_cc_vdf_info = VDFInfo(cc_challenge, iters, eos.challenge_chain.challenge_chain_end_of_slot_vdf.output)
        if eos.challenge_chain.challenge_chain_end_of_slot_vdf != dataclasses.replace(partial_cc_vdf_info,
          number_of_iterations=sub_slot_iters):
            return
        if not eos.proofs.challenge_chain_slot_proof.normalized_to_identity:
            if not eos.proofs.challenge_chain_slot_proof.is_valid(self.constants, cc_start_element, partial_cc_vdf_info):
                return
        if eos.proofs.challenge_chain_slot_proof.normalized_to_identity:
            if not eos.proofs.challenge_chain_slot_proof.is_valid(self.constants, ClassgroupElement.get_default_element(), eos.challenge_chain.challenge_chain_end_of_slot_vdf):
                return
            if not eos.proofs.reward_chain_slot_proof.is_valid(self.constants, ClassgroupElement.get_default_element(), eos.reward_chain.end_of_slot_vdf, VDFInfo(rc_challenge, iters, eos.reward_chain.end_of_slot_vdf.output)):
                return
        if icc_challenge is not None:
            ...
            partial_icc_vdf_info = VDFInfo(icc_challenge, iters, eos.infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf.output)
            if eos.infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf != dataclasses.replace(partial_icc_vdf_info,
              number_of_iterations=icc_iters):
                return
            if not eos.proofs.infused_challenge_chain_slot_proof.normalized_to_identity:
                if not eos.proofs.infused_challenge_chain_slot_proof.is_valid(self.constants, icc_start_element, partial_icc_vdf_info):
                    return
            if eos.proofs.infused_challenge_chain_slot_proof.normalized_to_identity and not eos.proofs.infused_challenge_chain_slot_proof.is_valid(self.constants, ClassgroupElement.get_default_element(), eos.infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf):
                return
            else:
    ...
    def new_signage_point(self, index: uint8, blocks: BlockchainInterface, peak: Optional[BlockRecord], next_sub_slot_iters: uint64, signage_point: SignagePoint, skip_vdf_validation=False) -> bool:
        """
        ...
        if signage_point.cc_vdf is not None:
            if signage_point.cc_proof is not None:
                if not (signage_point.rc_vdf is not None and signage_point.rc_proof is not None):
                ...
                for sub_slot, sp_arr, start_ss_total_iters in self.finished_sub_slots:
                    ...
                    if ss_challenge_hash == signage_point.cc_vdf.challenge:
                        if peak is not None and start_ss_total_iters > peak.total_iters:
                            checkpoint_size = uint64(next_sub_slot_iters // self.constants.NUM_SPS_SUB_SLOT)
                        ...
                        if peak is None or future_sub_slot:
                            ...
                            while curr is not None:
                                if curr.total_iters > start_ss_total_iters:
                        ...
                        if check_from_start_of_ss:
                            ...
                            cc_vdf_info_expected = VDFInfo(ss_challenge_hash, uint64(sp_total_iters - curr.total_iters), signage_point.cc_vdf.output)
                            rc_vdf_info_expected = VDFInfo(curr.reward_infusion_new_challenge, uint64(sp_total_iters - curr.total_iters), signage_point.rc_vdf.output)
                        if not signage_point.cc_vdf == dataclasses.replace(cc_vdf_info_expected,
                            ...
                            if not skip_vdf_validation:
                                if not signage_point.cc_proof.normalized_to_identity:
                                    if not signage_point.cc_proof.is_valid(self.constants, start_ele, cc_vdf_info_expected):
                                        self.add_to_future_sp(signage_point, index)
                                        return False
                                if signage_point.cc_proof.normalized_to_identity:
                                    if not signage_point.cc_proof.is_valid(self.constants, ClassgroupElement.get_default_element(), signage_point.cc_vdf):
                                        self.add_to_future_sp(signage_point, index)
                                        return False
                                    if rc_vdf_info_expected.challenge != signage_point.rc_vdf.challenge:
                                        ...
                                        return False
                                    if not skip_vdf_validation:
                                        if not signage_point.rc_proof.is_valid(self.constants, ClassgroupElement.get_default_element(), signage_point.rc_vdf, rc_vdf_info_expected):
                                            self.add_to_future_sp(signage_point, index)
                                            return False
    ...
    def get_signage_point_by_index(self, challenge_hash: bytes32, index: uint8, last_rc_infusion: bytes32) -> Optional[SignagePoint]:
        assert len(self.finished_sub_slots) >= 1
        for sub_slot, sps, _ in self.finished_sub_slots:
            if sub_slot is not None:
    ...
    def have_newer_signage_point(self, challenge_hash, index, last_rc_infusion):
        """
    ...
    def new_peak(self, peak: BlockRecord, peak_full_block: FullBlock, sp_sub_slot: Optional[EndOfSubSlotBundle], ip_sub_slot: Optional[EndOfSubSlotBundle], fork_block: Optional[BlockRecord], blocks: BlockchainInterface) -> Tuple[(
 Optional[EndOfSubSlotBundle], List[Tuple[(uint8, SignagePoint)]], List[timelord_protocol.NewInfusionPointVDF])]:
        ...
        if ip_sub_slot is None:
            ...
            ip_sub_slot_sps = [None] * self.constants.NUM_SPS_SUB_SLOT
                    ...
                    if sub_slot is None:
                        ...
                        for i, sp in enumerate(sps):
                            if total_iters + i * interval_iters < fork_block.total_iters:
                                ...
                                if sp is not None:
                                    log.debug(f"Reverting {i} {total_iters + i * interval_iters} {fork_block.total_iters}")
            ...
            if sp_sub_slot is not None or prev_sub_slot_total_iters == 0:
                if not peak.overflow:
                    assert prev_sub_slot_total_iters
        ...
        new_sps = []
        new_ips = []
        future_eos = self.future_eos_cache.get(peak.reward_infusion_new_challenge, []).copy()
        for eos in future_eos:
            if self.new_finished_sub_slot(eos, blocks, peak, peak_full_block) is not None:
                new_eos = eos
                break

        future_sps = self.future_sp_cache.get(peak.reward_infusion_new_challenge, []).copy()
    ...
    def get_finished_sub_slots(self, block_records: BlockchainInterface, prev_b: Optional[BlockRecord], last_challenge_to_add: bytes32) -> Optional[List[EndOfSubSlotBundle]]:
        """
        ...
        if prev_b is None:
            ...
            challenge_in_chain = self.constants.GENESIS_CHALLENGE
        else:
            curr = prev_b
            while not curr.first_in_sub_slot:
        ...
        for sub_slot, sps, total_iters in self.finished_sub_slots[1:]:
            ...
            if found_connecting_challenge:
                if sub_slot.challenge_chain.get_hash() == last_challenge_to_add:
