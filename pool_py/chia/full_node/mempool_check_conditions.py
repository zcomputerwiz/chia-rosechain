...
# Embedded file name: chia\full_node\mempool_check_conditions.py
import logging, time
...
def mempool_assert_absolute_block_height_exceeds(condition: ConditionWithArgs, prev_transaction_block_height: uint32) -> Optional[Err]:
    """
...
def mempool_assert_relative_block_height_exceeds(condition: ConditionWithArgs, unspent: CoinRecord, prev_transaction_block_height: uint32) -> Optional[Err]:
    """
...
def mempool_assert_relative_time_exceeds(condition: ConditionWithArgs, unspent: CoinRecord, timestamp: uint64) -> Optional[Err]:
    """
    Check if the current time in seconds exceeds the time specified by condition
    """
    try:
...
def parse_create_coin(args: SExp, safe_mode: bool) -> List[bytes]:
    ...
    if amount_int >= 18446744073709551616:
...
def parse_seconds(args: SExp, safe_mode: bool, error_code: Err) -> Optional[List[bytes]]:
    ...
    if seconds_int <= 0:
        return
    if seconds_int >= 18446744073709551616:
        raise ValidationError(error_code)
    return [
     int_to_bytes(seconds_int)]
...
def parse_fee(args: SExp, safe_mode: bool) -> List[bytes]:
    ...
    if fee_int >= 18446744073709551616 or fee_int < 0:
...
def parse_condition(cond: SExp, safe_mode: bool) -> Tuple[(int, Optional[ConditionWithArgs])]:
    ...
    if condition in CONDITION_OPCODES:
        opcode = ConditionOpcode(condition)
...
def get_name_puzzle_conditions(generator, max_cost, *, cost_per_byte, safe_mode):
    """
    ...
    try:
        ...
        for res in result.first().as_iter():
            ...
            if len(res.first().atom) != 32:
                ...
                spent_coin_parent_id = res.first().as_atom()
                res = res.rest()
                if len(res.first().atom) != 32:
                    raise ValidationError(Err.INVALID_CONDITION)
                spent_coin_puzzle_hash = res.first().as_atom()
                res = res.rest()
                spent_coin_amount = uint64(sanitize_int(res.first(), safe_mode))
                res = res.rest()
                spent_coin = Coin(spent_coin_parent_id, spent_coin_puzzle_hash, spent_coin_amount)
...
def get_puzzle_and_solution_for_coin(generator: BlockGenerator, coin_name: bytes, max_cost: int):
    try:
        ...
        cost, result = GENERATOR_FOR_SINGLE_COIN_MOD.run_with_cost(max_cost, block_program, block_program_args, coin_name)
        puzzle = result.first()
        solution = result.rest().first()
        return (
         None, puzzle, solution)
...
def mempool_check_conditions_dict(unspent: CoinRecord, coin_announcement_names: Set[bytes32], puzzle_announcement_names: Set[bytes32], conditions_dict: Dict[(ConditionOpcode, List[ConditionWithArgs])], prev_transaction_block_height: uint32, timestamp: uint64) -> Optional[Err]:
    """
    ...
    for con_list in conditions_dict.values():
        for cvp in con_list:
            ...
            if cvp.opcode is ConditionOpcode.ASSERT_MY_COIN_ID:
                ...
                if cvp.opcode is ConditionOpcode.ASSERT_COIN_ANNOUNCEMENT:
                    error = mempool_assert_announcement(cvp, coin_announcement_names)
                else:
                    if cvp.opcode is ConditionOpcode.ASSERT_PUZZLE_ANNOUNCEMENT:
                        ...
                        if cvp.opcode is ConditionOpcode.ASSERT_HEIGHT_ABSOLUTE:
                            ...
                            if cvp.opcode is ConditionOpcode.ASSERT_HEIGHT_RELATIVE:
                                error = mempool_assert_relative_block_height_exceeds(cvp, unspent, prev_transaction_block_height)
                            else:
                                if cvp.opcode is ConditionOpcode.ASSERT_SECONDS_ABSOLUTE:
                                    error = mempool_assert_absolute_time_exceeds(cvp, timestamp)
                                else:
                                    if cvp.opcode is ConditionOpcode.ASSERT_SECONDS_RELATIVE:
                                        error = mempool_assert_relative_time_exceeds(cvp, unspent, timestamp)
                                    else:
                                        if cvp.opcode is ConditionOpcode.ASSERT_MY_PARENT_ID:
                                            error = mempool_assert_my_parent_id(cvp, unspent)
                                        else:
                                            if cvp.opcode is ConditionOpcode.ASSERT_MY_PUZZLEHASH:
                                                error = mempool_assert_my_puzzlehash(cvp, unspent)
                                            else:
                                                if cvp.opcode is ConditionOpcode.ASSERT_MY_AMOUNT:
