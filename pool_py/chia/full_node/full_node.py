...
# Embedded file name: chia\full_node\full_node.py
import asyncio, dataclasses, logging, random, time, traceback
...
from chia.protocols.full_node_protocol import RejectBlocks, RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
...
class FullNode:
    ...
    _init_weight_proof = None
    _init_weight_proof: Optional[asyncio.Task]
    ...
    def __init__(self, config, root_path, consensus_constants, name=None):
    ...
    async def _start(self):
        ...
        if self.blockchain.get_peak() is None:
            ...
            self.log.info(f"Blockchain initialized to peak {self.blockchain.get_peak().header_hash} height {self.blockchain.get_peak().height}, time taken: {int(time_taken)}s")
        ...
        if self.config['send_uncompact_interval'] != 0:
            ...
            self.uncompact_task = asyncio.create_task(self.broadcast_uncompact_blocks(self.config['send_uncompact_interval'], self.config['target_uncompact_proofs'], sanitize_weight_proof_only))
    ...
    def set_server(self, server: ChiaServer):
        ...
        if 'dns_servers' in self.config:
            ...
            if self.config['port'] == 8520:
                dns_servers.append('dns-introducer.rosechain.net')
        try:
            self.full_node_peers = FullNodePeers(self.server, self.root_path, self.config['target_peer_count'] - self.config['target_outbound_peer_count'], self.config['target_outbound_peer_count'], self.config['peer_db_path'], self.config['introducer_peer'], dns_servers, self.config['peer_connect_interval'], self.config['selected_network'], default_port, self.log)
    ...
    async def short_sync_batch(self, peer: ws.WSChiaConnection, start_height: uint32, target_height: uint32) -> bool:
        ...
        """
        if peer.peer_node_id in self.sync_store.backtrack_syncing:
            if self.sync_store.backtrack_syncing[peer.peer_node_id] > 0:
                return True
        ...
        if start_height > 0:
            ...
            if not (first is None or isinstance(first, full_node_protocol.RespondBlock)):
                    ...
                    if not response:
                        ...
                        async with self.blockchain.lock:
                            ...
                            if advanced_peak:
                                ...
                                if peak is not None:
                                    if not (peak_fb is not None and fork_height is not None):
    ...
    async def short_sync_backtrack(self, peer: ws.WSChiaConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32):
        """
        ...
        try:
            ...
            while curr_height > peak_height - 5:
                fetch_tx = unfinished_block is None or curr_height != target_height
                ...
                if not (curr is None or isinstance(curr, full_node_protocol.RespondBlock)):
                    raise ValueError(f"Failed to fetch block {curr_height} from {peer.get_peer_info()}, wrong type {type(curr)}")
...
, self._ui_tasks))
        ...
        if peak is not None:
            if peak.weight > request.weight:
                return
        if self.sync_store.get_sync_mode():
            peak_sync_hash = self.sync_store.get_sync_target_hash()
            peak_sync_height = self.sync_store.get_sync_target_height()
            if peak_sync_hash is not None:
                if not request.header_hash != peak_sync_hash or peak_sync_height is not None:
                    peak_peers = self.sync_store.get_peers_that_have_peak([peak_sync_hash])
                    if peer.peer_node_id not in peak_peers:
                        target_peak_response = await peer.request_block((full_node_protocol.RequestBlock(uint32(peak_sync_height), False)),
                          timeout=10)
                        if not target_peak_response is not None or isinstance(target_peak_response, RespondBlock):
                            self.sync_store.peer_has_block(peak_sync_hash, peer.peer_node_id, target_peak_response.block.weight, peak_sync_height, False)
        else:
            if request.height <= curr_peak_height + self.config['short_sync_blocks_behind_threshold']:
                if await self.short_sync_backtrack(peer, curr_peak_height, request.height, request.unfinished_reward_block_hash):
                    return
    ...
    async def send_peak_to_timelords(self, peak_block: Optional[FullBlock]=None, peer: Optional[ws.WSChiaConnection]=None):
        """
        ...
        if peak_block is not None:
            ...
            ses = next_sub_epoch_summary(self.constants, self.blockchain, peak.required_iters, peak_block, True)
            ...
            if curr.sub_epoch_summary_included or curr.height == 0:
                passed_ses_height_but_not_yet_included = False
            timelord_new_peak = timelord_protocol.NewPeakTimelord(peak_block.reward_chain_block, difficulty, peak.deficit, peak.sub_slot_iters, ses, recent_rc, last_csb_or_eos, passed_ses_height_but_not_yet_included)
    ...
    async def synced(self) -> bool:
        ...
        now = time.time()
        if not curr is None:
            if curr.timestamp is None or curr.timestamp < uint64(int(now - 420)) or self.sync_store.get_sync_mode():
                return False
            return True
        ...
        if connection.connection_type is NodeType.FULL_NODE:
            ...
            if synced:
                if peak_height is not None:
                    if current_time > self.constants.INITIAL_FREEZE_END_TIMESTAMP:
                        my_filter = self.mempool_manager.get_filter()
                        mempool_request = full_node_protocol.RequestMempoolTransactions(my_filter)
                        msg = make_msg(ProtocolMessageTypes.request_mempool_transactions, mempool_request)
                        await connection.send_message(msg)
        peak_full = await self.blockchain.get_full_peak()
        if peak_full is not None:
            peak = self.blockchain.block_record(peak_full.header_hash)
            if connection.connection_type is NodeType.FULL_NODE:
                request_node = full_node_protocol.NewPeak(peak.header_hash, peak.height, peak.weight, peak.height, peak_full.reward_chain_block.get_unfinished().get_hash())
                await connection.send_message(make_msg(ProtocolMessageTypes.new_peak, request_node))
            else:
                if connection.connection_type is NodeType.WALLET:
                    ...
                    if connection.connection_type is NodeType.TIMELORD:
            ...
            try:
                ...
                for peer in self.server.all_connections.values():
                    if peer.connection_type == NodeType.FULL_NODE:
                        ...
                        coroutines.append(peer.request_block((full_node_protocol.RequestBlock(uint32(heaviest_peak_height), True)),
                          timeout=10))

                for i, target_peak_response in enumerate(await (asyncio.gather)(*coroutines)):
                    if target_peak_response is not None:
                        if isinstance(target_peak_response, RespondBlock):
                            self.sync_store.peer_has_block(heaviest_peak_hash, peers[i], heaviest_peak_weight, heaviest_peak_height, False)

                peer_ids = self.sync_store.get_peers_that_have_peak([heaviest_peak_hash])
                ...
                self.log.info(f"Requesting weight proof from peer {weight_proof_peer.peer_host} up to height {heaviest_peak_height}")
                if not self.blockchain.get_peak() is not None or heaviest_peak_weight <= self.blockchain.get_peak().weight:
                ...
                if not (response is None or isinstance(response, full_node_protocol.RespondProofOfWeight)):
                ...
                if not current_peak is not None or response.wp.recent_chain_data[-1].reward_chain_block.weight <= current_peak.weight:
    ...
    async def sync_from_fork_point(self, fork_point_height: int, target_peak_sb_height: uint32, peak_hash: bytes32, summaries: List[SubEpochSummary]):
        ...
        if len(ses_heigths) > 2:
            if our_peak_height is not None:
                ...
                if self.blockchain.get_peak_height() is not None:
                    if fork_point_height == max_fork_ses_height:
                        for peer in peers_with_peak:
                            block_response = await peer.request_block(full_node_protocol.RequestBlock(uint32(our_peak_height + 1), True))
                            if block_response is not None:
                                if isinstance(block_response, full_node_protocol.RespondBlock):
                                    peak = self.blockchain.get_peak()
                                    if peak is not None:
                                        if block_response.block.prev_header_hash == peak.header_hash:
        ...
        for i in range(fork_point_height, target_peak_sb_height, batch_size):
            ...
            for peer in peers_with_peak:
                ...
                if isinstance(response, RespondBlocks):
                    success, advanced_peak, _ = await self.receive_block_batch(response.blocks, peer, None if advanced_peak else uint32(fork_point_height), summaries)
                    if success is False:
            ...
            msg = make_msg(ProtocolMessageTypes.new_peak_wallet, wallet_protocol.NewPeakWallet(peak.header_hash, peak.height, peak.weight, uint32(max(peak.height - 1, uint32(0)))))
            ...
            if batch_added is False:
                ...
                self.blockchain.clean_block_record(min(end_height - self.constants.BLOCKS_CACHE_SIZE, peak.height - self.constants.BLOCKS_CACHE_SIZE))

    async def receive_block_batch(self, all_blocks: List[FullBlock], peer: ws.WSChiaConnection, fork_point: Optional[uint32], wp_summaries: Optional[List[SubEpochSummary]]=None) -> Tuple[(bool, bool, Optional[uint32])]:
        advanced_peak = False
        ...
        pre_validation_results = await self.blockchain.pre_validate_blocks_multiprocessing(blocks_to_validate, {}, wp_summaries=wp_summaries)
        ...
        for i, block in enumerate(blocks_to_validate):
            if pre_validation_results[i].error is not None:
                self.log.error(f"Invalid block from peer: {peer.get_peer_info()} {Err(pre_validation_results[i].error)}")
                return (
                 False, advanced_peak, fork_height)

        for i, block in enumerate(blocks_to_validate):
            if not pre_validation_results[i].required_iters is not None:
                raise AssertionError
            else:
                result, error, fork_height = await self.blockchain.receive_block(block, pre_validation_results[i], None if advanced_peak else fork_point)
        ...
        if advanced_peak:
            ...
            self.log.debug(f"Total time for {len(blocks_to_validate)} blocks: {time.time() - pre_validate_start}, advanced: {advanced_peak}")
        return (
         True, advanced_peak, fork_height)
    ...
    def has_valid_pool_sig(self, block: Union[(UnfinishedBlock, FullBlock)]):
        if not block.foliage.foliage_block_data.pool_target == PoolTarget(self.constants.GENESIS_PRE_FARM_POOL_PUZZLE_HASH, uint32(0)) and block.foliage.prev_block_hash != self.constants.GENESIS_CHALLENGE or block.reward_chain_block.proof_of_space.pool_public_key is not None:
            if not AugSchemeMPL.verify(block.reward_chain_block.proof_of_space.pool_public_key, bytes(block.foliage.foliage_block_data.pool_target), block.foliage.foliage_block_data.pool_signature):
                return False
            return True

    async def signage_point_post_processing(self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSChiaConnection, ip_sub_slot: Optional[EndOfSubSlotBundle]):
        self.log.info(f"⏲️  Finished signage point {request.index_from_challenge}/{self.constants.NUM_SPS_SUB_SLOT}: CC: {request.challenge_chain_vdf.output.get_hash()} RC: {request.reward_chain_vdf.output.get_hash()} ")
        ...
        if sub_slot_tuple is not None:
            ...
            prev_challenge = None
        broadcast = full_node_protocol.NewSignagePointOrEndOfSubSlot(prev_challenge, request.challenge_chain_vdf.challenge, request.index_from_challenge, request.reward_chain_vdf.challenge)
        ...
        broadcast_farmer = farmer_protocol.NewSignagePoint(request.challenge_chain_vdf.challenge, request.challenge_chain_vdf.output.get_hash(), request.reward_chain_vdf.output.get_hash(), difficulty, sub_slot_iters, request.index_from_challenge)
        msg = make_msg(ProtocolMessageTypes.new_signage_point, broadcast_farmer)
        await self.server.send_to_all([msg], NodeType.FARMER)

    async def peak_post_processing(self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSChiaConnection]):
        """
        ...
        self.log.info(f"🌱 Updated peak to height {record.height}, weight {record.weight}, hh {record.header_hash}, forked at {fork_height}, rh: {record.reward_infusion_new_challenge}, total iters: {record.total_iters}, overflow: {record.overflow}, deficit: {record.deficit}, difficulty: {difficulty}, sub slot iters: {sub_slot_iters}, Generator size: {len(bytes(block.transactions_generator)) if block.transactions_generator else 'No tx'}, Generator ref list size: {len(block.transactions_generator_ref_list) if block.transactions_generator else 'No tx'}")
        ...
        fork_block = None
        if fork_height != block.height - 1:
            if block.height != 0:
                fork_block = self.blockchain.block_record(self.blockchain.height_to_hash(fork_height))
        added_eos, new_sps, new_ips = self.full_node_store.new_peak(record, block, sub_slots[0], sub_slots[1], fork_block, self.blockchain)
        if sub_slots[1] is None:
            assert record.ip_sub_slot_total_iters(self.constants) == 0
            self.full_node_store.new_signage_point((record.signage_point_index),
              (self.blockchain),
              record,
              (record.sub_slot_iters),
              (SignagePoint(block.reward_chain_block.challenge_chain_sp_vdf, block.challenge_chain_sp_proof, block.reward_chain_block.reward_chain_sp_vdf, block.reward_chain_sp_proof)),
            ...
            for bundle, result, spend_name in await self.mempool_manager.new_peak(self.blockchain.get_peak()):
                ...
                if not mempool_item is not None:
                    raise AssertionError
                else:
            ...
            if added_eos is not None:
                broadcast = full_node_protocol.NewSignagePointOrEndOfSubSlot(added_eos.challenge_chain.challenge_chain_end_of_slot_vdf.challenge, added_eos.challenge_chain.get_hash(), uint8(0), added_eos.reward_chain.end_of_slot_vdf.challenge)
                msg = make_msg(ProtocolMessageTypes.new_signage_point_or_end_of_sub_slot, broadcast)
                await self.server.send_to_all([msg], NodeType.FULL_NODE)
            if new_sps is not None:
                if peer is not None:
                    for index, sp in new_sps:
                        if sp.cc_vdf is not None:
                            if sp.cc_proof is not None:
                                if not (sp.rc_vdf is not None and sp.rc_proof is not None):
                                    raise AssertionError
                                else:
                                    await self.signage_point_post_processing(RespondSignagePoint(index, sp.cc_vdf, sp.cc_proof, sp.rc_vdf, sp.rc_proof), peer, sub_slots[1])
            ...
            if self.sync_store.get_sync_mode() is False:
                ...
                msg = make_msg(ProtocolMessageTypes.new_peak, full_node_protocol.NewPeak(record.header_hash, record.height, record.weight, fork_height, block.reward_chain_block.get_unfinished().get_hash()))
                if peer is not None:
                    await self.server.send_to_all_except([msg], NodeType.FULL_NODE, peer.peer_node_id)
                else:
                    await self.server.send_to_all([msg], NodeType.FULL_NODE)
            msg = make_msg(ProtocolMessageTypes.new_peak_wallet, wallet_protocol.NewPeakWallet(record.header_hash, record.height, record.weight, fork_height))
            await self.server.send_to_all([msg], NodeType.WALLET)
            if block.transactions_generator is not None:
                if self.full_node_store.previous_generator is None:
    ...
    async def respond_block(self, respond_block: full_node_protocol.RespondBlock, peer: Optional[ws.WSChiaConnection]=None) -> Optional[Message]:
        """
        Receive a full block from a peer full node (or ourselves).
        """
        block = respond_block.block
        if self.sync_store.get_sync_mode():
            return
            ...
            if block.transactions_info is not None:
                if block.transactions_info.generator_root != bytes([0] * 32):
                    if block.transactions_generator is None:
                        unfinished_rh = block.reward_chain_block.get_unfinished().get_hash()
                        unf_block = self.full_node_store.get_unfinished_block(unfinished_rh)
                        if unf_block is not None and unf_block.transactions_generator is not None and unf_block.foliage_transaction_block == block.foliage_transaction_block:
                            ...
                            if peer is None:
                                return
                            block_response = await peer.request_block(full_node_protocol.RequestBlock(block.height, True))
                            if not (block_response is None or isinstance(block_response, full_node_protocol.RespondBlock)):
                                self.log.warning(f"Was not able to fetch the correct block for height {block.height} {block_response}")
                                return
                            new_block = block_response.block
                            ...
                            self.log.debug(f"Wrong info in the cache for bh {new_block.header_hash}, there might be multiple blocks from the same farmer with the same pospace.")
        ...
        async with self.blockchain.lock:
            ...
            if pre_validation_result is not None:
                if pre_validation_result.npc_result is not None:
            ...
            if pre_validation_results[0].error is not None:
                if Err(pre_validation_results[0].error) == Err.INVALID_PREV_BLOCK_HASH:
                    ...
                    error_code = Err.INVALID_PREV_BLOCK_HASH
                    fork_height = None
                else:
                    raise ValueError(f"Failed to validate block {header_hash} height {block.height}: {Err(pre_validation_results[0].error).name}")
            else:
                ...
                if self.full_node_store.previous_generator is not None:
                    if fork_height is not None:
                        if fork_height < self.full_node_store.previous_generator.block_height:
            ...
            if added == ReceiveBlockResult.INVALID_BLOCK:
                ...
                if added == ReceiveBlockResult.DISCONNECTED_BLOCK:
                ...
                if added == ReceiveBlockResult.NEW_PEAK:
                    ...
                    if not (new_peak is not None and fork_height is not None):
                        raise AssertionError
                    await self.peak_post_processing(block, new_peak, fork_height, peer)
                else:
                    if added == ReceiveBlockResult.ADDED_AS_ORPHAN:
                        self.log.info(f"Received orphan block of height {block.height} rh {block.reward_chain_block.get_hash()}")
                    else:
                        raise RuntimeError(f"Invalid result from receive_block {added}")
        percent_full_str = ', percent full: ' + str(round(100.0 * float(block.transactions_info.cost) / self.constants.MAX_BLOCK_COST_CLVM, 3)) + '%' if block.transactions_info is not None else ''
        self.log.info(f"Block validation time: {validation_time}, cost: {block.transactions_info.cost if block.transactions_info is not None else 'None'}{percent_full_str}")
        peak = self.blockchain.get_peak()
        assert peak is not None
        clear_height = uint32(max(0, peak.height - 50))
        self.full_node_store.clear_candidate_blocks_below(clear_height)
        self.full_node_store.clear_unfinished_blocks_below(clear_height)
        if peak.height % 1000 == 0:
            if not self.sync_store.get_sync_mode():
                await self.sync_store.clear_sync_info()
            self._state_changed('block')
            record = self.blockchain.block_record(block.header_hash)
            if not self.weight_proof_handler is not None or record.sub_epoch_summary_included is not None:
    ...
    async def respond_unfinished_block(self, respond_unfinished_block: full_node_protocol.RespondUnfinishedBlock, peer: Optional[ws.WSChiaConnection], farmed_block: bool=False):
        """
        ...
        if block.prev_header_hash != self.constants.GENESIS_CHALLENGE:
            if not self.blockchain.contains_block(block.prev_header_hash):
        ...
        ses = next_sub_epoch_summary(self.constants, self.blockchain, validate_result.required_iters, block, True)
        self.full_node_store.add_unfinished_block(height, block, validate_result)
        if farmed_block is True:
            self.log.info(f"🍀 ️Farmed unfinished_block {block_hash}, SP: {block.reward_chain_block.signage_point_index}, validation time: {validation_time}, cost: {block.transactions_info.cost if block.transactions_info else 'None'}")
        else:
            percent_full_str = ', percent full: ' + str(round(100.0 * float(block.transactions_info.cost) / self.constants.MAX_BLOCK_COST_CLVM, 3)) + '%' if block.transactions_info is not None else ''
            self.log.info(f"Added unfinished_block {block_hash}, not farmed by us, SP: {block.reward_chain_block.signage_point_index} farmer response time: {time.time() - self.signage_point_times[block.reward_chain_block.signage_point_index]}, Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'xch')}, validation time: {validation_time}, cost: {block.transactions_info.cost if block.transactions_info else 'None'}{percent_full_str}")
        sub_slot_iters, difficulty = get_next_sub_slot_iters_and_difficulty(self.constants, len(block.finished_sub_slots) > 0, prev_b, self.blockchain)
        ...
        timelord_request = timelord_protocol.NewUnfinishedBlockTimelord(block.reward_chain_block, difficulty, sub_slot_iters, block.foliage, ses, rc_prev)
    ...
    async def new_infusion_point_vdf(self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSChiaConnection]=None) -> Optional[Message]:
        unfinished_block = self.full_node_store.get_unfinished_block(request.unfinished_reward_hash)
        ...
        for eos, _, _ in reversed(self.full_node_store.finished_sub_slots):
            if eos is not None:
                if eos.reward_chain.get_hash() == target_rc_hash:
        ...
        if target_rc_hash == self.constants.GENESIS_CHALLENGE:
            ...
            if prev_b is None:
                ...
                return
        finished_sub_slots = self.full_node_store.get_finished_sub_slots(self.blockchain, prev_b, last_slot_cc_hash)
        if finished_sub_slots is None:
            return
        sub_slot_iters, difficulty = get_next_sub_slot_iters_and_difficulty(self.constants, len(finished_sub_slots) > 0, prev_b, self.blockchain)
        ...
        sp_total_iters = uint128(sub_slot_start_iters + calculate_sp_iters(self.constants, sub_slot_iters, unfinished_block.reward_chain_block.signage_point_index))
        block = unfinished_block_to_full_block(unfinished_block, request.challenge_chain_ip_vdf, request.challenge_chain_ip_proof, request.reward_chain_ip_vdf, request.reward_chain_ip_proof, request.infused_challenge_chain_ip_vdf, request.infused_challenge_chain_ip_proof, finished_sub_slots, prev_b, self.blockchain, sp_total_iters, difficulty)
    ...
    async def respond_end_of_sub_slot(self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSChiaConnection) -> Tuple[(Optional[Message], bool)]:
        ...
        async with self.timelord_lock:
            fetched_ss = self.full_node_store.get_sub_slot(request.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge)
            if fetched_ss is None:
                if request.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge != self.constants.GENESIS_CHALLENGE:
                    full_node_request = full_node_protocol.RequestSignagePointOrEndOfSubSlot(request.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge, uint8(0), bytes([0] * 32))
                    return (
            ...
            new_infusions = self.full_node_store.new_finished_sub_slot(request.end_of_slot_bundle, self.blockchain, peak, await self.blockchain.get_full_peak())
            if new_infusions is not None:
                self.log.info(f"⏲️  Finished sub slot, SP {self.constants.NUM_SPS_SUB_SLOT}/{self.constants.NUM_SPS_SUB_SLOT}, {request.end_of_slot_bundle.challenge_chain.get_hash()}, number of sub-slots: {len(self.full_node_store.finished_sub_slots)}, RC hash: {request.end_of_slot_bundle.reward_chain.get_hash()}, Deficit {request.end_of_slot_bundle.reward_chain.deficit}")
                broadcast = full_node_protocol.NewSignagePointOrEndOfSubSlot(request.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge, request.end_of_slot_bundle.challenge_chain.get_hash(), uint8(0), request.end_of_slot_bundle.reward_chain.end_of_slot_vdf.challenge)
                msg = make_msg(ProtocolMessageTypes.new_signage_point_or_end_of_sub_slot, broadcast)
                await self.server.send_to_all_except([msg], NodeType.FULL_NODE, peer.peer_node_id)
                for infusion in new_infusions:
                    await self.new_infusion_point_vdf(infusion)

                broadcast_farmer = farmer_protocol.NewSignagePoint(request.end_of_slot_bundle.challenge_chain.get_hash(), request.end_of_slot_bundle.challenge_chain.get_hash(), request.end_of_slot_bundle.reward_chain.get_hash(), next_difficulty, next_sub_slot_iters, uint8(0))
                msg = make_msg(ProtocolMessageTypes.new_signage_point, broadcast_farmer)
                await self.server.send_to_all([msg], NodeType.FARMER)
                return (None, True)
            self.log.info(f"End of slot not added CC challenge {request.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge}")
        return (None, False)

    async def respond_transaction(self, transaction: SpendBundle, spend_name: bytes32, peer: Optional[ws.WSChiaConnection]=None, test: bool=False) -> Tuple[(MempoolInclusionStatus, Optional[Err])]:
        if self.sync_store.get_sync_mode():
            return (MempoolInclusionStatus.FAILED, Err.NO_TRANSACTIONS_WHILE_SYNCING)
        if not test:
            if not await self.synced():
                return (
                 MempoolInclusionStatus.FAILED, Err.NO_TRANSACTIONS_WHILE_SYNCING)
            ...
            if self.sync_store.get_sync_mode():
                ...
                error = Err.NO_TRANSACTIONS_WHILE_SYNCING
                ...
                async with self.mempool_manager.lock:
                    if self.mempool_manager.get_spendbundle(spend_name) is not None:
                        self.mempool_manager.remove_seen(spend_name)
                        return (
                         MempoolInclusionStatus.FAILED, Err.ALREADY_INCLUDING_TRANSACTION)
                    cost, status, error = await self.mempool_manager.add_spendbundle(transaction, cost_result, spend_name)
                if status == MempoolInclusionStatus.SUCCESS:
                    self.log.debug(f"Added transaction to mempool: {spend_name} mempool size: {self.mempool_manager.mempool.total_mempool_cost}")
                    mempool_item = self.mempool_manager.get_mempool_item(spend_name)
                    assert mempool_item is not None
                    fees = mempool_item.fee
                    assert fees >= 0
                    assert cost is not None
                    new_tx = full_node_protocol.NewTransaction(spend_name, cost, fees)
                    msg = make_msg(ProtocolMessageTypes.new_transaction, new_tx)
                    if peer is None:
                        await self.server.send_to_all([msg], NodeType.FULL_NODE)
                    else:
    ...
    async def _needs_compact_proof(self, vdf_info, header_block, field_vdf):
            ...
            for sub_slot in header_block.finished_sub_slots:
                if sub_slot.infused_challenge_chain is not None:
                    if sub_slot.infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf == vdf_info:
                        if not sub_slot.proofs.infused_challenge_chain_slot_proof is not None:
                            raise AssertionError
                        else:
                            if sub_slot.proofs.infused_challenge_chain_slot_proof.witness_type == 0:
        ...
        if field_vdf == CompressibleVDFField.CC_SP_VDF:
            ...
            if vdf_info == header_block.reward_chain_block.challenge_chain_sp_vdf:
                ...
                if header_block.challenge_chain_sp_proof.witness_type == 0:
                    if header_block.challenge_chain_sp_proof.normalized_to_identity:
                        return False
                return True
        if field_vdf == CompressibleVDFField.CC_IP_VDF:
            if vdf_info == header_block.reward_chain_block.challenge_chain_ip_vdf:
                if header_block.challenge_chain_ip_proof.witness_type == 0:
                    if header_block.challenge_chain_ip_proof.normalized_to_identity:
                        return False
                return True
        return False

    async def _can_accept_compact_proof(self, vdf_info, vdf_proof, height, header_hash, field_vdf):
        """
        ...
        if is_fully_compactified is None or is_fully_compactified:
            ...
            return False
        if not (vdf_proof.witness_type > 0 or vdf_proof.normalized_to_identity):
    ...
    async def _replace_proof(self, vdf_info, vdf_proof, height, field_vdf):
    ...
    async def respond_compact_proof_of_time(self, request: timelord_protocol.RespondCompactProofOfTime):
        ...
        if not await self._can_accept_compact_proof(request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf):
            return
        ...
        if not replaced:
            ...
            return
        msg = make_msg(ProtocolMessageTypes.new_compact_vdf, full_node_protocol.NewCompactVDF(request.height, request.header_hash, request.field_vdf, request.vdf_info))
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSChiaConnection):
        ...
        header_block = await self.blockchain.get_header_block_by_height((request.height),
          (request.header_hash), tx_filter=False)
        if header_block is None:
            return
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if await self._needs_compact_proof(request.vdf_info, header_block, field_vdf):
            peer_request = full_node_protocol.RequestCompactVDF(request.height, request.header_hash, request.field_vdf, request.vdf_info)
            response = await peer.request_compact_vdf(peer_request, timeout=10)
            if response is not None:
                if isinstance(response, full_node_protocol.RespondCompactVDF):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSChiaConnection):
        ...
        if field_vdf == CompressibleVDFField.ICC_EOS_VDF:
            for sub_slot in header_block.finished_sub_slots:
                if sub_slot.infused_challenge_chain is not None:
                    if sub_slot.infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf == request.vdf_info:
                        vdf_proof = sub_slot.proofs.infused_challenge_chain_slot_proof
                        break

        if field_vdf == CompressibleVDFField.CC_SP_VDF:
            if header_block.reward_chain_block.challenge_chain_sp_vdf == request.vdf_info:
                vdf_proof = header_block.challenge_chain_sp_proof
        if field_vdf == CompressibleVDFField.CC_IP_VDF:
            if header_block.reward_chain_block.challenge_chain_ip_vdf == request.vdf_info:
                vdf_proof = header_block.challenge_chain_ip_proof
        if vdf_proof is None or vdf_proof.witness_type > 0 or vdf_proof.normalized_to_identity:
            self.log.error(f"{peer} requested compact vdf we don't have, height: {request.height}.")
            return
        compact_vdf = full_node_protocol.RespondCompactVDF(request.height, request.header_hash, request.field_vdf, request.vdf_info, vdf_proof)
    ...
    async def broadcast_uncompact_blocks(self, uncompact_interval_scan: int, target_uncompact_proofs: int, sanitize_weight_proof_only: bool):
        try:
                                ...
                                if header.header_hash != expected_header_hash:
                                    ...
                                    for sub_slot in header.finished_sub_slots:
                                        if not (sub_slot.proofs.challenge_chain_slot_proof.witness_type > 0 or sub_slot.proofs.challenge_chain_slot_proof.normalized_to_identity):
                                            broadcast_list.append(timelord_protocol.RequestCompactProofOfTime(sub_slot.challenge_chain.challenge_chain_end_of_slot_vdf, header.header_hash, header.height, uint8(CompressibleVDFField.CC_EOS_VDF)))
                                        if sub_slot.proofs.infused_challenge_chain_slot_proof is not None:
                                            if not sub_slot.proofs.infused_challenge_chain_slot_proof.witness_type > 0 or sub_slot.proofs.infused_challenge_chain_slot_proof.normalized_to_identity:
                                                pass
                                            if not sub_slot.infused_challenge_chain is not None:
                                                raise AssertionError
                                            else:
                                                broadcast_list.append(timelord_protocol.RequestCompactProofOfTime(sub_slot.infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf, header.header_hash, header.height, uint8(CompressibleVDFField.ICC_EOS_VDF)))

                                if sanitize_weight_proof_only:
                                    if not record.is_challenge_block(self.constants):
                                        continue
                                if header.challenge_chain_sp_proof is not None:
                                    if not (header.challenge_chain_sp_proof.witness_type > 0 or header.challenge_chain_sp_proof.normalized_to_identity):
                                        assert header.reward_chain_block.challenge_chain_sp_vdf is not None
                                        broadcast_list.append(timelord_protocol.RequestCompactProofOfTime(header.reward_chain_block.challenge_chain_sp_vdf, header.header_hash, header.height, uint8(CompressibleVDFField.CC_SP_VDF)))
                                    if not header.challenge_chain_ip_proof.witness_type > 0 or header.challenge_chain_ip_proof.normalized_to_identity:
                                        broadcast_list.append(timelord_protocol.RequestCompactProofOfTime(header.reward_chain_block.challenge_chain_ip_vdf, header.header_hash, header.height, uint8(CompressibleVDFField.CC_IP_VDF)))
