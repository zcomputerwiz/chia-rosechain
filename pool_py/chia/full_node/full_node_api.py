...
# Embedded file name: chia\full_node\full_node_api.py
import asyncio, dataclasses, time
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: ws.WSChiaConnection) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def respond_peers_introducer(self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSChiaConnection) -> Optional[Message]:
    ...
    async def new_transaction(self, transaction: full_node_protocol.NewTransaction, peer: ws.WSChiaConnection) -> Optional[Message]:
        ...
        if self.full_node.mempool_manager.is_fee_enough(transaction.fees, transaction.cost):
            ...
            async def tx_request_and_timeout(full_node, transaction_id, task_id):
                counter = 0
                try:
                    try:
                        while 1:
                            ...
                            if transaction_id not in full_node.full_node_store.peers_with_tx:
                                ...
                                peers_with_tx = full_node.full_node_store.peers_with_tx[transaction_id]
    ...
    async def request_proof_of_weight(self, request: full_node_protocol.RequestProofOfWeight) -> Optional[Message]:
        ...
        message = make_msg(ProtocolMessageTypes.respond_proof_of_weight, full_node_protocol.RespondProofOfWeight(wp, request.tip))
    ...
    async def request_block(self, request: full_node_protocol.RequestBlock) -> Optional[Message]:
        ...
        block = await self.full_node.block_store.get_full_block(header_hash)
    ...
    async def request_blocks(self, request: full_node_protocol.RequestBlocks) -> Optional[Message]:
        ...
        if not request.include_transaction_block:
            blocks = []
            for i in range(request.start_height, request.end_height + 1):
                block = await self.full_node.block_store.get_full_block(self.full_node.blockchain.height_to_hash(uint32(i)))
                if block is None:
                    reject = RejectBlocks(request.start_height, request.end_height)
                    msg = make_msg(ProtocolMessageTypes.reject_blocks, reject)
                    return msg
            ...
            msg = make_msg(ProtocolMessageTypes.respond_blocks, full_node_protocol.RespondBlocks(request.start_height, request.end_height, blocks))
        else:
            blocks_bytes = []
            for i in range(request.start_height, request.end_height + 1):
                block_bytes = await self.full_node.block_store.get_full_block_bytes(self.full_node.blockchain.height_to_hash(uint32(i)))
            ...
            respond_blocks_manually_streamed = bytes(uint32(request.start_height)) + bytes(uint32(request.end_height)) + len(blocks_bytes).to_bytes(4, 'big', signed=False)
    ...
    @api_request
    async def new_unfinished_block(self, new_unfinished_block: full_node_protocol.NewUnfinishedBlock) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return
        ...
        if block_hash in self.full_node.full_node_store.requesting_unfinished_blocks:
            return
        msg = make_msg(ProtocolMessageTypes.request_unfinished_block, full_node_protocol.RequestUnfinishedBlock(block_hash))
    ...
    @api_request
    async def request_unfinished_block(self, request_unfinished_block: full_node_protocol.RequestUnfinishedBlock) -> Optional[Message]:
        unfinished_block = self.full_node.full_node_store.get_unfinished_block(request_unfinished_block.unfinished_reward_hash)
        if unfinished_block is not None:
            msg = make_msg(ProtocolMessageTypes.respond_unfinished_block, full_node_protocol.RespondUnfinishedBlock(unfinished_block))
            return msg

    @peer_required
    @api_request
    async def respond_unfinished_block(self, respond_unfinished_block: full_node_protocol.RespondUnfinishedBlock, peer: ws.WSChiaConnection) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return
    ...
    @api_request
    @peer_required
    async def new_signage_point_or_end_of_sub_slot(self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSChiaConnection) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return
        if self.full_node.full_node_store.get_signage_point_by_index(new_sp.challenge_hash, new_sp.index_from_challenge, new_sp.last_rc_infusion) is not None:
            return
        if self.full_node.full_node_store.have_newer_signage_point(new_sp.challenge_hash, new_sp.index_from_challenge, new_sp.last_rc_infusion):
            return
        if new_sp.index_from_challenge == 0:
            if new_sp.prev_challenge_hash is not None:
                if self.full_node.full_node_store.get_sub_slot(new_sp.prev_challenge_hash) is None:
                    ...
                    for _ in range(30):
                        if num_non_empty_sub_slots_seen >= 3:
                            ...
                            full_node_request = full_node_protocol.RequestSignagePointOrEndOfSubSlot(challenge_hash_to_request, uint8(0), last_rc)
                            ...
                            if self.full_node.full_node_store.get_sub_slot(response.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge) is not None or response.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge == self.full_node.constants.GENESIS_CHALLENGE:
                                for eos in reversed(collected_eos):
                                    await self.respond_end_of_sub_slot(eos, peer)

                                return
                            if response.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.number_of_iterations != response.end_of_slot_bundle.reward_chain.end_of_slot_vdf.number_of_iterations:
                                num_non_empty_sub_slots_seen += 1
                            challenge_hash_to_request = response.end_of_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge
                    ...
                    return
        if new_sp.index_from_challenge > 0:
            if new_sp.challenge_hash != self.full_node.constants.GENESIS_CHALLENGE:
                if self.full_node.full_node_store.get_sub_slot(new_sp.challenge_hash) is None:
                    full_node_request = full_node_protocol.RequestSignagePointOrEndOfSubSlot(new_sp.challenge_hash, uint8(0), new_sp.last_rc_infusion)
                    return make_msg(ProtocolMessageTypes.request_signage_point_or_end_of_sub_slot, full_node_request)
        full_node_request = full_node_protocol.RequestSignagePointOrEndOfSubSlot(new_sp.challenge_hash, new_sp.index_from_challenge, new_sp.last_rc_infusion)
        return make_msg(ProtocolMessageTypes.request_signage_point_or_end_of_sub_slot, full_node_request)

    @api_request
    async def request_signage_point_or_end_of_sub_slot(self, request: full_node_protocol.RequestSignagePointOrEndOfSubSlot) -> Optional[Message]:
        if request.index_from_challenge == 0:
            sub_slot = self.full_node.full_node_store.get_sub_slot(request.challenge_hash)
            if sub_slot is not None:
                return make_msg(ProtocolMessageTypes.respond_end_of_sub_slot, full_node_protocol.RespondEndOfSubSlot(sub_slot[0]))
        else:
            ...
            sp = self.full_node.full_node_store.get_signage_point_by_index(request.challenge_hash, request.index_from_challenge, request.last_rc_infusion)
            if sp is not None:
                if sp.cc_vdf is not None:
                    if sp.cc_proof is not None:
                        if not (sp.rc_vdf is not None and sp.rc_proof is not None):
                            raise AssertionError
                    full_node_response = full_node_protocol.RespondSignagePoint(request.index_from_challenge, sp.cc_vdf, sp.cc_proof, sp.rc_vdf, sp.rc_proof)
    ...
    async def respond_signage_point(self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSChiaConnection) -> Optional[Message]:
        ...
        async with self.full_node.timelord_lock:
            if self.full_node.full_node_store.have_newer_signage_point(request.challenge_chain_vdf.challenge, request.index_from_challenge, request.reward_chain_vdf.challenge):
                return
            existing_sp = self.full_node.full_node_store.get_signage_point(request.challenge_chain_vdf.output.get_hash())
            if existing_sp is not None:
                if existing_sp.rc_vdf == request.reward_chain_vdf:
                    return
            ...
            if peak is not None and peak.height > self.full_node.constants.MAX_SUB_SLOT_BLOCKS:
                ...
                ip_sub_slot = sub_slots_for_peak[1]
            else:
                sub_slot_iters = self.full_node.constants.SUB_SLOT_ITERS_STARTING
                next_sub_slot_iters = sub_slot_iters
                ip_sub_slot = None
            added = self.full_node.full_node_store.new_signage_point(request.index_from_challenge, self.full_node.blockchain, self.full_node.blockchain.get_peak(), next_sub_slot_iters, SignagePoint(request.challenge_chain_vdf, request.challenge_chain_proof, request.reward_chain_vdf, request.reward_chain_proof))
            if added:
                await self.full_node.signage_point_post_processing(request, peer, ip_sub_slot)
            else:
                self.log.debug(f"Signage point {request.index_from_challenge} not added, CC challenge: {request.challenge_chain_vdf.challenge}, RC challenge: {request.reward_chain_vdf.challenge}")
            return

    @peer_required
    @api_request
    async def respond_end_of_sub_slot(self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSChiaConnection) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return
    ...
    @peer_required
    @api_request
    async def request_mempool_transactions(self, request: full_node_protocol.RequestMempoolTransactions, peer: ws.WSChiaConnection) -> Optional[Message]:
        received_filter = PyBIP158(bytearray(request.filter))
        items = await self.full_node.mempool_manager.get_items_not_in_filter(received_filter)
        for item in items:
    ...
    async def declare_proof_of_space(self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSChiaConnection) -> Optional[Message]:
        ...
        if self.full_node.sync_store.get_sync_mode():
            return
        async with self.full_node.timelord_lock:
            sp_vdfs = self.full_node.full_node_store.get_signage_point(request.challenge_chain_sp)
            if sp_vdfs is None:
            ...
            if request.signage_point_index > 0:
                ...
                if sp_vdfs.rc_vdf.output.get_hash() != request.reward_chain_sp:
                    ...
                    return
            if request.signage_point_index == 0:
                cc_challenge_hash = request.challenge_chain_sp
            else:
            ...
            if request.challenge_hash != self.full_node.constants.GENESIS_CHALLENGE:
                ...
                if pos_sub_slot is None:
                    ...
                    return
                total_iters_pos_slot = pos_sub_slot[2]
            else:
                total_iters_pos_slot = uint128(0)
            assert cc_challenge_hash == request.challenge_hash
            quality_string = request.proof_of_space.verify_and_get_quality_string(self.full_node.constants, cc_challenge_hash, request.challenge_chain_sp)
            if not (quality_string is not None and len(quality_string) == 32):
                raise AssertionError
            ...
            def get_pool_sig(_1, _2):
                return request.pool_signature

            prev_b = self.full_node.blockchain.get_peak()
            if prev_b is not None:
                ...
                for eos, _, _ in reversed(self.full_node.full_node_store.finished_sub_slots):
                    if eos is not None:
                        if eos.reward_chain.get_hash() == rc_challenge:
                ...
                while prev_b is not None:
                    if attempts < 10:
                        if prev_b.reward_infusion_new_challenge == rc_challenge:
                            found = True
                            break
                        if prev_b.finished_reward_slot_hashes is not None:
                            if len(prev_b.finished_reward_slot_hashes) > 0:
                ...
                if not found:
                    ...
                    return
                try:
                    finished_sub_slots = self.full_node.full_node_store.get_finished_sub_slots(self.full_node.blockchain, prev_b, cc_challenge_hash)
                    if finished_sub_slots is None:
                        return
                    if len(finished_sub_slots) > 0:
                        if pos_sub_slot is not None:
                            if finished_sub_slots[-1] != pos_sub_slot[0]:
                ...
                required_iters = calculate_iterations_quality(self.full_node.constants.DIFFICULTY_CONSTANT_FACTOR, quality_string, request.proof_of_space.size, difficulty, request.challenge_chain_sp)
                sp_iters = calculate_sp_iters(self.full_node.constants, sub_slot_iters, request.signage_point_index)
                ip_iters = calculate_ip_iters(self.full_node.constants, sub_slot_iters, request.signage_point_index, required_iters)
                ...
                if unfinished_block.is_transaction_block():
                    if unfinished_block.transactions_generator is not None:
                        ...
                        self.full_node.full_node_store.add_candidate_block(quality_string,
                          height, unfinished_block_backup, backup=True)
    ...
    async def signed_values(self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSChiaConnection) -> Optional[Message]:
        ...
        """
        candidate_tuple = self.full_node.full_node_store.get_candidate_block(farmer_request.quality_string)
        ...
        if not AugSchemeMPL.verify(candidate.reward_chain_block.proof_of_space.plot_public_key, candidate.foliage.foliage_block_data.get_hash(), farmer_request.foliage_block_data_signature):
            ...
            return
        fsb2 = dataclasses.replace((candidate.foliage),
          foliage_block_data_signature=(farmer_request.foliage_block_data_signature))
        if candidate.is_transaction_block():
            fsb2 = dataclasses.replace(fsb2,
              foliage_transaction_block_signature=(farmer_request.foliage_transaction_block_signature))
    ...
    @peer_required
    @api_request
    async def new_infusion_point_vdf(self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSChiaConnection) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return
        async with self.full_node.timelord_lock:
    ...
    @peer_required
    @api_request
    async def new_signage_point_vdf(self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSChiaConnection) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return
        full_node_message = full_node_protocol.RespondSignagePoint(request.index_from_challenge, request.challenge_chain_sp_vdf, request.challenge_chain_sp_proof, request.reward_chain_sp_vdf, request.reward_chain_sp_proof)
    ...
    async def new_end_of_sub_slot_vdf(self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSChiaConnection) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return
        if self.full_node.full_node_store.get_sub_slot(request.end_of_sub_slot_bundle.challenge_chain.get_hash()) is not None:
            return
        ...
        if not added:
            self.log.error(f"Was not able to add end of sub-slot: {request.end_of_sub_slot_bundle.challenge_chain.challenge_chain_end_of_slot_vdf.challenge}. Re-sending new-peak to timelord")
    ...
    async def request_block_header(self, request: wallet_protocol.RequestBlockHeader) -> Optional[Message]:
        ...
        if block is not None:
            ...
            msg = make_msg(ProtocolMessageTypes.respond_block_header, wallet_protocol.RespondBlockHeader(header_block))
            return msg

    @api_request
    async def request_additions(self, request: wallet_protocol.RequestAdditions) -> Optional[Message]:
        block = await self.full_node.block_store.get_full_block(request.header_hash)
        if not block is None:
            if block.is_transaction_block() is False or self.full_node.blockchain.height_to_hash(block.height) != request.header_hash:
                ...
                return msg
            if not (block is not None and block.foliage_transaction_block is not None):
                raise AssertionError
            ...
            for coin_record in additions:
                if coin_record.coin.puzzle_hash in puzzlehash_coins_map:
                    ...
                    puzzlehash_coins_map[coin_record.coin.puzzle_hash] = [
                     coin_record.coin]
    ...
    async def request_removals(self, request: wallet_protocol.RequestRemovals) -> Optional[Message]:
        block = await self.full_node.block_store.get_full_block(request.header_hash)
        if not block is None:
            if block.is_transaction_block() is False or block.height != request.height or block.height > self.full_node.blockchain.get_peak_height() or self.full_node.blockchain.height_to_hash(block.height) != request.header_hash:
            ...
            if not (block is not None and block.foliage_transaction_block is not None):
                raise AssertionError
            all_removals = await self.full_node.coin_store.get_coins_removed_at_height(block.height)
            if self.full_node.blockchain.height_to_hash(block.height) != request.header_hash:
                raise ValueError(f"Block {block.header_hash} no longer in chain")
            all_removals_dict = {}
            ...
            if block.transactions_generator is None:
                ...
                if request.coin_names is None or len(request.coin_names) == 0:
    ...
    async def request_puzzle_solution(self, request: wallet_protocol.RequestPuzzleSolution) -> Optional[Message]:
        ...
        error, puzzle, solution = get_puzzle_and_solution_for_coin(block_generator, coin_name, self.full_node.constants.MAX_BLOCK_COST_CLVM)
    ...
    async def request_header_blocks(self, request: wallet_protocol.RequestHeaderBlocks) -> Optional[Message]:
        ...
        blocks = await self.full_node.block_store.get_blocks_by_hash(header_hashes)
        header_blocks = []
        for block in blocks:
        ...
        msg = make_msg(ProtocolMessageTypes.respond_header_blocks, wallet_protocol.RespondHeaderBlocks(request.start_height, request.end_height, header_blocks))
        return msg
