...
# Embedded file name: chia\full_node\coin_store.py
...
class CoinStore:
    ...
    async def create(cls, db_wrapper: DBWrapper, cache_size: uint32=uint32(60000)):
        ...
        await self.coin_record_db.execute('CREATE TABLE IF NOT EXISTS coin_record(coin_name text PRIMARY KEY, confirmed_index bigint, spent_index bigint, spent int, coinbase int, puzzle_hash text, coin_parent text, amount blob, timestamp bigint)')
        await self.coin_record_db.execute('CREATE INDEX IF NOT EXISTS coin_confirmed_index on coin_record(confirmed_index)')
    ...
    async def new_block(self, block: FullBlock, tx_additions: List[Coin], tx_removals: List[bytes32]):
        ...
        for coin in tx_additions:
            record = CoinRecord(coin, block.height, uint32(0), False, False, block.foliage_transaction_block.timestamp)
            await self._add_coin_record(record, False)

        included_reward_coins = block.get_included_reward_coins()
        if block.height == 0:
            if not len(included_reward_coins) == 0:
                raise AssertionError
        else:
            assert len(included_reward_coins) >= 2
        for coin in included_reward_coins:
            reward_coin_r = CoinRecord(coin, block.height, uint32(0), False, True, block.foliage_transaction_block.timestamp)
    ...
    async def get_coin_records_by_puzzle_hash(self, include_spent_coins: bool, puzzle_hash: bytes32, start_height: uint32=uint32(0), end_height: uint32=uint32(4294967295)) -> List[CoinRecord]:
        coins = set()
        cursor = await self.coin_record_db.execute(f"SELECT * from coin_record WHERE puzzle_hash=? AND confirmed_index>=? AND confirmed_index<? {'' if include_spent_coins else 'AND spent=0'}", (
    ...
    async def get_coin_records_by_puzzle_hashes(self, include_spent_coins: bool, puzzle_hashes: List[bytes32], start_height: uint32=uint32(0), end_height: uint32=uint32(4294967295)) -> List[CoinRecord]:
        if len(puzzle_hashes) == 0:
            return []
        coins = set()
        puzzle_hashes_db = tuple([ph.hex() for ph in puzzle_hashes])
        cursor = await self.coin_record_db.execute(f"SELECT * from coin_record WHERE puzzle_hash in ({'?,' * (len(puzzle_hashes_db) - 1)}?) AND confirmed_index>=? AND confirmed_index<? {'' if include_spent_coins else 'AND spent=0'}", puzzle_hashes_db + (start_height, end_height))
        rows = await cursor.fetchall()
        await cursor.close()
        for row in rows:
            coin = Coin(bytes32(bytes.fromhex(row[6])), bytes32(bytes.fromhex(row[5])), uint64.from_bytes(row[7]))
    ...
    async def get_coin_records_by_parent_ids(self, include_spent_coins: bool, parent_ids: List[bytes32], start_height: uint32=uint32(0), end_height: uint32=uint32(4294967295)) -> List[CoinRecord]:
        if len(parent_ids) == 0:
            return []
        coins = set()
        parent_ids_db = tuple([pid.hex() for pid in parent_ids])
        cursor = await self.coin_record_db.execute(f"SELECT * from coin_record WHERE coin_parent in ({'?,' * (len(parent_ids_db) - 1)}?) AND confirmed_index>=? AND confirmed_index<? {'' if include_spent_coins else 'AND spent=0'}", parent_ids_db + (start_height, end_height))
        rows = await cursor.fetchall()
        await cursor.close()
        for row in rows:
            coin = Coin(bytes32(bytes.fromhex(row[6])), bytes32(bytes.fromhex(row[5])), uint64.from_bytes(row[7]))
    ...
    async def rollback_to_block(self, block_index: int):
        ...
        for coin_name, coin_record in list(self.coin_record_cache.cache.items()):
            if int(coin_record.spent_block_index) > block_index:
                new_record = CoinRecord(coin_record.coin, coin_record.confirmed_block_index, uint32(0), False, coin_record.coinbase, coin_record.timestamp)
        ...
        await c1.close()
        c2 = await self.coin_record_db.execute('UPDATE coin_record SET spent_index = 0, spent = 0 WHERE spent_index>?', (
         block_index,))
        await c2.close()
    ...
    async def _add_coin_record(self, record: CoinRecord, allow_replace: bool) -> None:
        ...
        cursor = await self.coin_record_db.execute(f"INSERT {'OR REPLACE ' if allow_replace else ''}INTO coin_record VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)", (
    ...
    async def _set_spent(self, coin_name: bytes32, index: uint32) -> uint64:
        current = await self.get_coin_record(coin_name)
        if current is None:
            raise ValueError(f"Cannot spend a coin that does not exist in db: {coin_name}")
        assert not current.spent
        spent = CoinRecord(current.coin, current.confirmed_block_index, index, True, current.coinbase, current.timestamp)
