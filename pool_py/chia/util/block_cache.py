...
# Embedded file name: chia\util\block_cache.py
import logging
...
class BlockCache(BlockchainInterface):
    ...
    def __init__(self, blocks: Dict[(bytes32, BlockRecord)], headers: Dict[(bytes32, HeaderBlock)]=None, height_to_hash: Dict[(uint32, bytes32)]=None, sub_epoch_summaries: Dict[(uint32, SubEpochSummary)]=None):
    ...
    async def persist_sub_epoch_challenge_segments(self, sub_epoch_summary_height: uint32, segments: List[SubEpochChallengeSegment]):
        self._sub_epoch_segments[sub_epoch_summary_height] = SubEpochSegments(segments)

    async def get_sub_epoch_challenge_segments(self, sub_epoch_summary_height: uint32) -> Optional[List[SubEpochChallengeSegment]]:
