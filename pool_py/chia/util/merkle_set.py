...
# Embedded file name: chia\util\merkle_set.py
...
class TerminalNode(Node):
    ...
    def _audit(self, hashes: List[bytes], bits: List[int]):
        ...
        for pos, v in enumerate(bits):
            if not get_bit(self.hash, pos) == v:
...
class MiddleNode(Node):
    ...
    def __init__(self, children: List[Node]):
        ...
        if children[0].is_empty() and children[1].is_double():
            ...
            if children[1].is_empty() and children[0].is_double():
                self.hash = children[0].hash
            else:
                if children[0].is_empty():
                    if children[1].is_empty() or children[1].is_terminal():
                        raise SetError()
                    if children[1].is_empty():
                        if children[0].is_terminal():
                            raise SetError
                    if children[0].is_terminal():
                        if children[1].is_terminal():
    ...
    def remove(self, toremove: bytes, depth: int) -> Node:
        ...
        if newchild.is_empty():
            if otherchild.is_terminal():
                return otherchild
        if newchild.is_terminal():
            if otherchild.is_empty():
                return newchild
        newvals = [x for x in self.children]
        newvals[bit] = newchild
        return MiddleNode(newvals)
