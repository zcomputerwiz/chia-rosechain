...
# Embedded file name: chia\ssl\create_ssl.py
import datetime
...
def generate_ca_signed_cert(ca_crt, ca_key, cert_out, key_out):
    ...
    key_pem = cert_key.private_bytes(encoding=(serialization.Encoding.PEM),
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name([
     ...
     x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, 'Organic Farming Division')])
    root_cert = x509.CertificateBuilder().subject_name(subject).issuer_name(issuer).public_key(root_key.public_key()).serial_number(x509.random_serial_number()).not_valid_before(datetime.datetime.utcnow()).not_valid_after(datetime.datetime.utcnow() + datetime.timedelta(days=3650)).add_extension(x509.BasicConstraints(ca=True, path_length=None),
      critical=True).sign(root_key, hashes.SHA256(), default_backend())
    cert_path.write_bytes(root_cert.public_bytes(encoding=(serialization.Encoding.PEM)))
    key_path.write_bytes(root_key.private_bytes(encoding=(serialization.Encoding.PEM),
      format=(serialization.PrivateFormat.TraditionalOpenSSL),
      encryption_algorithm=(serialization.NoEncryption())))
