...
# Embedded file name: chia\cmds\plots.py
import logging, sys
...
def show_plots(root_path: Path):
    ...
    print("Add with 'chia plots add -d [dir]' and remove with 'chia plots remove -d [dir]' Scan and check plots with 'chia plots check'")
    print()
...
@click.option('-x',
  '--exclude_final_dir', help='Skips adding [final dir] to harvester for farming', default=False, is_flag=True)
@click.pass_context
def create_cmd(ctx: click.Context, size: int, override_k: bool, num: int, buffer: int, num_threads: int, buckets: int, alt_fingerprint: int, pool_contract_address: str, farmer_public_key: str, pool_public_key: str, tmp_dir: str, tmp2_dir: str, final_dir: str, plotid: str, memo: str, nobitfield: bool, exclude_final_dir: bool):
    import chia.plotting.create_plots as create_plots
...
@click.pass_context
def check_cmd(ctx: click.Context, num: int, grep_string: str, list_duplicates: bool, debug_show_memo: bool, challenge_start: int):
    import chia.plotting.check_plots as check_plots
