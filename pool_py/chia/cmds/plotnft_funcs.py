...
# Embedded file name: chia\cmds\plotnft_funcs.py
from collections import Counter
import aiohttp, asyncio, functools, json, time
...
async def create(args, wallet_client, fingerprint):
    state = args['state']
    ...
    if state == 'SELF_POOLING':
        ...
        if state == 'FARMING_TO_POOL':
            ...
            if enforce_https:
                if not pool_url.startswith('https://'):
    ...
    if prompt:
        user_input = input('Confirm [n]/y: ')
    else:
        user_input = 'yes'
    if user_input.lower() == 'y' or user_input.lower() == 'yes':
        try:
            tx_record = await wallet_client.create_new_pool_wallet(target_puzzle_hash, pool_url, relative_lock_height, 'localhost:5000', 'new', state)
            start = time.time()
...
async def pprint_pool_wallet_state(wallet_client, wallet_id, pool_wallet_info, address_prefix, pool_state_dict, plot_counts):
    ...
    print(f"Target address (not for plotting): {encode_puzzle_hash(pool_wallet_info.current.target_puzzle_hash, address_prefix)}")
    ...
    print(f"Pool contract address (use ONLY for plotting - do not send money to this address): {encode_puzzle_hash(pool_wallet_info.p2_singleton_puzzle_hash, address_prefix)}")
    ...
    if pool_wallet_info.current.state == PoolSingletonState.SELF_POOLING.value:
        balances = await wallet_client.get_wallet_balance(str(wallet_id))
    ...
    if pool_wallet_info.current.state == PoolSingletonState.FARMING_TO_POOL:
        ...
        payout_instructions = pool_state_dict[pool_wallet_info.launcher_id]['pool_config']['payout_instructions']
        try:
...
async def show(args, wallet_client, fingerprint):
    config = load_config(DEFAULT_ROOT_PATH, 'config.yaml')
    ...
    try:
        ...
        try:
            if isinstance(e, aiohttp.ClientConnectorError):
                print(f"Connection error. Check if farmer is running at {farmer_rpc_port}. You can run the farmer by:\n    chia start farmer-only")
            else:
    ...
    pool_state_dict = {hexstr_to_bytes(pool_state_item['pool_config']['launcher_id']): pool_state_item for pool_state_item in pool_state_list}
    if wallet_id_passed_in is not None:
        for summary in summaries_response:
            typ = WalletType(int(summary['type']))
            if summary['id'] == wallet_id_passed_in:
        ...
        await pprint_pool_wallet_state(wallet_client, wallet_id_passed_in, pool_wallet_info, address_prefix, pool_state_dict, plot_counts)
    else:
        ...
        for summary in summaries_response:
            ...
            if typ == WalletType.POOLING_WALLET:
                ...
                await pprint_pool_wallet_state(wallet_client, wallet_id, pool_wallet_info, address_prefix, pool_state_dict, plot_counts)
                print('')
...
async def get_login_link(launcher_id_str: str) -> None:
    launcher_id = hexstr_to_bytes(launcher_id_str)
    config = load_config(DEFAULT_ROOT_PATH, 'config.yaml')
    self_hostname = config['self_hostname']
    farmer_rpc_port = config['farmer']['rpc_port']
    farmer_client = await FarmerRpcClient.create(self_hostname, uint16(farmer_rpc_port), DEFAULT_ROOT_PATH, config)
    try:
        try:
            login_link = await farmer_client.get_pool_login_link(launcher_id)
...
async def submit_tx_with_confirmation(message, prompt, func, wallet_client, fingerprint, wallet_id):
    print(message)
    if prompt:
        user_input = input('Confirm [n]/y: ')
    else:
...
async def join_pool(args, wallet_client, fingerprint):
    config = load_config(DEFAULT_ROOT_PATH, 'config.yaml')
    enforce_https = config['full_node']['selected_network'] == 'mainnet'
    ...
    func = functools.partial(wallet_client.pw_join_pool, wallet_id, hexstr_to_bytes(json_dict['target_puzzle_hash']), pool_url, json_dict['relative_lock_height'])
    await submit_tx_with_confirmation(msg, prompt, func, wallet_client, fingerprint, wallet_id)


async def self_pool(args, wallet_client, fingerprint):
    wallet_id = args.get('id', None)
    prompt = not args.get('yes', False)
...
async def inspect_cmd(args, wallet_client, fingerprint):
    wallet_id = args.get('id', None)
    pool_wallet_info, unconfirmed_transactions = await wallet_client.pw_status(wallet_id)
    print({'pool_wallet_info':pool_wallet_info, 
     'unconfirmed_transactions':[{'sent_to':tx.sent_to,  'transaction_id':tx.name.hex()} for tx in unconfirmed_transactions]})


async def claim_cmd(args, wallet_client, fingerprint):
    wallet_id = args.get('id', None)
    msg = f"\nWill claim rewards for wallet ID: {wallet_id}."
    func = functools.partial(wallet_client.pw_absorb_rewards, wallet_id)
