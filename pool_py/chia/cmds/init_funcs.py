...
# Embedded file name: chia\cmds\init_funcs.py
import os, shutil
...
from chia.util.config import create_default_chia_config, initial_config_file, load_config, save_config, unflatten_properties
from chia.util.ints import uint32
from chia.util.keychain import Keychain
from chia.util.path import mkdir
from chia.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
private_node_names = {
 'full_node','wallet','farmer','harvester','timelord','daemon'}
...
def dict_add_new_default(updated: Dict, default: Dict, do_not_migrate_keys: Dict[(str, Any)]):
    ...
    for k, v in default.items():
        ...
        if isinstance(v, dict):
            if k in updated and ignore is False:
...
def check_keys(new_root: Path) -> None:
        ...
        if stop_searching_for_farmer:
            if stop_searching_for_pool:
                if i > 0:
                    break
        for sk, _ in all_sks:
            all_targets.append(encode_puzzle_hash(create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(i)).get_g1()), prefix))
    ...
    if 'xch_target_address' not in config['farmer']:
        ...
        if config['farmer']['xch_target_address'] not in all_targets:
            print(f"WARNING: using a farmer address which we don't have the private keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding {config['farmer']['xch_target_address']} with {all_targets[0]}")
    ...
    if 'xch_target_address' not in config['pool']:
        ...
        if config['pool']['xch_target_address'] not in all_targets:
            print(f"WARNING: using a pool address which we don't have the private keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding {config['pool']['xch_target_address']} with {all_targets[0]}")
...
def migrate_from(old_root: Path, new_root: Path, manifest: List[str], do_not_migrate_settings: List[str]):
    """
...
def create_all_ssl(root: Path):
    ...
    if not (private_ca_key_path.exists() and private_ca_crt_path.exists()):
...
def generate_ssl_for_nodes(ssl_dir, ca_crt, ca_key, private, overwrite=True):
    if private:
    ...
    if create_certs is not None:
        if root_path.exists():
            if os.path.isdir(create_certs):
                ca_dir = root_path / 'config/ssl/ca'
                if ca_dir.exists():
...
def chia_version_number() -> Tuple[(str, str, str, str)]:
    ...
    if '0b' in scm_minor_version:
        ...
        if not smc_patch_version or 'dev' in smc_patch_version:
            dev_release_number = '.' + smc_patch_version
    else:
        if '0rc' in version[1]:
            original_minor_ver_list = scm_minor_version.split('0rc')
            major_release_number = str(1 - int(scm_major_version))
            minor_release_number = str(int(scm_major_version) + 1)
            patch_release_number = original_minor_ver_list[1]
            if not smc_patch_version or 'dev' in smc_patch_version:
                dev_release_number = '.' + smc_patch_version
        else:
    ...
    return (
     major_release_number, minor_release_number, patch_release_number, dev_release_number)
...
def chia_init(root_path: Path):
    if os.environ.get('CHIA_ROOT', None) is not None:
        print(f"warning, your CHIA_ROOT is set to {os.environ['CHIA_ROOT']}. Please unset the environment variable and run chia init again\nor manually migrate config.yaml")
    print(f"Chia directory {root_path}")
    if root_path.is_dir():
        if Path(root_path / 'config' / 'config.yaml').exists():
