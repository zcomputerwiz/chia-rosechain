...
# Embedded file name: chia\cmds\farm.py
...
@click.option('-hp',
  '--harvester-rpc-port',
  help='Set the port where the Harvester is hosting the RPC interfaceSee the rpc_port under harvester in config.yaml',
  type=int,
  default=None,
  show_default=True)
@click.option('-fp',
  ...
  default=None,
  show_default=True)
def summary_cmd(rpc_port: Optional[int], wallet_rpc_port: Optional[int], harvester_rpc_port: Optional[int], farmer_rpc_port: Optional[int]) -> None:
