...
# Embedded file name: chia\cmds\farm_funcs.py
from typing import Any, Dict, List, Optional
import aiohttp
import chia.cmds.units as units
...
async def get_average_block_time(rpc_port: Optional[int]) -> float:
    try:
        ...
        while past_curr is not None:
            if past_curr.height > 0:
                if not past_curr.is_transaction_block:
...
async def summary(rpc_port: Optional[int], wallet_rpc_port: Optional[int], harvester_rpc_port: Optional[int], farmer_rpc_port: Optional[int]) -> None:
    ...
    if blockchain_state is None:
        ...
        if blockchain_state['sync']['sync_mode']:
            print('Syncing')
        else:
    ...
    if all_harvesters is not None:
        ...
        def process_harvesters(harvester_peers_in):
            for harvester_peer_id, plots in harvester_peers_in.items():
                total_plot_size_harvester = sum(map(lambda x: x['file_size']
, plots['plots']))
