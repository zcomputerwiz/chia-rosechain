...
# Embedded file name: chia\cmds\wallet.py
...
@click.option('-f', '--fingerprint', help='Set the fingerprint to specify which wallet to use', type=int)
@click.option('-i', '--id', help='Id of the wallet to use', type=int, default=1, show_default=True, required=True)
@click.option('-a', '--amount', help='How much chia to send, in XCR', type=str, required=True)
@click.option('-m',
  '--fee',
  help='Set the fees for the transaction, in XCR',
  type=str,
  default='0',
  show_default=True,
  required=True)
@click.option('-t', '--address', help='Address to send the XCR', type=str, required=True)
@click.option('-o',
  '--override', help='Submits transaction without checking for unusual values', is_flag=True, default=False)
def send_cmd(wallet_rpc_port: Optional[int], fingerprint: int, id: int, amount: str, fee: str, address: str, override: bool) -> None:
...
@wallet_cmd.command('delete_unconfirmed_transactions',
