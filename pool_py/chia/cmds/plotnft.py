...
# Embedded file name: chia\cmds\plotnft.py
...
@plotnft_cmd.command('get_login_link',
  short_help='Create a login link for a pool. To get the launcher id, use plotnft show.')
...
def create_cmd(wallet_rpc_port: Optional[int], fingerprint: int, pool_url: str, state: str, yes: bool) -> None:
    ...
    if pool_url in (None, ''):
        if state.lower() == 'pool':
            ...
            return
    valid_initial_states = {'pool':'FARMING_TO_POOL', 
     'local':'SELF_POOLING'}
