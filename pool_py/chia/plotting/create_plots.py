...
# Embedded file name: chia\plotting\create_plots.py
import logging
...
def create_plots(args, root_path, use_datetime=True, test_private_keys: Optional[List]=None):
    ...
    if pool_public_key is not None:
        log.info(f"Creating {num} plots of size {args.size}, pool public key:  {bytes(pool_public_key).hex()} farmer public key: {bytes(farmer_public_key).hex()}")
    else:
        assert pool_contract_puzzle_hash is not None
        log.info(f"Creating {num} plots of size {args.size}, pool contract address:  {args.pool_contract_address} farmer public key: {bytes(farmer_public_key).hex()}")
    ...
    for i in range(num):
        if test_private_keys is not None:
            ...
            sk = test_private_keys[i]
        else:
            sk = AugSchemeMPL.key_gen(token_bytes(32))
        include_taproot = pool_contract_puzzle_hash is not None
        plot_public_key = ProofOfSpace.generate_plot_public_key(master_sk_to_local_sk(sk).get_g1(), farmer_public_key, include_taproot)
        if pool_public_key is not None:
            plot_id = ProofOfSpace.calculate_plot_id_pk(pool_public_key, plot_public_key)
            plot_memo = stream_plot_info_pk(pool_public_key, farmer_public_key, sk)
        else:
            assert pool_contract_puzzle_hash is not None
        ...
        if use_datetime:
            filename = f"plot-k{args.size}-{dt_string}-{plot_id}.plot"
        else:
            filename = f"plot-k{args.size}-{plot_id}.plot"
        full_path = args.final_dir / filename
        resolved_final_dir = str(Path(args.final_dir).resolve())
        plot_directories_list = config['harvester']['plot_directories']
        ...
        if not full_path.exists():
            ...
            plotter = DiskPlotter()
            plotter.create_plot_disk(str(args.tmp_dir), str(args.tmp2_dir), str(args.final_dir), filename, args.size, plot_memo, plot_id, args.buffer, args.buckets, args.stripe_size, args.num_threads, args.nobitfield)
