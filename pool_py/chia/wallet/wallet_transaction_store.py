...
# Embedded file name: chia\wallet\wallet_transaction_store.py
import time
...
class WalletTransactionStore:
    ...
    async def create(cls, db_wrapper: DBWrapper):
        ...
        await self.db_connection.execute('CREATE TABLE IF NOT EXISTS transaction_record( transaction_record blob, bundle_id text PRIMARY KEY, confirmed_at_height bigint, created_at_time bigint, to_puzzle_hash text, amount blob, fee_amount blob, confirmed int, sent int, wallet_id bigint, trade_id text, type int)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS tx_confirmed_index on transaction_record(confirmed_at_height)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS tx_created_index on transaction_record(created_at_time)')
        ...
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS tx_created_time on transaction_record(created_at_time)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS tx_type on transaction_record(type)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS tx_to_puzzle_hash on transaction_record(to_puzzle_hash)')
    ...
    async def add_transaction_record(self, record: TransactionRecord, in_transaction: bool) -> None:
        ...
        if record.confirmed:
            if record.name in unconfirmed_dict:
        ...
        try:
            try:
                cursor = await self.db_connection.execute('INSERT OR REPLACE INTO transaction_record VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (
                 bytes(record),
                 record.name,
    ...
    async def set_confirmed(self, tx_id: bytes32, height: uint32):
        ...
        """
        current = await self.get_transaction_record(tx_id)
        if current is None:
            return
        tx = TransactionRecord(confirmed_at_height=height,
    ...
    async def increment_sent(self, tx_id: bytes32, name: str, send_status: MempoolInclusionStatus, err: Optional[Err]) -> bool:
        """
        ...
        tx = TransactionRecord(confirmed_at_height=(current.confirmed_at_height),
          created_at_time=(current.created_at_time),
          to_puzzle_hash=(current.to_puzzle_hash),
          amount=(current.amount),
          fee_amount=(current.fee_amount),
    ...
    async def tx_reorged(self, record: TransactionRecord):
        ...
        """
        tx = TransactionRecord(confirmed_at_height=(uint32(0)),
    ...
    async def get_not_sent(self) -> List[TransactionRecord]:
        ...
        for row in rows:
            ...
            if record.name in self.tx_submitted:
                ...
                if time_submitted < current_time - 600:
    ...
    async def get_farming_rewards(self) -> List[TransactionRecord]:
        ...
        cursor = await self.db_connection.execute('SELECT * from transaction_record WHERE confirmed=? and (type=? or type=?)', (1, fee_int, pool_int))
        rows = await cursor.fetchall()
        await cursor.close()
        records = []
        for row in rows:
    ...
    async def get_transactions_between(self, wallet_id: int, start, end) -> List[TransactionRecord]:
        ...
        """
        limit = end - start
        cursor = await self.db_connection.execute(f"SELECT * from transaction_record where wallet_id=? and confirmed_at_height not in (select confirmed_at_height from transaction_record order by confirmed_at_height ASC LIMIT {start}) order by confirmed_at_height DESC LIMIT {limit}", (
         wallet_id,))
        rows = await cursor.fetchall()
        await cursor.close()
        records = []
    ...
    async def get_transaction_count_for_wallet(self, wallet_id) -> int:
        cursor = await self.db_connection.execute('SELECT COUNT(*) FROM transaction_record where wallet_id=?', (wallet_id,))
    ...
    async def get_all_transactions_for_wallet(self, wallet_id: int, type: int=None) -> List[TransactionRecord]:
        ...
        if type is None:
            ...
            cursor = await self.db_connection.execute('SELECT * from transaction_record where wallet_id=? and type=?', (
             wallet_id,
             type))
        rows = await cursor.fetchall()
        await cursor.close()
    ...
    async def get_transaction_above(self, height: int) -> List[TransactionRecord]:
        cursor = await self.db_connection.execute('SELECT * from transaction_record WHERE confirmed_at_height>?', (height,))
        rows = await cursor.fetchall()
        await cursor.close()
        records = []
        for row in rows:
    ...
    async def delete_unconfirmed_transactions(self, wallet_id: int):
        cursor = await self.db_connection.execute('DELETE FROM transaction_record WHERE confirmed=0 AND wallet_id=?', (wallet_id,))
        await cursor.close()