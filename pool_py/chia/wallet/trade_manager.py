...
# Embedded file name: chia\wallet\trade_manager.py
...
from chia.wallet.util.trade_utils import get_discrepancies_for_spend_bundle, get_output_amount_for_puzzle_and_solution, get_output_discrepancy_for_puzzle_and_solution
...
class TradeManager:
    ...
    @staticmethod
    async def create(wallet_state_manager: Any, db_wrapper: DBWrapper, name: str=None):
        self = TradeManager()
        if name:
    ...
    async def coins_of_interest_farmed(self, removals: List[Coin], additions: List[Coin], height: uint32):
        ...
        for coin in all_coins:
            ...
            if trade.status == TradeStatus.PENDING_CONFIRM.value:
    ...
    async def get_all_trades(self):
        all = await self.trade_store.get_all_trades()
        return all
            ...
            if wallet is None:
                ...
                if wallet.type() == WalletType.COLOURED_COIN.value:
                    tx = await wallet.generate_signed_transaction([
                     coin.amount],
                      [new_ph], 0, coins={coin}, ignore_max_send_amount=True)
                else:
                    tx = await wallet.generate_signed_transaction((coin.amount),
                      new_ph, 0, coins={coin}, ignore_max_send_amount=True)
    ...
    async def create_offer_for_ids(self, offer: Dict[(int, int)], file_name: str) -> Tuple[(bool, Optional[TradeRecord], Optional[str])]:
    ...
    async def _create_offer_for_ids(self, offer: Dict[(int, int)]) -> Tuple[(bool, Optional[TradeRecord], Optional[str])]:
        ...
        try:
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if amount > 0:
                        ...
                        zero_spend_bundle = await wallet.generate_zero_val_coin(False, to_exclude)
                        if spend_bundle is None:
            ...
            trade_offer = TradeRecord(confirmed_at_index=(uint32(0)),
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[(bool, Optional[TradeRecord], Optional[str])]:
        ...
        if chia_discrepancy is not None:
            chia_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_chia(chia_discrepancy, [])
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    zero_spend_bundle = await wallets[colour].generate_zero_val_coin(False, to_exclude)
                    if zero_spend_bundle is None:
                        return (False, None, 'Unable to generate zero value coin. Confirm that you have chia available')
            ...
            if my_cc_spends == set() or my_cc_spends is None:
                ...
                for coloured_coin in my_cc_spends:
                    ...
                    if not inner_puzzle is not None:
                ...
                inner_solution = self.wallet_state_manager.main_wallet.make_solution(primaries=[
                ...
                aggsig = AugSchemeMPL.aggregate(sigs)
                if spend_bundle is None:
                    spend_bundle = spend_bundle_for_spendable_ccs(CC_MOD, Program.from_bytes(bytes.fromhex(colour)), spendable_cc_list, innersol_list, [
                     aggsig])
                else:
                    new_spend_bundle = spend_bundle_for_spendable_ccs(CC_MOD, Program.from_bytes(bytes.fromhex(colour)), spendable_cc_list, innersol_list, [
                     aggsig])
                    spend_bundle = SpendBundle.aggregate([spend_bundle, new_spend_bundle])
                sigs = []
                aggsig = AugSchemeMPL.aggregate(sigs)
        ...
        tx_record = TransactionRecord(confirmed_at_height=(uint32(0)),
          ...
          name=(spend_bundle.name()))
        now = uint64(int(time.time()))
        trade_record = TradeRecord(confirmed_at_index=(uint32(0)),
