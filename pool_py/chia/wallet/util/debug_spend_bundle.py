...
# Embedded file name: chia\wallet\util\debug_spend_bundle.py
...
def debug_spend_bundle(spend_bundle, agg_sig_additional_data=bytes([3] * 32)) -> None:
            ...
            if error:
                ...
                if conditions is not None:
                    ...
                    if conditions and len(conditions) > 0:
                        ...
                        created_coin_announcements.extend(([coin_name] + _.vars for _ in conditions.get(ConditionOpcode.CREATE_COIN_ANNOUNCEMENT, [])))
                        asserted_coin_announcements.extend([_.vars[0].hex() for _ in conditions.get(ConditionOpcode.ASSERT_COIN_ANNOUNCEMENT, [])])
                        created_puzzle_announcements.extend(([puzzle_reveal.get_tree_hash()] + _.vars for _ in conditions.get(ConditionOpcode.CREATE_PUZZLE_ANNOUNCEMENT, [])))
                        asserted_puzzle_announcements.extend([_.vars[0].hex() for _ in conditions.get(ConditionOpcode.ASSERT_PUZZLE_ANNOUNCEMENT, [])])
                        print()
                    else:
    ...
    eor_coin_announcements = sorted(set((_[-1] for _ in created_coin_announcement_pairs)) ^ set(asserted_coin_announcements))
    ...
    eor_puzzle_announcements = sorted(set((_[-1] for _ in created_puzzle_announcement_pairs)) ^ set(asserted_puzzle_announcements))
    print()
    print()
...
def solution_for_pay_to_any(puzzle_hash_amount_pairs: Tuple[(bytes32, int)]) -> Program:
    output_conditions = [[ConditionOpcode.CREATE_COIN, puzzle_hash, amount] for puzzle_hash, amount in puzzle_hash_amount_pairs]
