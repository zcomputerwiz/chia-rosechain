...
# Embedded file name: chia\wallet\trading\trade_store.py
...
class TradeStore:
    ...
    async def create(cls, db_wrapper: DBWrapper, cache_size: uint32=uint32(600000)):
        ...
        await self.db_connection.execute('CREATE TABLE IF NOT EXISTS trade_records( trade_record blob, trade_id text PRIMARY KEY, status int, confirmed_at_index int, created_at_time bigint, sent int)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS trade_confirmed_index on trade_records(confirmed_at_index)')
    ...
    async def add_trade_record(self, record: TradeRecord, in_transaction) -> None:
        ...
        try:
            cursor = await self.db_connection.execute('INSERT OR REPLACE INTO trade_records VALUES(?, ?, ?, ?, ?, ?)', (
             bytes(record),
             record.trade_id.hex(),
             record.status,
    ...
    async def set_status(self, trade_id, status, in_transaction, index=uint32(0)):
        """
        Updates the status of the trade
        """
        current = await self.get_trade_record(trade_id)
        if current is None:
            return
        confirmed_at_index = current.confirmed_at_index
        if index != 0:
            confirmed_at_index = index
        tx = TradeRecord(confirmed_at_index=confirmed_at_index,
    ...
    async def increment_sent(self, id: bytes32, name: str, send_status: MempoolInclusionStatus, err: Optional[Err]) -> bool:
        """
        Updates trade sent count (Full Node has received spend_bundle and sent ack).
        """
        current = await self.get_trade_record(id)
        if current is None:
            return False
        ...
        tx = TradeRecord(confirmed_at_index=(current.confirmed_at_index),
          accepted_at_time=(current.accepted_at_time),
          created_at_time=(current.created_at_time),
          my_offer=(current.my_offer),
    ...
    async def get_not_sent(self) -> List[TradeRecord]:
        ...
        """
        cursor = await self.db_connection.execute('SELECT * from trade_records WHERE sent<? and confirmed=?', (4,
                                                                                                               0))
        rows = await cursor.fetchall()
        await cursor.close()
        records = []
    ...
    async def rollback_to_block(self, block_index):
        cursor = await self.db_connection.execute('DELETE FROM trade_records WHERE confirmed_at_index>?', (block_index,))
        await cursor.close()
        await self.db_connection.commit()