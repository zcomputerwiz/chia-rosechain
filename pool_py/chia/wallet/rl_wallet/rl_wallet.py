...
# Embedded file name: chia\wallet\rl_wallet\rl_wallet.py
...
from chia.wallet.rl_wallet.rl_wallet_puzzles import make_clawback_solution, rl_make_aggregation_puzzle, rl_make_aggregation_solution, rl_make_solution_mode_2, rl_puzzle_for_pk, solution_for_rl
...
class RLWallet:
    ...
    async def create_rl_admin(wallet_state_manager: Any):
        ...
        wallet_info = await wallet_state_manager.user_store.create_wallet('RL Admin', WalletType.RATE_LIMITED, info_as_string)
        if wallet_info is None:
            raise Exception('wallet_info is None')
        await wallet_state_manager.puzzle_store.add_derivation_paths([
         DerivationRecord(unused, bytes32(token_bytes(32)), pubkey, WalletType.RATE_LIMITED, wallet_info.id)])
    ...
    async def create_rl_user(wallet_state_manager: Any):
        async with wallet_state_manager.puzzle_store.lock:
            ...
            pubkey = master_sk_to_wallet_sk(private_key, unused).get_g1()
    ...
    async def admin_create_coin(self, interval, limit, user_pubkey, amount, fee):
        ...
        rl_puzzle = rl_puzzle_for_pk(pubkey=user_pubkey_bytes,
          rate_amount=limit,
          interval_time=interval,
          origin_id=origin_id,
          clawback_pk=(self.rl_info.admin_pubkey))
        rl_puzzle_hash = rl_puzzle.get_tree_hash()
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(G1Element.from_bytes(self.rl_info.admin_pubkey))
        assert index is not None
        record = DerivationRecord(index, rl_puzzle_hash, G1Element.from_bytes(self.rl_info.admin_pubkey), WalletType.RATE_LIMITED, self.id())
        ...
        new_rl_info = RLInfo('admin', self.rl_info.admin_pubkey, user_pubkey_bytes, limit, interval, origin, origin.name(), rl_puzzle_hash, True)
    ...
    async def set_user_info(self, interval, limit, origin_parent_id, origin_puzzle_hash, origin_amount, admin_pubkey):
        ...
        rl_puzzle = rl_puzzle_for_pk(pubkey=(self.rl_info.user_pubkey),
          rate_amount=limit,
          interval_time=interval,
          origin_id=(origin.name()),
          clawback_pk=admin_pubkey_bytes)
        rl_puzzle_hash = rl_puzzle.get_tree_hash()
        new_rl_info = RLInfo('user', admin_pubkey_bytes, self.rl_info.user_pubkey, limit, interval, origin, origin.name(), rl_puzzle_hash, True)
        rl_puzzle_hash = rl_puzzle.get_tree_hash()
        if await self.wallet_state_manager.puzzle_store.puzzle_hash_exists(rl_puzzle_hash):
            raise ValueError('Cannot create multiple Rate Limited wallets under the same keys. This will change in a future release.')
        user_pubkey = G1Element.from_bytes(self.rl_info.user_pubkey)
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(user_pubkey)
        assert index is not None
        record = DerivationRecord(index, rl_puzzle_hash, user_pubkey, WalletType.RATE_LIMITED, self.id())
        aggregation_puzzlehash = self.rl_get_aggregation_puzzlehash(new_rl_info.rl_puzzle_hash)
        record2 = DerivationRecord(index + 1, aggregation_puzzlehash, user_pubkey, WalletType.RATE_LIMITED, self.id())
        await self.wallet_state_manager.puzzle_store.add_derivation_paths([record, record2])
        self.wallet_state_manager.set_coin_with_puzzlehash_created_callback(aggregation_puzzlehash, self.aggregate_this_coin)
        data_str = json.dumps(new_rl_info.to_json_dict())
        new_wallet_info = WalletInfo(self.id(), self.wallet_info.name, self.type(), data_str)
        await self.wallet_state_manager.user_store.update_wallet(new_wallet_info, False)
        await self.wallet_state_manager.add_new_wallet(self, self.id())
    ...
    async def aggregate_this_coin(self, coin: Coin):
        spend_bundle = await self.rl_generate_signed_aggregation_transaction(self.rl_info, coin, await self._get_rl_parent(), await self._get_rl_coin())
    ...
    async def rl_available_balance(self) -> uint64:
        ...
        unlocked = int((height - self.rl_coin_record.confirmed_block_height) / self.rl_info.interval * int(self.rl_info.limit))
    ...
    def get_new_puzzle(self) -> Program:
        if not self.rl_info.limit is None:
            if self.rl_info.interval is None or self.rl_info.user_pubkey is None or self.rl_info.admin_pubkey is None or self.rl_info.rl_origin_id is None:
                raise ValueError('One or more of the RL info fields is None')
            return rl_puzzle_for_pk(pubkey=(self.rl_info.user_pubkey),
    ...
    async def get_keys(self, puzzle_hash: bytes32) -> Tuple[(G1Element, PrivateKey)]:
        ...
        """
        index_for_puzzlehash = await self.wallet_state_manager.puzzle_store.index_for_puzzle_hash_and_wallet(puzzle_hash, self.id())
    ...
    async def get_keys_pk(self, clawback_pubkey: bytes):
        ...
        """
        index_for_pubkey = await self.wallet_state_manager.puzzle_store.index_for_pubkey(G1Element.from_bytes(clawback_pubkey))
    ...
    async def _get_rl_coin(self) -> Optional[Coin]:
        rl_coins = await self.wallet_state_manager.coin_store.get_coin_records_by_puzzle_hash(self.rl_info.rl_puzzle_hash)
    ...
    async def rl_generate_unsigned_transaction(self, to_puzzlehash, amount, fee) -> List[CoinSpend]:
        ...
        rl_parent = await self._get_rl_parent()
        if rl_parent is None:
        ...
        assert self.rl_info.admin_pubkey is not None
        puzzle = rl_puzzle_for_pk(bytes(pubkey), self.rl_info.limit, self.rl_info.interval, self.rl_info.rl_origin_id, self.rl_info.admin_pubkey)
        solution = solution_for_rl(coin.parent_coin_info, puzzle_hash, coin.amount, to_puzzlehash, amount, rl_parent.parent_coin_info, rl_parent.amount, self.rl_info.interval, self.rl_info.limit, fee)
    ...
    async def generate_signed_transaction(self, amount, to_puzzle_hash, fee: uint64=uint64(0)) -> TransactionRecord:
        ...
        return TransactionRecord(confirmed_at_height=(uint32(0)),
          created_at_time=(uint64(int(time.time()))),
    ...
    def generate_unsigned_clawback_transaction(self, clawback_coin: Coin, clawback_puzzle_hash: bytes32, fee):
        if not self.rl_info.limit is None:
            if self.rl_info.interval is None or self.rl_info.user_pubkey is None or self.rl_info.admin_pubkey is None:
            ...
            puzzle = rl_puzzle_for_pk(self.rl_info.user_pubkey, self.rl_info.limit, self.rl_info.interval, self.rl_info.rl_origin.name(), self.rl_info.admin_pubkey)
    ...
    async def rl_generate_signed_aggregation_transaction(self, rl_info, consolidating_coin, rl_parent, rl_coin):
        if not rl_info.limit is None:
            if rl_info.interval is None or rl_info.user_pubkey is None or rl_info.admin_pubkey is None:
            ...
            puzzle = rl_puzzle_for_pk(rl_info.user_pubkey, rl_info.limit, rl_info.interval, rl_info.rl_origin_id, rl_info.admin_pubkey)
            solution = rl_make_solution_mode_2(rl_coin.puzzle_hash, consolidating_coin.parent_coin_info, consolidating_coin.puzzle_hash, consolidating_coin.amount, rl_coin.parent_coin_info, rl_coin.amount, rl_parent.amount, rl_parent.parent_coin_info)
            ...
            solution = rl_make_aggregation_solution(consolidating_coin.name(), self.rl_coin_record.coin.parent_coin_info, self.rl_coin_record.coin.amount)
