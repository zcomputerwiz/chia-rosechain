...
# Embedded file name: chia\wallet\rl_wallet\rl_wallet_puzzles.py
import math
...
def rl_puzzle_for_pk(pubkey, rate_amount, interval_time, origin_id, clawback_pk):
    """
...
def rl_make_solution_mode_2(my_puzzle_hash, consolidating_primary_input, consolidating_coin_puzzle_hash, outgoing_amount, my_primary_input, incoming_amount, parent_amount, my_parent_parent_id):
    ...
    sol = sexp(AGGREGATION_MODE, '0x' + my_puzzle_hash, '0x' + consolidating_primary_input, '0x' + consolidating_coin_puzzle_hash, outgoing_amount, '0x' + primary_input, incoming_amount, parent_amount, '0x' + str(my_parent_parent_id))
    return Program.to(binutils.assemble(sol))


def solution_for_rl(my_parent_id, my_puzzlehash, my_amount, out_puzzlehash, out_amount, my_parent_parent_id, parent_amount, interval, limit, fee):
    """
    ...
    solution = sexp(RATE_LIMITED_MODE, '0x' + my_parent_id.hex(), '0x' + my_puzzlehash.hex(), my_amount, '0x' + out_puzzlehash.hex(), out_amount, min_block_count, '0x' + my_parent_parent_id.hex(), parent_amount, fee)
    return Program.to(binutils.assemble(solution))
