...
# Embedded file name: chia\wallet\cc_wallet\cc_utils.py
import dataclasses
...
from chia.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker, lineage_proof_for_coin, lineage_proof_for_genesis, lineage_proof_for_zero
...
def cc_puzzle_hash_for_inner_puzzle_hash(mod_code, genesis_coin_checker, inner_puzzle_hash) -> bytes32:
    ...
    return mod_code.curry(mod_code.get_tree_hash(), gcc_hash, inner_puzzle_hash).get_tree_hash(gcc_hash, inner_puzzle_hash)
...
def coin_spend_for_lock_coin(prev_coin, subtotal, coin):
...
def spend_bundle_for_spendable_ccs(mod_code: Program, genesis_coin_checker: Program, spendable_cc_list: List[SpendableCC], inner_solutions: List[Program], sigs: Optional[List[G2Element]]=[]) -> SpendBundle:
    """
    ...
    for cc_spend_info, inner_solution in zip(spendable_cc_list, inner_solutions):
        error, conditions, cost = conditions_dict_for_solution(cc_spend_info.inner_puzzle, inner_solution, INFINITE_COST)
        total = 0
        if conditions:
