...
# Embedded file name: chia\wallet\cc_wallet\cc_wallet.py
...
from chia.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, cc_puzzle_for_inner_puzzle, cc_puzzle_hash_for_inner_puzzle_hash, get_lineage_proof_from_coin_and_puz, spend_bundle_for_spendable_ccs, uncurry_cc
from chia.wallet.derivation_record import DerivationRecord
from chia.wallet.puzzles.genesis_by_coin_id_with_0 import create_genesis_or_zero_coin_checker, genesis_coin_id_for_genesis_coin_checker, lineage_proof_for_genesis
from chia.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import DEFAULT_HIDDEN_PUZZLE_HASH, calculate_synthetic_secret_key
...
class CCWallet:
    ...
    @staticmethod
    async def create_new_cc(wallet_state_manager: 'Any', wallet: 'Wallet', amount: 'uint64'):
        self = CCWallet()
        ...
        self.wallet_info = await wallet_state_manager.user_store.create_wallet('CC Wallet', WalletType.COLOURED_COIN, info_as_string)
        ...
        non_ephemeral_spends = spend_bundle.not_ephemeral_additions()
        cc_coin = None
        ...
        regular_record = TransactionRecord(confirmed_at_height=(uint32(0)),
          ...
          name=(token_bytes()))
        cc_record = TransactionRecord(confirmed_at_height=(uint32(0)),
          created_at_time=(uint64(int(time.time()))),
          to_puzzle_hash=(cc_coin.puzzle_hash),
          amount=(uint64(cc_coin.amount)),
          fee_amount=(uint64(0)),
    ...
    @staticmethod
    async def create_wallet_for_cc(wallet_state_manager, wallet, genesis_checker_hex):
        self = CCWallet()
        self.cost_of_single_tx = None
        self.base_puzzle_program = None
        self.base_inner_puzzle_hash = None
    ...
    @staticmethod
    async def create(wallet_state_manager, wallet, wallet_info):
        self = CCWallet()
        self.log = logging.getLogger(__name__)
        self.cost_of_single_tx = None
        self.wallet_state_manager = wallet_state_manager
    ...
    async def get_max_send_amount(self, records=None):
        spendable = list(await self.wallet_state_manager.get_spendable_coins_for_wallet(self.id(), records))
        if len(spendable) == 0:
            return 0
        ...
        if self.cost_of_single_tx is None:
            ...
            tx = await self.generate_signed_transaction([
             coin.amount],
              [coin.puzzle_hash], coins={coin}, ignore_max_send_amount=True)
            program = simple_solution_generator(tx.spend_bundle)
            result = get_name_puzzle_conditions(program,
              (self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM),
              cost_per_byte=(self.wallet_state_manager.constants.COST_PER_BYTE),
              safe_mode=True)
            cost_result = calculate_cost_of_program(program.program, result, self.wallet_state_manager.constants.COST_PER_BYTE)
    ...
    async def coin_added(self, coin: 'Coin', height: 'uint32'):
        ...
        if search_for_parent:
            data = {'data': {'action_data': {'api_name':'request_puzzle_solution', 
                                      'height':height, 
                                      'coin_name':coin.parent_coin_info, 
                                      'received_coin':coin.name()}}}
            data_str = dict_to_json_str(data)
            await self.wallet_state_manager.create_action(name='request_puzzle_solution',
              wallet_id=(self.id()),
    ...
    async def puzzle_solution_received(self, response: 'PuzzleSolutionResponse', action_id: 'int'):
        ...
        block = await self.wallet_state_manager.blockchain.block_store.get_header_block_record(header_hash)
        if block is None:
            return
    ...
    def puzzle_for_pk(self, pubkey) -> 'Program':
        ...
        cc_puzzle = cc_puzzle_for_inner_puzzle(CC_MOD, self.cc_info.my_genesis_checker, inner_puzzle)
    ...
    async def generate_zero_val_coin(self, send=True, exclude: 'List[Coin]'=None) -> 'SpendBundle':
        ...
        cc_puzzle_hash = cc_puzzle_hash_for_inner_puzzle_hash(CC_MOD, self.cc_info.my_genesis_checker, cc_inner)
        tx = await self.standard_wallet.generate_signed_transaction(uint64(0), cc_puzzle_hash, uint64(0), origin_id, coins)
    ...
    async def get_cc_spendable_coins(self, records=None) -> 'List[WalletCoinRecord]':
        result = []
        record_list = await self.wallet_state_manager.get_spendable_coins_for_wallet(self.id(), records)
        for record in record_list:
            lineage = await self.get_lineage_proof_for_coin(record.coin)
            if lineage is not None:
    ...
    async def select_coins(self, amount: 'uint64') -> 'Set[Coin]':
        ...
        spendable = await self.get_cc_spendable_coins()
        sum = 0
        used_coins = set()
        spendable.sort(key=(lambda r: r.confirmed_block_height
))
        unconfirmed_removals = await self.wallet_state_manager.unconfirmed_removals_for_wallet(self.id())
    ...
    async def get_sigs(self, innerpuz, innersol, coin_name):
        puzzle_hash = innerpuz.get_tree_hash()
        pubkey, private = await self.wallet_state_manager.get_keys(puzzle_hash)
        synthetic_secret_key = calculate_synthetic_secret_key(private, DEFAULT_HIDDEN_PUZZLE_HASH)
        sigs = []
        error, conditions, cost = conditions_dict_for_solution(innerpuz, innersol, self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM)
        if conditions is not None:
            for _, msg in pkm_pairs_for_conditions_dict(conditions, coin_name, self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA):
    ...
    async def inner_puzzle_for_cc_puzhash(self, cc_hash: 'bytes32') -> 'Program':
        record = await self.wallet_state_manager.puzzle_store.get_derivation_record_for_puzzle_hash(cc_hash.hex())
        inner_puzzle = self.standard_wallet.puzzle_for_pk(bytes(record.pubkey))
        return inner_puzzle
    ...
    async def generate_signed_transaction(self, amounts, puzzle_hashes, fee=uint64(0), origin_id=None, coins=None, ignore_max_send_amount=False):
        ...
        if coins is None:
            selected_coins = await self.select_coins(uint64(total_outgoing))
        else:
        ...
        for coin in selected_coins:
            ...
            if not lineage_proof is not None:
    ...
    async def add_lineage(self, name: 'bytes32', lineage: 'Optional[Program]', in_transaction=False):
        ...
        cc_info = CCInfo(self.cc_info.my_genesis_checker, current_list)
    ...
    async def generate_new_coloured_coin(self, amount: 'uint64') -> 'SpendBundle':
        ...
        tx_record = await self.standard_wallet.generate_signed_transaction(amount, minted_cc_puzzle_hash, uint64(0), origin_id, coins)
        assert tx_record.spend_bundle is not None
        lineage_proof = lineage_proof_for_genesis(origin)
        lineage_proofs = [(origin_id, lineage_proof)]
        cc_info = CCInfo(genesis_coin_checker, lineage_proofs)
    ...
    async def create_spend_bundle_relative_amount(self, cc_amount, zero_coin: 'Coin'=None) -> 'Optional[SpendBundle]':
        ...
        for coin in cc_spends:
            ...
            innerpuz = await self.inner_puzzle_for_cc_puzhash(coin.puzzle_hash)
