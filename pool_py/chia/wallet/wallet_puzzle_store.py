...
# Embedded file name: chia\wallet\wallet_puzzle_store.py
...
class WalletPuzzleStore:
    ...
    async def create(cls, db_wrapper: DBWrapper, cache_size: uint32=uint32(600000)):
        ...
        await self.db_connection.execute('CREATE TABLE IF NOT EXISTS derivation_paths(derivation_index int, pubkey text, puzzle_hash text PRIMARY_KEY, wallet_type int, wallet_id int, used tinyint)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS derivation_index_index on derivation_paths(derivation_index)')
    ...
    async def get_derivation_record(self, index: uint32, wallet_id: uint32) -> Optional[DerivationRecord]:
        ...
        """
        cursor = await self.db_connection.execute('SELECT * FROM derivation_paths WHERE derivation_index=? and wallet_id=?;', (
         index,
         wallet_id))
    ...
    async def get_derivation_record_for_puzzle_hash(self, puzzle_hash: str) -> Optional[DerivationRecord]:
        ...
        Returns the derivation record by index and wallet id.
        """
        cursor = await self.db_connection.execute('SELECT * FROM derivation_paths WHERE puzzle_hash=?;', (
         puzzle_hash,))
        row = await cursor.fetchone()
        await cursor.close()
        if row is not None:
    ...
    async def set_used_up_to(self, index: uint32, in_transaction=False) -> None:
        ...
        if not in_transaction:
            await self.db_wrapper.lock.acquire()
        try:
            cursor = await self.db_connection.execute('UPDATE derivation_paths SET used=1 WHERE derivation_index<=?', (
             index,))
            await cursor.close()
        finally:
            if not in_transaction:
    ...
    async def index_for_puzzle_hash_and_wallet(self, puzzle_hash: bytes32, wallet_id: uint32) -> Optional[uint32]:
        ...
        Returns None if not present.
        """
        cursor = await self.db_connection.execute('SELECT * from derivation_paths WHERE puzzle_hash=? and wallet_id=?;', (
         puzzle_hash.hex(),
         wallet_id))
        row = await cursor.fetchone()
        await cursor.close()
