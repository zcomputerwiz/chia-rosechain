...
# Embedded file name: chia\wallet\wallet_coin_store.py
...
class WalletCoinStore:
    ...
    async def create(cls, wrapper: DBWrapper):
        ...
        await self.db_connection.execute('CREATE TABLE IF NOT EXISTS coin_record(coin_name text PRIMARY KEY, confirmed_height bigint, spent_height bigint, spent int, coinbase int, puzzle_hash text, coin_parent text, amount blob, wallet_type int, wallet_id int)')
        await self.db_connection.execute('CREATE INDEX IF NOT EXISTS coin_confirmed_height on coin_record(confirmed_height)')
    ...
    async def add_coin_record(self, record: WalletCoinRecord) -> None:
        ...
        if record.wallet_id in self.unspent_coin_wallet_cache:
            if record.spent:
                if name in self.unspent_coin_wallet_cache[record.wallet_id]:
                    self.unspent_coin_wallet_cache[record.wallet_id].pop(name)
            self.unspent_coin_wallet_cache[record.wallet_id][name] = record.spent or record
        else:
            if not record.spent:
                self.unspent_coin_wallet_cache[record.wallet_id] = {}
                self.unspent_coin_wallet_cache[record.wallet_id][name] = record
        cursor = await self.db_connection.execute('INSERT OR REPLACE INTO coin_record VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (
         name.hex(),
    ...
    async def set_spent(self, coin_name: bytes32, height: uint32) -> WalletCoinRecord:
        ...
        spent = WalletCoinRecord(current.coin, current.confirmed_block_height, height, True, current.coinbase, current.wallet_type, current.wallet_id)
        await self.add_coin_record(spent)
        return spent

    def coin_record_from_row(self, row: sqlite3.Row) -> WalletCoinRecord:
        coin = Coin(bytes32(bytes.fromhex(row[6])), bytes32(bytes.fromhex(row[5])), uint64.from_bytes(row[7]))
        return WalletCoinRecord(coin, uint32(row[1]), uint32(row[2]), bool(row[3]), bool(row[4]), WalletType(row[8]), row[9])
    ...
    async def get_unspent_coins_at_height(self, height: Optional[uint32]=None) -> Set[WalletCoinRecord]:
        ...
        all_unspent = set()
        for name, coin_record in self.coin_record_cache.items():
            if not coin_record.spent is False:
                if coin_record.spent_block_height> height >= coin_record.confirmed_block_height:
                    all_unspent.add(coin_record)
    ...
    async def get_unspent_coins_for_wallet(self, wallet_id: int) -> Set[WalletCoinRecord]:
        ...
        if wallet_id in self.unspent_coin_wallet_cache:
            wallet_coins = self.unspent_coin_wallet_cache[wallet_id]
    ...
    async def rollback_to_block(self, height: int):
        ...
        for coin_name, coin_record in self.coin_record_cache.items():
            if coin_record.spent_block_height > height:
                new_record = WalletCoinRecord(coin_record.coin, coin_record.confirmed_block_height, uint32(0), False, coin_record.coinbase, coin_record.wallet_type, coin_record.wallet_id)
        ...
        await c1.close()
        c2 = await self.db_connection.execute('UPDATE coin_record SET spent_height = 0, spent = 0 WHERE spent_height>?', (
         height,))
        await c2.close()