...
# Embedded file name: chia\wallet\wallet_block_store.py
...
class WalletBlockStore:
    ...
    async def create(cls, db_wrapper: DBWrapper):
        ...
        await self.db.execute('CREATE TABLE IF NOT EXISTS header_blocks(header_hash text PRIMARY KEY, height int, timestamp int, block blob)')
        ...
        await self.db.execute('CREATE TABLE IF NOT EXISTS block_records(header_hash text PRIMARY KEY, prev_hash text, height bigint, weight bigint, total_iters text,block blob, sub_epoch_summary blob, is_peak tinyint)')
        await self.db.execute('CREATE TABLE IF NOT EXISTS additional_coin_spends(header_hash text PRIMARY KEY, spends_list_blob blob)')
    ...
    async def add_block_record(self, header_block_record: HeaderBlockRecord, block_record: BlockRecord, additional_coin_spends: List[CoinSpend]):
        """
        ...
        if header_block_record.header.foliage_transaction_block is not None:
            ...
            timestamp = uint64(0)
        cursor = await self.db.execute('INSERT OR REPLACE INTO header_blocks VALUES(?, ?, ?, ?)', (
        ...
        cursor_2 = await self.db.execute('INSERT OR REPLACE INTO block_records VALUES(?, ?, ?, ?, ?, ?, ?,?)', (
         ...
         header_block_record.header.weight.to_bytes(16, 'big', signed=False).hex(),
         header_block_record.header.total_iters.to_bytes(16, 'big', signed=False).hex(),
         bytes(block_record),
        ...
        if len(additional_coin_spends) > 0:
            blob = bytes(AdditionalCoinSpends(additional_coin_spends))
            cursor_3 = await self.db.execute('INSERT OR REPLACE INTO additional_coin_spends VALUES(?, ?)', (
    ...
    async def get_additional_coin_spends(self, header_hash: bytes32) -> Optional[List[CoinSpend]]:
        cursor = await self.db.execute('SELECT spends_list_blob from additional_coin_spends WHERE header_hash=?', (header_hash.hex(),))
        row = await cursor.fetchone()
        await cursor.close()
        if row is not None:
            coin_spends = AdditionalCoinSpends.from_bytes(row[0])
    ...
    async def get_block_records(self) -> Tuple[(Dict[(bytes32, BlockRecord)], Optional[bytes32])]:
        """
    ...
    async def get_block_records_close_to_peak(self, blocks_n: int) -> Tuple[(Dict[(bytes32, BlockRecord)], Optional[bytes32])]:
        ...
        peak = bytes32(bytes.fromhex(header_hash_bytes))
