...
# Embedded file name: chia\wallet\wallet_user_store.py
...
class WalletUserStore:
    ...
    async def create(cls, db_wrapper: DBWrapper):
        ...
        await self.db_connection.execute('CREATE TABLE IF NOT EXISTS users_wallets(id INTEGER PRIMARY KEY AUTOINCREMENT, name text, wallet_type int, data text)')
    ...
    async def create_wallet(self, name: str, wallet_type: int, data: str, id: Optional[int]=None, in_transaction=False) -> Optional[WalletInfo]:
        if not in_transaction:
            await self.db_wrapper.lock.acquire()
        try:
            cursor = await self.db_connection.execute('INSERT INTO users_wallets VALUES(?, ?, ?, ?)', (
    ...
    async def update_wallet(self, wallet_info: WalletInfo, in_transaction):
        if not in_transaction:
            await self.db_wrapper.lock.acquire()
        try:
            cursor = await self.db_connection.execute('INSERT or REPLACE INTO users_wallets VALUES(?, ?, ?, ?)', (
             wallet_info.id,
             wallet_info.name,
             wallet_info.type,
             wallet_info.data))
