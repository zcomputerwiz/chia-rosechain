...
# Embedded file name: chia\wallet\wallet.py
import logging, time
...
from chia.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import DEFAULT_HIDDEN_PUZZLE_HASH, calculate_synthetic_secret_key, puzzle_for_pk, solution_for_conditions
...
class Wallet:
    ...
    @staticmethod
    async def create(wallet_state_manager: Any, info: WalletInfo, name: str=None):
        self = Wallet()
    ...
    async def get_max_send_amount(self, records=None):
        ...
        if self.cost_of_single_tx is None:
            ...
            tx = await self.generate_signed_transaction((coin.amount),
        ...
        max_cost = self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM / 5
        current_cost = 0
        total_amount = 0
        total_coin_count = 0
    ...
    async def get_spendable_balance(self, unspent_records=None) -> uint128:
        spendable = await self.wallet_state_manager.get_confirmed_spendable_balance_for_wallet(self.id(), unspent_records)
        return spendable
    ...
    async def select_coins(self, amount, exclude: List[Coin]=None) -> Set[Coin]:
        ...
        if amount > spendable_amount:
            error_msg = f"Can't select amount higher than our spendable balance.  Amount: {amount}, spendable:  {spendable_amount}"
            self.log.warning(error_msg)
            raise ValueError(error_msg)
        self.log.info(f"About to select coins for amount {amount}")
        unspent = list(await self.wallet_state_manager.get_spendable_coins_for_wallet(self.id()))
        sum_value = 0
        used_coins = set()
    ...
    async def _generate_unsigned_transaction(self, amount: uint64, newpuzzlehash: bytes32, fee: uint64=uint64(0), origin_id: bytes32=None, coins: Set[Coin]=None, primaries_input: Optional[List[Dict[(str, Any)]]]=None, ignore_max_send_amount: bool=False, announcements_to_consume: Set[Announcement]=None) -> List[CoinSpend]:
        """
        ...
        for coin in coins:
            ...
            spends.append(CoinSpend(coin, SerializedProgram.from_bytes(bytes(puzzle)), SerializedProgram.from_bytes(bytes(solution))))

        self.log.info(f"Spends is {spends}")
        return spends

    async def sign_transaction(self, coin_spends: List[CoinSpend]) -> SpendBundle:
        return await sign_coin_spends(coin_spends, self.secret_key_store.secret_key_for_public_key, self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA, self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM)

    async def generate_signed_transaction(self, amount: uint64, puzzle_hash: bytes32, fee: uint64=uint64(0), origin_id: bytes32=None, coins: Set[Coin]=None, primaries: Optional[List[Dict[(str, bytes32)]]]=None, ignore_max_send_amount: bool=False, announcements_to_consume: Set[Announcement]=None) -> TransactionRecord:
        """
        ...
        transaction = await self._generate_unsigned_transaction(amount, puzzle_hash, fee, origin_id, coins, primaries, ignore_max_send_amount, announcements_to_consume)
        assert len(transaction) > 0
        self.log.info('About to sign a transaction')
        await self.hack_populate_secret_keys_for_coin_spends(transaction)
        spend_bundle = await sign_coin_spends(transaction, self.secret_key_store.secret_key_for_public_key, self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA, self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM)
    ...
    async def create_spend_bundle_relative_chia(self, chia_amount: int, exclude: List[Coin]) -> SpendBundle:
        ...
        spend_bundle = await sign_coin_spends(list_of_solutions, self.secret_key_store.secret_key_for_public_key, self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA, self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM)
        return spend_bundle