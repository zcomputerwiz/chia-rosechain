...
# Embedded file name: chia\wallet\puzzles\puzzle_utils.py
...
def make_assert_puzzle_announcement(announcement_hash):
    return [
     ConditionOpcode.ASSERT_PUZZLE_ANNOUNCEMENT, announcement_hash]
...
def make_create_puzzle_announcement(message):
    return [
     ConditionOpcode.CREATE_PUZZLE_ANNOUNCEMENT, message]


def make_assert_my_parent_id(parent_id):
    return [
     ConditionOpcode.ASSERT_MY_PARENT_ID, parent_id]


def make_assert_my_puzzlehash(puzzlehash):
    return [
     ConditionOpcode.ASSERT_MY_PUZZLEHASH, puzzlehash]


def make_assert_my_amount(amount):
    return [
     ConditionOpcode.ASSERT_MY_AMOUNT, amount]