...
# Embedded file name: chia\wallet\wallet_pool_store.py
import logging
...
class WalletPoolStore:
    ...
    async def create(cls, wrapper: DBWrapper):
        ...
        await self.db_connection.execute('CREATE TABLE IF NOT EXISTS pool_state_transitions(transition_index integer, wallet_id integer, height bigint, coin_spend blob, PRIMARY KEY(transition_index, wallet_id))')
    ...
    async def add_spend(self, wallet_id, spend, height):
        """
        ...
        all_state_transitions = self.get_spends_for_wallet(wallet_id)
        if (
         height, spend) in all_state_transitions:
            return
        ...
        cursor = await self.db_connection.execute('INSERT OR REPLACE INTO pool_state_transitions VALUES (?, ?, ?, ?)', (
    ...
    async def rebuild_cache(self) -> None:
        ...
        for row in rows:
            ...
            coin_spend = CoinSpend.from_bytes(coin_spend_bytes)
            if wallet_id not in self._state_transitions_cache:
                self._state_transitions_cache[wallet_id] = []
            else:
    ...
    async def rollback(self, height: int, wallet_id_arg: int) -> None:
        ...
        cursor = await self.db_connection.execute('DELETE FROM pool_state_transitions WHERE height>? AND wallet_id=?', (height, wallet_id_arg))
        await cursor.close()