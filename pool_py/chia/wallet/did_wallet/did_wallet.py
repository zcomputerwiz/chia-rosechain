...
# Embedded file name: chia\wallet\did_wallet\did_wallet.py
...
class DIDWallet:
    ...
    @staticmethod
    async def create_new_did_wallet(wallet_state_manager, wallet, amount, backups_ids=[], num_of_backup_ids_needed=None, name=None):
        """
        ...
        self.wallet_info = await wallet_state_manager.user_store.create_wallet('DID Wallet', WalletType.DISTRIBUTED_ID.value, info_as_string)
        ...
        did_puzzle_hash = did_wallet_puzzles.create_fullpuz(self.did_info.current_inner, self.did_info.origin_coin.name()).get_tree_hash()
        did_record = TransactionRecord(confirmed_at_height=(uint32(0)),
          ...
          name=(token_bytes()))
        regular_record = TransactionRecord(confirmed_at_height=(uint32(0)),
          created_at_time=(uint64(int(time.time()))),
          to_puzzle_hash=did_puzzle_hash,
          amount=(uint64(amount)),
          fee_amount=(uint64(0)),
    ...
    @staticmethod
    async def create_new_did_wallet_from_recovery(wallet_state_manager, wallet, filename, name=None):
        self = DIDWallet()
        self.base_puzzle_program = None
        self.base_inner_puzzle_hash = None
        self.standard_wallet = wallet
    ...
    @staticmethod
    async def create(wallet_state_manager, wallet, wallet_info, name=None):
        self = DIDWallet()
        self.log = logging.getLogger(name if name else __name__)
        self.wallet_state_manager = wallet_state_manager
    ...
    async def select_coins(self, amount, exclude: List[Coin]=None) -> Optional[Set[Coin]]:
        ...
        unspent = list(await self.wallet_state_manager.get_spendable_coins_for_wallet(self.wallet_info.id))
        sum_value = 0
        used_coins = set()
        ...
        if sum_value < amount:
            raise ValueError("Can't make this transaction at the moment. Waiting for the change from the previous transaction.")
    ...
    async def coin_added(self, coin: Coin, _: uint32):
        ...
        new_info = DIDInfo(self.did_info.origin_coin, self.did_info.backup_ids, self.did_info.num_of_backup_ids_needed, self.did_info.parent_info, inner_puzzle, None, None, None)
        await self.save_info(new_info, True)
        future_parent = LineageProof(coin.parent_coin_info, inner_puzzle.get_tree_hash(), coin.amount)
    ...
    def create_backup(self, filename: str):
        ...
        try:
            ...
            output_str = output_str + f":{bytes(self.did_info.current_inner).hex()}:{self.did_info.num_of_backup_ids_needed}"
            f.write(output_str)
            f.close()
    ...
    async def load_backup(self, filename: str):
        try:
            ...
            did_info = DIDInfo(origin, backup_ids, num_of_backup_ids_needed, self.did_info.parent_info, innerpuz, None, None, None)
            ...
            sub_height, header_hash = await self.wallet_state_manager.search_blockrecords_for_puzzlehash(full_puzzle_hash)
            ...
            for puzzle_list_coin in additions.coins:
                ...
                if puzzle_hash == full_puzzle_hash:
                    for coin in coins:
                        future_parent = LineageProof(coin.parent_coin_info, innerpuz.get_tree_hash(), coin.amount)
                        await self.add_parent(coin.name(), future_parent, False)
                        if coin.name() in all_parents:
                            continue
                        else:
                            did_info = DIDInfo(origin, backup_ids, num_of_backup_ids_needed, self.did_info.parent_info, innerpuz, coin, new_puzhash, new_pubkey)
                            await self.save_info(did_info, False)
    ...
    def puzzle_for_pk(self, pubkey: bytes) -> Program:
        innerpuz = did_wallet_puzzles.create_innerpuz(pubkey, self.did_info.backup_ids, self.did_info.num_of_backup_ids_needed)
        if self.did_info.origin_coin is not None:
            return did_wallet_puzzles.create_fullpuz(innerpuz, self.did_info.origin_coin.name())
        return did_wallet_puzzles.create_fullpuz(innerpuz, 0)

    async def get_new_puzzle(self) -> Program:
        return self.puzzle_for_pk(bytes((await self.wallet_state_manager.get_unused_derivation_record(self.wallet_info.id)).pubkey))
    ...
    async def create_update_spend(self):
        ...
        innersol = Program.to([1, coin.amount, [], new_puzhash])
        innerpuz = self.did_info.current_inner
        full_puzzle = did_wallet_puzzles.create_fullpuz(innerpuz, self.did_info.origin_coin.name())
        ...
        fullsol = Program.to([
         ...
         innersol])
        list_of_solutions = [
         CoinSpend(coin, full_puzzle, fullsol)]
        message = Program.to([new_puzhash, coin.amount, []]).get_tree_hash() + coin.name() + self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA
    ...
    async def create_message_spend(self, messages: List[bytes], new_innerpuzhash: Optional[bytes32]=None):
        ...
        innersol = Program.to([1, coin.amount, messages, new_innerpuzhash])
        full_puzzle = did_wallet_puzzles.create_fullpuz(innerpuz, self.did_info.origin_coin.name())
        parent_info = await self.get_parent_for_coin(coin)
        assert parent_info is not None
        fullsol = Program.to([
        ...
        list_of_solutions = [
         CoinSpend(coin, full_puzzle, fullsol)]
        message = Program.to([new_innerpuzhash, coin.amount, messages]).get_tree_hash() + coin.name() + self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA
        pubkey = did_wallet_puzzles.get_pubkey_from_innerpuz(innerpuz)
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(pubkey)
        private = master_sk_to_wallet_sk(self.wallet_state_manager.private_key, index)
        signature = AugSchemeMPL.sign(private, message)
    ...
    async def create_exit_spend(self, puzhash: bytes32):
        ...
        innersol = Program.to([0, amount, puzhash])
        innerpuz = self.did_info.current_inner
        full_puzzle = did_wallet_puzzles.create_fullpuz(innerpuz, self.did_info.origin_coin.name())
        parent_info = await self.get_parent_for_coin(coin)
        assert parent_info is not None
        ...
        list_of_solutions = [
         CoinSpend(coin, full_puzzle, fullsol)]
        message = Program.to([amount, puzhash]).get_tree_hash() + coin.name() + self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA
        pubkey = did_wallet_puzzles.get_pubkey_from_innerpuz(innerpuz)
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(pubkey)
        private = master_sk_to_wallet_sk(self.wallet_state_manager.private_key, index)
        signature = AugSchemeMPL.sign(private, message)
    ...
    async def create_attestment(self, recovering_coin_name, newpuz, pubkey, filename=None):
        assert self.did_info.current_inner is not None
        assert self.did_info.origin_coin is not None
        coins = await self.select_coins(1)
        if not (coins is not None and coins != set()):
    ...
    async def load_attest_files_for_recovery_spend(self, filenames):
        ...
        try:
            ...
            my_recovery_list = self.did_info.backup_ids
            info_list = []
            ...
            return (
             info_list, message_spend_bundle)
        except Exception:
            raise

    async def recovery_spend(self, coin: Coin, puzhash: bytes, parent_innerpuzhash_amounts_for_recovery_ids: List[Tuple[(bytes, bytes, int)]], pubkey: G1Element, spend_bundle: SpendBundle) -> SpendBundle:
        assert self.did_info.origin_coin is not None
        innersol = Program.to([
         2,
         coin.amount,
    ...
    async def get_new_innerpuz(self) -> Program:
        ...
        innerpuz = did_wallet_puzzles.create_innerpuz(pubkey, self.did_info.backup_ids, uint64(self.did_info.num_of_backup_ids_needed))
        return innerpuz
    ...
    async def inner_puzzle_for_did_puzzle(self, did_hash: bytes32) -> Program:
        record = await self.wallet_state_manager.puzzle_store.get_derivation_record_for_puzzle_hash(did_hash.hex())
        inner_puzzle = did_wallet_puzzles.create_innerpuz(bytes(record.pubkey), self.did_info.backup_ids, self.did_info.num_of_backup_ids_needed)
        return inner_puzzle
    ...
    async def generate_new_decentralised_id(self, amount: uint64) -> Optional[SpendBundle]:
        ...
        announcement_message = Program.to([did_puzzle_hash, amount, bytes(128)]).get_tree_hash()
        announcement_set.add(Announcement(launcher_coin.name(), announcement_message).name())
        tx_record = await self.standard_wallet.generate_signed_transaction(amount, genesis_launcher_puz.get_tree_hash(), uint64(0), origin.name(), coins, None, False, announcement_set)
        genesis_launcher_solution = Program.to([did_puzzle_hash, amount, bytes(128)])
        launcher_cs = CoinSpend(launcher_coin, genesis_launcher_puz, genesis_launcher_solution)
        launcher_sb = SpendBundle([launcher_cs], AugSchemeMPL.aggregate([]))
        eve_coin = Coin(launcher_coin.name(), did_puzzle_hash, amount)
        future_parent = LineageProof(eve_coin.parent_coin_info, did_inner_hash, eve_coin.amount)
        eve_parent = LineageProof(launcher_coin.parent_coin_info, launcher_coin.puzzle_hash, launcher_coin.amount)
        ...
        if tx_record is None or tx_record.spend_bundle is None:
            return
        did_info = DIDInfo(launcher_coin, self.did_info.backup_ids, self.did_info.num_of_backup_ids_needed, self.did_info.parent_info, did_inner, None, None, None)
        await self.save_info(did_info, False)
    ...
    async def generate_eve_spend(self, coin: Coin, full_puzzle: Program, innerpuz: Program):
        ...
        list_of_solutions = [
         CoinSpend(coin, full_puzzle, fullsol)]
        message = Program.to([innerpuz.get_tree_hash(), coin.amount, []]).get_tree_hash() + coin.name() + self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA
        pubkey = did_wallet_puzzles.get_pubkey_from_innerpuz(innerpuz)
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(pubkey)
        private = master_sk_to_wallet_sk(self.wallet_state_manager.private_key, index)
        signature = AugSchemeMPL.sign(private, message)
    ...
    async def get_spendable_balance(self, unspent_records=None) -> uint64:
        spendable_am = await self.wallet_state_manager.get_confirmed_spendable_balance_for_wallet(self.wallet_info.id, unspent_records)
        return spendable_am
    ...
    async def add_parent(self, name: bytes32, parent: Optional[LineageProof], in_transaction: bool):
        ...
        did_info = DIDInfo(self.did_info.origin_coin, self.did_info.backup_ids, self.did_info.num_of_backup_ids_needed, current_list, self.did_info.current_inner, self.did_info.temp_coin, self.did_info.temp_puzhash, self.did_info.temp_pubkey)
    ...
    async def update_recovery_list(self, recover_list: List[bytes], num_of_backup_ids_needed: uint64):
        if num_of_backup_ids_needed > len(recover_list):
            return False
        did_info = DIDInfo(self.did_info.origin_coin, recover_list, num_of_backup_ids_needed, self.did_info.parent_info, self.did_info.current_inner, self.did_info.temp_coin, self.did_info.temp_puzhash, self.did_info.temp_pubkey)
        await self.save_info(did_info, False)
        await self.wallet_state_manager.update_wallet_puzzle_hashes(self.wallet_info.id)
        return True
