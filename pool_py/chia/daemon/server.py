...
# Embedded file name: chia\daemon\server.py
import asyncio, json, logging, os, signal, subprocess, sys, time, traceback, uuid
...
class WebSocketServer:
    ...
    async def start(self):
        ...
        self.websocket_server = await serve((self.safe_handle),
          (self.self_hostname),
          (self.daemon_port),
          max_size=50000000,
          ping_interval=500,
    ...
    async def handle_message(self, websocket: WebSocketServerProtocol, message: WsRpcMessage) -> Tuple[(Optional[str], List[Any])]:
        """
        ...
        if destination != 'daemon':
            ...
            if destination in self.connections:
                ...
                return (
                 dict_to_json_str(message), sockets)
            return (
             None, [])
        data = message['data']
        commands_with_data = [
    ...
    def _is_serial_plotting_running(self, queue: str='default') -> bool:
        ...
        for item in self.plots_queue:
            if item['queue'] == queue:
                if item['parallel'] is False:
                    if item['state'] is PlotState.RUNNING:
                        response = True
    ...
    def _run_next_serial_plotting(self, loop: asyncio.AbstractEventLoop, queue: str='default'):
        ...
        for item in self.plots_queue:
            if item['queue'] == queue:
                if item['state'] is PlotState.SUBMITTED:
                    if item['parallel'] is False:
    ...
    async def start_plotting(self, request: Dict[(str, Any)]):
        ...
        for k in range(count):
            ...
            if not parallel is True or can_start_serial_plotting:
        ...
        response = {'success':True,  'service_name':service_name}
        return response
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    ...
    process = subprocess.Popen(service_array,
      shell=False,
      stderr=outfile,
      stdout=outfile,
...
def launch_service(root_path: Path, service_command) -> Tuple[(subprocess.Popen, Path)]:
    ...
    process = subprocess.Popen(service_array,
      shell=False, startupinfo=startupinfo, creationflags=creationflags, env=environ_copy)
...
async def kill_process(process: subprocess.Popen, root_path: Path, service_name: str, id: str, delay_before_kill: int=15) -> bool:
    pid_path = pid_path_for_service(root_path, service_name, id)
    if sys.platform == 'win32' or sys.platform == 'cygwin':
...
async def kill_service(root_path: Path, services: Dict[(str, subprocess.Popen)], service_name: str, delay_before_kill: int=15) -> bool:
...
def create_server_for_daemon(root_path: Path):
    ...
    async def start_service(request):
        ...
        if not (service_name is None or validate_service(service_name)):
...
async def async_run_daemon(root_path: Path) -> int:
    ...
    sys.stdout.flush()
    json_msg = dict_to_json_str({'message':'cert_path', 
     'success':True, 
     'cert':f"{crt_path}", 
     'key':f"{key_path}", 
