...
# Embedded file name: chia\server\address_manager_store.py
import logging
from typing import Dict, List, Tuple
import aiosqlite
from chia.server.address_manager import BUCKET_SIZE, NEW_BUCKET_COUNT, NEW_BUCKETS_PER_ADDRESS, AddressManager, ExtendedPeerInfo
...
class AddressManagerStore:
    ...
    async def set_metadata(self, metadata) -> None:
        for key, value in metadata:
            cursor = await self.db.execute('INSERT OR REPLACE INTO peer_metadata VALUES(?, ?)', (
             key, value))
            await cursor.close()
    ...
    async def set_nodes(self, node_list) -> None:
        for node_id, peer_info in node_list:
            cursor = await self.db.execute('INSERT OR REPLACE INTO peer_nodes VALUES(?, ?)', (
    ...
    async def set_new_table(self, entries) -> None:
        for node_id, bucket in entries:
            cursor = await self.db.execute('INSERT OR REPLACE INTO peer_new_table VALUES(?, ?)', (
             node_id, bucket))
            await cursor.close()
    ...
    async def serialize(self, address_manager: AddressManager):
        ...
        for node_id, info in address_manager.map_info.items():
            ...
            if info.ref_count > 0:
                if not count_ids != address_manager.new_count:
