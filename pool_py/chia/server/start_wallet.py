...
# Embedded file name: chia\server\start_wallet.py
import pathlib
...
def service_kwargs_for_wallet(root_path: pathlib.Path, config: Dict, consensus_constants: ConsensusConstants, keychain: Keychain) -> Dict:
    ...
    if port is not None:
        kwargs.update(advertised_port=(config['port']),
          server_listen_ports=[
         config['port']])
    rpc_port = config.get('rpc_port')
    if rpc_port is not None:
        kwargs['rpc_info'] = (
         WalletRpcApi, config['rpc_port'])
    return kwargs
