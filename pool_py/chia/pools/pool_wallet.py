...
# Embedded file name: chia\pools\pool_wallet.py
import logging, time
...
from chia.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState, PoolState, FARMING_TO_POOL, SELF_POOLING, LEAVING_POOL, create_pool_state
...
from chia.pools.pool_puzzles import create_waiting_room_inner_puzzle, create_full_puzzle, SINGLETON_LAUNCHER, create_pooling_inner_puzzle, solution_to_extra_data, pool_state_to_inner_puzzle, get_most_recent_singleton_coin_from_coin_spend, launcher_id_to_p2_puzzle_hash, create_travel_spend, uncurry_pool_member_inner_puzzle, create_absorb_spend, is_pool_member_inner_puzzle, is_pool_waitingroom_inner_puzzle, uncurry_pool_waitingroom_inner_puzzle, get_delayed_puz_info_from_launcher_spend
from chia.util.ints import uint8, uint32, uint64
from chia.wallet.derive_keys import master_sk_to_pooling_authentication_sk, find_owner_sk
import chia.wallet.sign_coin_spends as sign_coin_spends
...
class PoolWallet:
    ...
    def _verify_pooling_state(cls, state) -> Optional[str]:
        ...
        if state.relative_lock_height < cls.MINIMUM_RELATIVE_LOCK_HEIGHT:
            err += f" Pool relative_lock_height ({state.relative_lock_height})is less than recommended minimum ({cls.MINIMUM_RELATIVE_LOCK_HEIGHT})"
        else:
            if state.relative_lock_height > cls.MAXIMUM_RELATIVE_LOCK_HEIGHT:
                err += f" Pool relative_lock_height ({state.relative_lock_height})is greater than recommended maximum ({cls.MAXIMUM_RELATIVE_LOCK_HEIGHT})"
    ...
    def _verify_pool_state(cls, state: PoolState) -> Optional[str]:
        ...
        if state.version > POOL_PROTOCOL_VERSION:
            return f"Detected pool protocol version {state.version}, which is newer than this wallet's version ({POOL_PROTOCOL_VERSION}). Please upgrade to use this pooling wallet"
        if state.state == PoolSingletonState.SELF_POOLING:
            return cls._verify_self_pooled(state)
        if state.state == PoolSingletonState.FARMING_TO_POOL or state.state == PoolSingletonState.LEAVING_POOL:
    ...
    async def get_current_state(self) -> PoolWalletInfo:
        ...
        launcher_coin = all_spends[0].coin
        delayed_seconds, delayed_puzhash = get_delayed_puz_info_from_launcher_spend(all_spends[0])
        tip_singleton_coin = get_most_recent_singleton_coin_from_coin_spend(all_spends[-1])
        ...
        while extra_data is None:
            full_spend = all_spends[curr_spend_i]
            extra_data = solution_to_extra_data(full_spend)
            last_singleton_spend_height = uint32(history[curr_spend_i][0])
            curr_spend_i -= 1

        assert extra_data is not None
        current_inner = pool_state_to_inner_puzzle(extra_data, launcher_coin.name(), self.wallet_state_manager.constants.GENESIS_CHALLENGE, delayed_seconds, delayed_puzhash)
        config = load_config(DEFAULT_ROOT_PATH, 'config.yaml')
        contractMap = {}
        p2_chia_map_value = None
        if 'pool' in config:
            if 'p2_chia_contract_or_pool_public_key_map' in config['pool']:
                contractMap = config['pool']['p2_chia_contract_or_pool_public_key_map']
                if p2_singleton_puzzle_hash.hex() in contractMap:
                    p2_chia_map_value = contractMap[p2_singleton_puzzle_hash.hex()]
        return PoolWalletInfo(extra_data, self.target_state, launcher_coin, launcher_id, p2_singleton_puzzle_hash, current_inner, tip_singleton_coin.name(), last_singleton_spend_height, p2_chia_map_value)
    ...
    async def update_pool_config(self, make_new_authentication_key: bool):
        ...
        pool_config_list = load_pool_config(self.wallet_state_manager.root_path)
        pool_config_dict = {c.launcher_id: c for c in pool_config_list}
        existing_config = pool_config_dict.get(current_state.launcher_id, None)
        if make_new_authentication_key or existing_config is None:
            new_auth_sk = master_sk_to_pooling_authentication_sk(self.wallet_state_manager.private_key, uint32(self.wallet_id), uint32(0))
            auth_pk = new_auth_sk.get_g1()
        ...
        new_config = PoolWalletConfig(current_state.launcher_id, current_state.current.pool_url if current_state.current.pool_url else '', payout_instructions, current_state.current.target_puzzle_hash, current_state.p2_singleton_puzzle_hash, current_state.current.owner_pubkey, auth_pk, current_state.p2_chia_contract_or_pool_public_key)
    ...
    def get_next_interesting_coin_ids(spend: CoinSpend) -> List[bytes32]:
        coin = get_most_recent_singleton_coin_from_coin_spend(spend)
        if coin is not None:
            return [coin.name()]
        return []
    ...
    async def apply_state_transitions(self, block_spends: List[CoinSpend], block_height: uint32):
        ...
        """
        coin_name_to_spend = {cs.coin.name(): cs for cs in block_spends}
        tip = await self.get_tip()
        tip_height = tip[0]
        tip_spend = tip[1]
        assert block_height >= tip_height
        while 1:
            tip_coin = get_most_recent_singleton_coin_from_coin_spend(tip_spend)
            assert tip_coin is not None
            spent_coin_name = tip_coin.name()
            if spent_coin_name not in coin_name_to_spend:
                break
            else:
                spend = coin_name_to_spend[spent_coin_name]
                ...
                for _, added_spend in reversed(self.wallet_state_manager.pool_store.get_spends_for_wallet(self.wallet_id)):
                    latest_state = solution_to_extra_data(added_spend)
    ...
    async def rewind(self, block_height: int) -> bool:
        ...
        """
        try:
            history = self.wallet_state_manager.pool_store.get_spends_for_wallet(self.wallet_id).copy()
            prev_state = await self.get_current_state()
            await self.wallet_state_manager.pool_store.rollback(block_height, self.wallet_id)
            if len(history) > 0:
                if history[0][0] > block_height:
                    await self.wallet_state_manager.interested_store.remove_interested_puzzle_hash((prev_state.p2_singleton_puzzle_hash),
                      in_transaction=True)
                    return True
    ...
    @staticmethod
    async def create(wallet_state_manager: Any, wallet: Wallet, launcher_coin_id: bytes32, block_spends: List[CoinSpend], block_height: uint32, in_transaction: bool, name: str=None):
        """
        ...
        self.wallet_info = await wallet_state_manager.user_store.create_wallet('Pool wallet',
          (WalletType.POOLING_WALLET.value), '', in_transaction=in_transaction)
        ...
        p2_puzzle_hash = (await self.get_current_state()).p2_singleton_puzzle_hash
        await self.wallet_state_manager.interested_store.add_interested_puzzle_hash(p2_puzzle_hash, self.wallet_id, True)
    ...
    @staticmethod
    async def create_from_db(wallet_state_manager, wallet, wallet_info, name=None):
        """
    ...
    @staticmethod
    async def create_new_pool_wallet_transaction(wallet_state_manager: Any, main_wallet: Wallet, initial_target_state: PoolState, fee: uint64=uint64(0), p2_singleton_delay_time: Optional[uint64]=None, p2_singleton_delayed_ph: Optional[bytes32]=None, chia_contract_address: Optional[str]=None) -> Tuple[(TransactionRecord, bytes32, bytes32)]:
        """
        ...
        spend_bundle, singleton_puzzle_hash, launcher_coin_id = await PoolWallet.generate_launcher_spend(standard_wallet, uint64(1), initial_target_state, wallet_state_manager.constants.GENESIS_CHALLENGE, p2_singleton_delay_time, p2_singleton_delayed_ph)
        if spend_bundle is None:
            raise ValueError('failed to generate ID for wallet')
        standard_wallet_record = TransactionRecord(confirmed_at_height=(uint32(0)),
        ...
        p2_singleton_puzzle_hash = launcher_id_to_p2_puzzle_hash(launcher_coin_id, p2_singleton_delay_time, p2_singleton_delayed_ph)
        chia_contract_address = chia_contract_address.strip()
        if chia_contract_address is not None:
            config = load_config(DEFAULT_ROOT_PATH, 'config.yaml')
            contractMap = {}
            if 'p2_chia_contract_or_pool_public_key_map' in config['pool']:
                contractMap = config['pool']['p2_chia_contract_or_pool_public_key_map']
            contractMap[p2_singleton_puzzle_hash.hex()] = chia_contract_address
            config['pool']['p2_chia_contract_or_pool_public_key_map'] = contractMap
            save_config(DEFAULT_ROOT_PATH, 'config.yaml', config)
        return (
         standard_wallet_record, p2_singleton_puzzle_hash, launcher_coin_id)

    async def sign(self, coin_spend: CoinSpend) -> SpendBundle:

        async def pk_to_sk(pk):
            owner_sk = await find_owner_sk([self.wallet_state_manager.private_key], pk)
            assert owner_sk is not None
            return owner_sk

        return await sign_coin_spends([
         coin_spend], pk_to_sk, self.wallet_state_manager.constants.AGG_SIG_ME_ADDITIONAL_DATA, self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM)

    async def generate_travel_transaction(self) -> TransactionRecord:
        pool_wallet_info = await self.get_current_state()
        spend_history = await self.get_spend_history()
        last_coin_spend = spend_history[-1][1]
        ...
        if pool_wallet_info.current.state in [FARMING_TO_POOL]:
            next_state = create_pool_state(LEAVING_POOL, pool_wallet_info.current.target_puzzle_hash, pool_wallet_info.current.owner_pubkey, pool_wallet_info.current.pool_url, pool_wallet_info.current.relative_lock_height)
        new_inner_puzzle = pool_state_to_inner_puzzle(next_state, pool_wallet_info.launcher_coin.name(), self.wallet_state_manager.constants.GENESIS_CHALLENGE, delayed_seconds, delayed_puzhash)
        new_full_puzzle = SerializedProgram.from_program(create_full_puzzle(new_inner_puzzle, pool_wallet_info.launcher_coin.name()))
        outgoing_coin_spend, inner_puzzle = create_travel_spend(last_coin_spend, pool_wallet_info.launcher_coin, pool_wallet_info.current, next_state, self.wallet_state_manager.constants.GENESIS_CHALLENGE, delayed_seconds, delayed_puzhash)
        ...
        if is_pool_member_inner_puzzle(inner_puzzle):
            inner_f, target_puzzle_hash, p2_singleton_hash, pubkey_as_program, pool_reward_prefix, escape_puzzle_hash = uncurry_pool_member_inner_puzzle(inner_puzzle)
            pk_bytes = bytes(pubkey_as_program.as_atom())
            assert len(pk_bytes) == 48
            owner_pubkey = G1Element.from_bytes(pk_bytes)
            if not owner_pubkey == pool_wallet_info.current.owner_pubkey:
                raise AssertionError
        else:
            if is_pool_waitingroom_inner_puzzle(inner_puzzle):
                target_puzzle_hash, relative_lock_height, owner_pubkey, p2_singleton_hash = uncurry_pool_waitingroom_inner_puzzle(inner_puzzle)
        ...
        tx_record = TransactionRecord(confirmed_at_height=(uint32(0)),
          created_at_time=(uint64(int(time.time()))),
    ...
    @staticmethod
    async def generate_launcher_spend(standard_wallet: Wallet, amount: uint64, initial_target_state: PoolState, genesis_challenge: bytes32, delay_time: uint64, delay_ph: bytes32) -> Tuple[(SpendBundle, bytes32, bytes32)]:
        """
        Creates the initial singleton, which includes spending an origin coin, the launcher, and creating a singleton
        with the "pooling" inner state, which can be either self pooling or using a pool
        """
        coins = await standard_wallet.select_coins(amount)
        if coins is None:
            raise ValueError('Not enough coins to create pool wallet')
        assert len(coins) == 1
        launcher_parent = coins.copy().pop()
        genesis_launcher_puz = SINGLETON_LAUNCHER
        launcher_coin = Coin(launcher_parent.name(), genesis_launcher_puz.get_tree_hash(), amount)
        escaping_inner_puzzle = create_waiting_room_inner_puzzle(initial_target_state.target_puzzle_hash, initial_target_state.relative_lock_height, initial_target_state.owner_pubkey, launcher_coin.name(), genesis_challenge, delay_time, delay_ph)
        escaping_inner_puzzle_hash = escaping_inner_puzzle.get_tree_hash()
        self_pooling_inner_puzzle = create_pooling_inner_puzzle(initial_target_state.target_puzzle_hash, escaping_inner_puzzle_hash, initial_target_state.owner_pubkey, launcher_coin.name(), genesis_challenge, delay_time, delay_ph)
        ...
        full_pooling_puzzle = create_full_puzzle(puzzle, launcher_id=(launcher_coin.name()))
        puzzle_hash = full_pooling_puzzle.get_tree_hash()
        ...
        create_launcher_tx_record = await standard_wallet.generate_signed_transaction(amount, genesis_launcher_puz.get_tree_hash(), uint64(0), None, coins, None, False, announcement_set)
        if not (create_launcher_tx_record is not None and create_launcher_tx_record.spend_bundle is not None):
            raise AssertionError
        genesis_launcher_solution = Program.to([puzzle_hash, amount, extra_data_bytes])
        launcher_cs = CoinSpend(launcher_coin, SerializedProgram.from_program(genesis_launcher_puz), SerializedProgram.from_program(genesis_launcher_solution))
        launcher_sb = SpendBundle([launcher_cs], G2Element())
        full_spend = SpendBundle.aggregate([create_launcher_tx_record.spend_bundle, launcher_sb])
        return (
         full_spend, puzzle_hash, launcher_coin.name())
    ...
    async def join_pool(self, target_state: PoolState):
        ...
        if await self.have_unconfirmed_transaction():
            raise ValueError('Cannot claim due to unconfirmed transaction. If this is stuck, delete the unconfirmed transaction.')
        current_state = await self.get_current_state()
        if current_state.current == target_state:
            ...
            return
        if self.target_state is not None:
            raise ValueError(f"Cannot change to state {target_state} when already having target state: {self.target_state}")
        PoolWallet._verify_initial_target_state(target_state)
        if current_state.current.state == LEAVING_POOL:
            history = await self.get_spend_history()
            last_height = history[-1][0]
            if self.wallet_state_manager.get_peak().height <= last_height + current_state.current.relative_lock_height:
                raise ValueError(f"Cannot join a pool until height {last_height + current_state.current.relative_lock_height}")
    ...
    async def self_pool(self):
        ...
        if current_state.current.state == LEAVING_POOL:
            ...
            last_height = history[-1][0]
            if self.wallet_state_manager.get_peak().height <= last_height + current_state.current.relative_lock_height:
                raise ValueError(f"Cannot self pool until height {last_height + current_state.current.relative_lock_height}")
        self.target_state = create_pool_state(SELF_POOLING,
          owner_puzzlehash, owner_pubkey, pool_url=None, relative_lock_height=(uint32(0)))
        tx_record = await self.generate_travel_transaction()
        await self.wallet_state_manager.add_pending_transaction(tx_record)
        return tx_record
    ...
    async def claim_pool_rewards(self, fee: uint64) -> TransactionRecord:
        if await self.have_unconfirmed_transaction():
            raise ValueError('Cannot claim due to unconfirmed transaction. If this is stuck, delete the unconfirmed transaction.')
        unspent_coin_records = list(await self.wallet_state_manager.coin_store.get_unspent_coins_for_wallet(self.wallet_id))
        if len(unspent_coin_records) == 0:
            raise ValueError('Nothing to claim, no transactions to p2_singleton_puzzle_hash')
        farming_rewards = await self.wallet_state_manager.tx_store.get_farming_rewards()
        coin_to_height_farmed = {}
        for tx_record in farming_rewards:
            height_farmed = tx_record.height_farmed(self.wallet_state_manager.constants.GENESIS_CHALLENGE)
            if not height_farmed is not None:
                raise AssertionError
            else:
        ...
        for coin_record in unspent_coin_records:
            ...
            if len(all_spends) >= 100:
                ...
                absorb_spend = create_absorb_spend(last_solution, current_state.current, current_state.launcher_coin, coin_to_height_farmed[coin_record.coin], self.wallet_state_manager.constants.GENESIS_CHALLENGE, delayed_seconds, delayed_puzhash)
                last_solution = absorb_spend[0]
                all_spends += absorb_spend
                total_amount += coin_record.coin.amount
                self.log.info(f"Farmer coin: {coin_record.coin} {coin_record.coin.name()} {coin_to_height_farmed[coin_record.coin]}")

        if len(all_spends) == 0:
            raise ValueError('Nothing to claim, no unspent coinbase rewards')
        spend_bundle = SpendBundle(all_spends, G2Element())
        absorb_transaction = TransactionRecord(confirmed_at_height=(uint32(0)),
          created_at_time=(uint64(int(time.time()))),
    ...
    async def new_peak(self, peak: BlockRecord) -> None:
        ...
        if self.target_state.state in [FARMING_TO_POOL, SELF_POOLING]:
            if pool_wallet_info.current.state == LEAVING_POOL:
                ...
                if curr.height > leave_height + 2:
                    unconfirmed = await self.wallet_state_manager.tx_store.get_unconfirmed_for_wallet(self.wallet_id)
                    next_tip = get_most_recent_singleton_coin_from_coin_spend(tip_spend)
                    assert next_tip is not None
                    if any([rem.name() == next_tip.name() for tx_rec in unconfirmed for rem in iter((tx_rec.removals))]):
                    ...
                    if self.target_state.state == SELF_POOLING:
                        ...
                        if not self.target_state.pool_url is None:
                            raise AssertionError
                    else:
                        if self.target_state.state == FARMING_TO_POOL:
